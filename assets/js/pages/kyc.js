...
function submit_kyc(form,form_type,type)
{
   // $('.overlayy').css('display','block');
	...
	var bankid       = $('#submit_bnk_detail').val();
		...
		{
			var records      =  {'type':type,'form_type':form_type,'ifsc':ifsc,'bank_acc_no':bank_acc_no,'con_acc_no':con_acc_no} //,'name':name,'bank_branch':bank_branch
		    $(".overlayy").css("display","block");
		 setTimeout(function(){ 
			$.ajax({
				url:baseURL+"index.php/chitscheme/kyc_details",
				type : "POST",
				data: records ,
				async:false,
				dataType: "json",
				success : function(data) {
					...
					if(data.status==true){
						...
						window.location.href= baseURL+"index.php/chitscheme/schemes";
