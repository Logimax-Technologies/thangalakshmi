...
//due count	
    ...
    $(".brn_btn").on("click",function(ev){
    	$(".brn_btn").removeClass("btn-warning");
...
payment = {
	...
	function calculate_payAmt(schData){
        ...
        var gold_metal_rate = parseFloat(schData.gold_metal_rate);
        var gst = 0;
		...
		console.log(gst_percent+"-"+gold_metal_rate+"-"+gst_type+"-"+sel_dues+"-"+discount);
        var metal_weight = 0;
        var weight = 0;
        ...
        if(gst_percent > 0){
            if(gst_type == 0){ 
            	...
            	// Set Value
            	if((schData.flexible_sch_type == 2 || schData.flexible_sch_type == 3) || (schData.scheme_type == 2 || schData.scheme_type == 3 )){
            	    metal_weight = (gst_removed_amt+discount)/gold_metal_rate;
            	    weight = setMetalWgt(metal_weight);
            	}
            	else if(schData.scheme_type == 1  && schData.is_fixed_weight == 0){
                    metal_weight = $('#selected_weight').val();
                    weight = setMetalWgt(metal_weight);
            	}
            	else if(schData.scheme_type == 1 && schData.is_fixed_weight == 1){
            	...
            	// Set Value
            	if(schData.flexible_sch_type == 2 || schData.scheme_type == 2 ){
