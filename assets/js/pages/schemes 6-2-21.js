...
//existing reg sub//
   ...
   if(one_time_premium==0)
   {
		...
		if(pay_amt != "" && flx_denomintion!='' && get_amt_in_schjoin==1 && scheme_type==3 && flexible_sch_type!=3)
		{
	...
	$.ajax({
			...
			url:baseURL+"index.php/chitscheme/get_classification/"+id+"?nocache=" + my_Date.getUTCSeconds(),
				   ...
				   success : function(result) {
						...
						var modal_desc = $("<p class='schemeName'>"+result.classification_name+"</p><div><div class='heading'></div><div class='description'>"+result.description+"</div></div>");
...
function confirm_scheme(schemeId,schemeName,companyName,newSchjoinonline,isReferal,is_pan_required,cus_single,emp_single,cus_ref_code,emp_ref_code,is_branchwise_cus_reg,cusName_edit,allow_join,get_amt_in_schjoin,min_amount,max_amount,flx_denomintion,scheme_type,flexible_sch_type,one_time_premium,e){
...
/*	$('input[name*="min_amount"]').remove();
	...
	if(newSchjoinonline == 1)
	{
	 ...
	 console.log(cusName_edit);
	  if(one_time_premium == 0 && cusName_edit != 1 )
	  {
	    if(schjoin_amt == 0 || sch_type!=3 || flx_sch_type==3)  //Getting Payment Amount From Customer
        {
        if(($('#branch_set').val()==1 && is_branchwise_cus_reg!=1 )) //Disable Branch When Branchwise customer registration is Enabled
        {
            get_branch();
        ...
        {
                if(($('#branch_set').val()==1 && is_branchwise_cus_reg!=1 ))
                {
                    get_branch();
                ...
                {	
                        ...
                        if(isReferal==0)
                        {
                                ...
                                {
                                        var confirmText = "<p>Are you sure you want to join the scheme <strong>"+schemeName+"</strong> ?</p><p>Account Name"+"&nbsp;&nbsp;&nbsp;"+"  :"+"&nbsp;&nbsp;"+"<input type='name' id='acc_name' name='id_customer'   class='form-control width50' readonly='readonly' /></p>"+"<p>Payment Amount"+""+"  :"+"&nbsp;&nbsp;"+"<input type='number' class='form-control width50' name='payment_amount' id='pay_amt' placeholder='Enter Payment Amount' required/></p>"+"<p>By joining the scheme you agree with the Terms & Conditions of "+'<strong>'+companyName+'</strong>'+"";
                                }
                        }
                        else
                        {	
	     ...
	     else if(one_time_premium == 0 && cusName_edit == 1)
	  {
        ...
        {	
                if(isReferal==0)
                {
                        ...
                        {
                                var confirmText = "<p>Are you sure you want to join the Purchase plan <strong>"+schemeName+"</strong> ?</p><p>Account Name"+"&nbsp;&nbsp;&nbsp;"+"  :"+"&nbsp;&nbsp;"+"<input type='text' class='form-control width50' name='account_name' id='acc_name' placeholder='Name' required/></p>"+"<p>By joining the Purchase plan you agree with the Terms & Conditions of "+'<strong>'+companyName+'</strong>'+"";
                        }
                }
                else
                {	
                        if(cus_single==0 && emp_single==0)
                        {
                            ...
                            {	
                                    ...
                                    {
                                        var confirmText = "<p>Are you sure you want to join the Purchase plan <strong>"+schemeName+"</strong> ?</p><p>Account Name"+"&nbsp;&nbsp;&nbsp;"+"  :"+"&nbsp;&nbsp;"+"<input type='text' class='form-control width50' name='account_name' id='acc_name' placeholder='Name' required/></p>"+"<p>By joining the scheme you agree with the Terms & Conditions of "+'<strong>'+companyName+'</strong>'+"";
                                    }
                            }
                            else if(cus_ref_code=='' && emp_ref_code=='')
                            {
        ...
        else
        {
                ...
                {	
                        ...
                        if(isReferal==0)
                        {
                                ...
                                {
                                        var confirmText = "<p>Are you sure you want to join the Purchase plan <strong>"+schemeName+"</strong> ?</p><p>Account Name"+"&nbsp;&nbsp;&nbsp;"+"  :"+"&nbsp;&nbsp;"+"<input type='text' class='form-control width50' name='account_name' id='acc_name' placeholder='Name' required/></p>"+"<p>Payment Amount"+""+"  :"+"&nbsp;&nbsp;"+"<input type='number' class='form-control width50' name='payment_amount' id='pay_amt' placeholder='Enter Payment Amount' required/></p>"+"<p>By joining the Purchase plan you agree with the Terms & Conditions of "+'<strong>'+companyName+'</strong>'+"";
                                }
                        }
                        else
                        {	
...
function clsfyTermsbtn(sch_clsfy_id)
	{
		...
		terms_btn = "<button class='btn-warning btn' onclick='javascript:schemeTermsModal("+sch_clsfy_id+");'>Terms & Conditions</button>";
			...
			url:baseURL+"index.php/chitscheme/get_classifications/"+"?nocache=" + my_Date.getUTCSeconds(),
				   ...
				   success : function(result) {
							...
							$.each( result, function( i, v ) {
								if(i==0) {
										active='active';
										terms_btn = "<button class='btn-warning btn' onclick='javascript:schemeTermsModal("+v.id_classification+");'>Terms & Conditions</button>";
										$("#clsfy_tc_block").append(terms_btn); 
    							...
    							var tab_header = $("<li class='"+active+"'><a href='#tab_"+v.id_classification+"' data-toggle='pill' onclick='javascript:clsfyTermsbtn("+v.id_classification+");' >"+v.classification_name+"</a></li>");
				...
				success : function(result) {		
					...
					$.each( result.schemes, function( key, value ) {						
                        ...
                        if(cus_kyc_status == 1)
                        {
                            if(one_time_premium==0)
                            {
                                  join_btn = "<button class='btn-primary btn join-btn' onclick='javascript:confirm_scheme("+value.id_scheme+","+schName+","+cmpName+","+newSchjoinonline+","+isReferal+","+is_pan_required+","+cus_single+","+emp_single+","+cus_ref_code+","+emp_ref_code+","+is_branchwise_cus_reg+","+cusName_edit+","+allow_join+","+get_amt_in_schjoin+","+min_amount+","+max_amount+","+flx_denomintion+","+scheme_type+","+flexible_sch_type+","+one_time_premium+",event);'> Join</button> ";
                            }else{
                                  join_btn = "<button style='font-size: 9px;width: 64px;' class='btn-primary btn join-btn' onclick='javascript:confirm_scheme("+value.id_scheme+","+schName+","+cmpName+","+newSchjoinonline+","+isReferal+","+is_pan_required+","+cus_single+","+emp_single+","+cus_ref_code+","+emp_ref_code+","+is_branchwise_cus_reg+","+cusName_edit+","+allow_join+","+get_amt_in_schjoin+","+min_amount+","+max_amount+","+flx_denomintion+","+scheme_type+","+flexible_sch_type+","+one_time_premium+",event);'> Enquiry</button> ";
                            }
                        }
                        else{
                            var kycURL = baseURL+"index.php/chitscheme/kyc_form";
                            join_btn = "<a href="+kycURL+" class='btn-primary btn join-btn'>Join</a>"
                        }
		 ...
		 $("#spinner").css('display','none');
		   ...
		   $.each(data.schemes, function (key, item) {					  	
			...
			$("#secheme_select").select2({
			 ...
			 //$("#secheme_select").select2("val",(id_scheme!='' && id_scheme>0?id_scheme:''));
...
/*function get_cusname()
{
	$.ajax({
		...
		url: baseURL+'index.php/chitscheme/join_scheme/',

		dataType: 'json',
         success: function(data){
         console.log(data);
	//	$("#acc_name").val(data.cus_name);	
