...
//    					'carat'  		 => $carat,
	 	...
	 	function set_image_ret($id)
    {
     ...
     { 
		...
		$filename = $_FILES['product']['name']['ret_product_img'];	
	...
	public function category($type="",$id="",$status=""){
		...
		{	
			...
			case 'active_category': 
					$data = $this->$model->getActiveCategorymtr();	  
					echo json_encode($data);
					break;
			case 'cat_purity': 
		...
		{	
	 		case "add":
	 				$data=array(
						...
						'code_karigar'         => ($this->input->post("code_karigar")!='' ? $this->input->post("code_karigar"):NULL),
						...
						'address3'             => ($this->input->post("address3")!='' ? $this->input->post("address3"):NULL),
						'id_country'		   => ($this->input->post("id_country")!=null ? $this->input->post("id_country"):NULL),
						'id_state'		       => ($this->input->post("id_state")!=null ? $this->input->post("id_state"):NULL),
						'id_city'		       => ($this->input->post("id_city")!=null ? $this->input->post("id_city") :NULL),
						'contactno1'		   => ($this->input->post("contactno1")!='' ? $this->input->post("contactno1"):NULL),
						'contactno2'		   => ($this->input->post("contactno2")!='' ? $this->input->post("contactno2"):NULL),
						'status_karigar'	   => ($this->input->post("status_karigar")!='' ? $this->input->post("status_karigar"):NULL),
						'email'		           => ($this->input->post("email")!='' ? $this->input->post("email"):NULL),
						'urname'		       => ($this->input->post("urname")!='' ? $this->input->post("urname"):NULL),
						'psword'		       => ($this->input->post("psword")!='' ? $this->input->post("psword"):NULL),
						'image'		           => ($this->input->post("image")!='' && $this->input->post("image")!='undefined' ? $this->input->post("image"):NULL),
					...
					break;
			case "edit":
						$data['user'] = $this->$model->get_karigar($id);
			...
			case 'update_status':
						...
						break;
			case 'active_list': 
					  	$data = $this->$model->getActiveKarigar();	  
						echo json_encode($data);
					    break;
			case 'list':
			...
			default:
						...
						$range['from_date']  = $this->input->post('from_date');
						$range['to_date']  = $this->input->post('to_date');
					  	$karigar = $this->$model->ajax_getkarigar($range['from_date'],$range['to_date']);	 
					  	$access = $this->$SETT_MOD->get_access('karigar/list');
				        $data = array(
				        					'karigar' =>$karigar,
											'access'=>$access
				        			 );  
		...
		switch($type)
		{
					...
					$stone             = $this->input->post('stone');
		 			$data['design']=array(
					  'design_status'          => (!empty($updData['design_status']) ? $updData['design_status'] :NULL ),
						...
						'design_name'		   => (!empty($updData['design_name']) ? $updData['design_name'] :NULL ),
						...
						'mc_cal_type'          => $updData['mc_cal_type'],
						'mc_cal_value'         => (!empty($updData['mc_cal_value']) ? $updData['mc_cal_value'] : 0 ),
						'wastag_value'         => (!empty($updData['wastag_value']) ? $updData['wastag_value'] : 0 ),
			...
			case 'ajax_update_bulk_retdesign':
			        	...
			        	foreach($reqdata as $data)
        				{
        					...
        					if($data['wastag_value'] != ""){
        					    $upd_data['wastag_value']   = $data['wastag_value'];
        					}
        					
        					$upd_data['updated_time']   = date("Y-m-d H:i:s");
        					$upd_data['updated_by']     = $employee;
        					$update_status=$this->$model->updateData($upd_data, 'design_no', $data['design_no'], 'ret_design_master');
        				}
        				if($update_status)
        				{		
        					$this->db->trans_commit();
        					$status=array('status'=>true,'msg'=>'MC / Wastage datta updated successfully');	
        				}
        				else
        				{
        					$this->db->trans_rollback();
...
/*	function old_metal_rate()
  ...
  function get_weight_range_details()
  {
    ...
    $data=$this->$model->weight_details($id_product,$id_design);
    echo json_encode($data);
  }
  ...
  public function reorder_settings($type="",$id="",$status=""){
		...
		switch($type)
		{
			...
			case 'active_design':
					...
					break;
			case 'weight_range':
					$data=$this->$model->ajax_get_weights($_POST['id_product'],$_POST['id_design']); 
					echo json_encode($data); 
			break;
			case 'list':
			...
			default:
					...
					$data 		= $this->$model->ajax_getreorder_settings('',$id_product,$id_design,$id_wt_range);	 
	...
	public function ret_section($type="",$id="")
	{
		...
		switch($type)
		{
			case "Add":
	 				...
	 				$this->db->trans_begin();
	 				$this->$model->insertData($data,'ret_section'); 
		 			if($this->db->trans_status()===TRUE)
		             {
					 	$this->db->trans_commit();
