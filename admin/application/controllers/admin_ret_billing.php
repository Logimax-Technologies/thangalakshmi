...
class Admin_ret_billing extends CI_Controller
{
	...
	*/
            		 				...
            		 				if($insId)
            		 				{
            							...
            							{ 
                	 					...
                	 					if($addData['bill_type']==5) 
                	 					{
                							...
                							$arrayAdv = array(
                							'bill_id'           =>$insId,
                							'advance_weight'    =>($store_as==2 ? ($addData['tot_amt_received']/($addData['rate_calc']==1 ?$metal_rate['goldrate_22ct'] :$metal_rate['silverrate_1gm'])):0),
                							'advance_amount'    =>($store_as==1 ? ($addData['tot_amt_received']):0),
                							'advance_type'      =>1,
            								...
            								foreach($billSale['is_est_details'] as $key => $val){
            									...
            									if(!empty($arrayBillSales))
            									{
            										...
            										if($tagInsert)
            										{
            											...
            											{
            												$existData=array('id_product'=>$billSale['product'][$key],'id_design'=>$billSale['design'][$key],'id_sub_design'=>$billSale['id_sub_design'][$key],'id_branch'=>$addData['id_branch']);
            												$isExist = $this->$model->checkNonTagItemExist($existData);
            												if($isExist['status'] == TRUE)
            												{
            													$nt_data = array(
	...
	public function get_scheme_accounts()
	{
		...
		$data=$this->$model->get_closed_accounts($searchTxt,$id_customer,$id_scheme_acc);
		echo json_encode($data);
	}
	//chit acc
	//Advance Adj
	public function get_advance_details()
	{
		...
		$data=$this->$model->get_advance_details($bill_cus_id,$_POST['is_eda']);
		echo json_encode($data);
	}
	//Advance Adj
	...
	function billing_invoice($id)
	{
		...
		$content = $this->config->item('bill_qrcode_url')."index.php/admin_app_api/printbill/".$qrfilename;
