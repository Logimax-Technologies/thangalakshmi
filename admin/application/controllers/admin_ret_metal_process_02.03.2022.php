	...
	{   
		...
		switch($type)
		{
            ...
            case 'process_save':
                ...
                {
                    $process_no=$this->$model->generate_process_number();
                    $insData=array(
                                    'id_branch'         =>$ho['id_branch'],
                                    'process_no'        =>$process_no,
                                    'process_for'       =>$addData['process_for'],
                                    'id_metal_process'  =>$addData['id_metal_process'],
                                    'id_karigar'        =>$addData['id_karigar'],
                                    'issue_date'        =>date("Y-m-d H:i:s"),
                                    'issued_by'         =>$this->session->userdata('uid'),
                                   );
                    $this->db->trans_begin();
                    ...
                    if($insId)
                    {
                        ...
                        {
                            foreach($item_details['id_metal_pocket'] as $key => $val)
                            {
                                $processDetails=array(
                                                     'id_metal_process' =>$insId,
                                                     'id_pocket'        =>$item_details['id_metal_pocket'][$key],
                                                     'issue_gross_wt'   =>$item_details['gross_wt'][$key],
                                                     'issue_net_wt'     =>$item_details['net_wt'][$key],
                                                     'issue_amount'     =>$item_details['amount'][$key],
                                                     'issue_purity'     =>$item_details['avg_purity'][$key],
                                                     );
                                $this->$model->insertData($processDetails,'ret_old_metal_process_details');
                            }
                            ...
                            if($this->db->trans_status()===TRUE)
        		 		    {
                                ...
                                'module'        => 'Old Metal Process',
                                'operation'     => 'Add',
                                'record'        =>  $insId,  
                                'remark'        => 'Process Issue successfully'
                                );
                                $this->log_model->log_detail('insert','',$log_data);
                                $responseData=array('status'=>TRUE,'message'=>'Process Issue successfully');
        		 		    }else{
                                ...
                                if(!empty($receipt_details))
                                {
                                    ...
                                    foreach($receipt_details['is_melting_select'] as $key => $val)
                                    {
                                       if($receipt_details['is_melting_select'][$key]==1)
                                       {
                                            ...
                                            if($melting_receipt_status)
                                            {
                    			        	        $itemExistData=array('id_ret_category'=>$receipt_details['id_ret_category'][$key],'id_branch'=>$ho['id_branch'],'type'=>0); 
                    			        	        ...
                    			        	        $pur_item_stock_summary = array(
                  										        'id_branch'	        => $ho['id_branch'],
                  										        'id_product'	    => $receipt_details['id_ret_category'][$key],
                  										        'type'              => 0,//From Old Metal Process
                									...
                									if($is_po_item_exist['status']) //IF ITEM EXISTS ALREADY IN TABLE
                									{
                											 ...
                											 $this->$model->updatePurItemData($pur_item_stock_summary,'+');
                									}
                                    ...
                                    foreach($testing_issue_details['is_melting_select'] as $key => $val)
                                    {
                                       if($testing_issue_details['is_melting_select'][$key]==1)
                                       {
                                            $testing_issue_data=array(
                                                'id_old_metal_process' =>$insId,
                                                'id_melting'           =>$testing_issue_details['id_melting'][$key],
                                            ...
                                            {
                                                $TestingIssue=$this->$model->updateData(array('melting_status'=>2),'id_melting',$testing_issue_details['id_melting'][$key],'ret_old_metal_melting');
                                                 $branchDetails=$this->$model->get_branch_details();
                                    ...
                                    foreach($receipt_details['is_melting_select'] as $key => $val)
                                    {
                                       if($receipt_details['is_melting_select'][$key]==1)
                                       {
                                            ...
                                            if($melting_receipt_status)
                                            {
                                                ...
                                                $this->$model->updateData(array('melting_status'=>3),'id_melting',$receipt_details['id_melting'][$key],'ret_old_metal_melting');
                                                //print_r($this->db->last_query());exit;
                                                $pur_items_log=array(
                                                'id_old_metal_process'  =>$insId,
                                                'to_branch'             =>$branchDetails['id_branch'],
                                    ...
                                    foreach($refining_issue_details['is_melting_select'] as $key => $val)
                                    {
                                       if($refining_issue_details['is_melting_select'][$key]==1)
                                       {
                                            $refining_issue_data=array(
                                                'id_old_metal_process' =>$insId,
                                                'id_melting'           =>$refining_issue_details['id_melting'][$key],
                                            ...
                                            {
                                                $TestingIssue=$this->$model->updateData(array('melting_status'=>4),'id_melting',$refining_issue_details['id_melting'][$key],'ret_old_metal_melting');
                                            ...
                                            if($refining_receipt_status)
                                            {
                                                ...
                                                if(!empty($CategoryDetails))
                                                {
                                                   ...
                                                   /*     //UPDATE INTO PURCHASE ITEM STOCK SUMMARY
                                                        if($insStatus)
                                                        {
                                			        	        $itemExistData=array('id_product'=>$orderDetails['id_product'][$key],'id_branch'=>$ho['id_branch'],'type'=>1); 
                                			        	        $is_po_item_exist = $this->$model->checkPurchaseItemStockExist($itemExistData); //CHECK ITEM EXISTS IN TABLE 
                                			        	        $pur_item_stock_summary = array(
                              										        'id_branch'	        => $ho['id_branch'],
                              										        'id_product'	    => $orderDetails['id_product'][$key],
                              										        'type'              => 1,//Bullion Purchase
