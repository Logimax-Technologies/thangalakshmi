...
//							'tax_group_id'			=> (isset($addData['tax_group_id']) ? $addData['tax_group_id'] :NULL ),
    ...
    function create_retag()
    {
    	    ...
    	    if($process_id)
    	    {
            	...
            	else if($report_type==1 && $tag_process==4) //NON TAG SALES RETURN TO NON TAG STOCK
            	{
            	     foreach($reqdata as $tag)
                     {  
                            $this->$model->insertData(array('id_process'=>$process_id,'ref_no'=>$tag['tag_id'],'gross_wt' => $tag['gross_wt'],'net_wt'=> $tag['net_wt'],'piece'=>$tag['piece']),'ret_acc_stock_process_details'); 
                         
        		             $this->$model->updateData(array('non_tag_process'=>1),'bill_det_id',$tag['tag_id'],'ret_bill_details');
