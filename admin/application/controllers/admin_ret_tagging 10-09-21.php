	...
	public function tagging($type="",$id=""){
		...
		switch($type)
		{
			...
			case "save": 
						...
						$ref_no = mt_rand(10,100).'-'.$this->session->userdata('uid');
						...
						if($addData['id_branch'] != $addData['to_branch'] && $addData['to_branch'] != ""){
	 					...
	 					{ 
							...
							$arrayTag = array(
								...
								'design_for' 		=> $addData['design_for'][$key], 
								...
								'size' 				=> ($addData['size'][$key]!='' ? $addData['size'][$key]:NULL), 
						...
						//echo "<pre>"; print_r($arrayTag);exit;
	 					...
	 					/*	//Update order Status
	 						...
	 						$this->$model->updateData(array('tag_code'=>$addData['product_short_code'][$key].'-'.$tag_no),'tag_id',$insId,'ret_taging');
	 						//Tag Code Generation
		 					...
		 					if($addData['stone_details'][$key])
							{
								...
								foreach($stone_details as $stone)
								{
									$stone_data=array(
													...
													'is_certification_req'  =>($stone['is_certification']!='' ? $stone['is_certification']:0),
													'certification_cost'    =>($stone['price']!='' ? $stone['price']: NULL),
													'image'                 =>($stone['image']!='' ?$stone['image']:NULL)
													);
						...
						foreach($tagging[0] as $tag)
			 		    {
			 		        if($tag!='')
			 		    	{
                                ...
                                QRcode::png($data['tag_id'].'/'.$print_taken,$file_name);  //Passing QR data
                                    $src['img'][]=array(
                                    'code'              =>$code,
