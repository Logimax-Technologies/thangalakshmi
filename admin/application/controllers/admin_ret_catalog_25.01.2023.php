	...
	public function karigar($type="",$id="",$status="")
    {
		...
		{	
			...
			case 'edit':
				...
				$data['karigar_wastages'] = $this->$model->get_karigar_wastages($id);
				...
				foreach($data['karigar_wastages'] as $key => $wastages)
				{
					$des_args = array(
						"id_product" => $wastages['id_product']
					);
					$subdes_args = array(
						"id_product" => $wastages['id_product'],
						"design_no"  => $wastages['id_design']
					);
					$data['karigar_wastages'][$key]['design'] 		= $this->$model->get_active_design_products($des_args);
					$data['karigar_wastages'][$key]['sub_design'] 	= $this->$model->get_ActiveSubDesigns($subdes_args);
				}
			...
			break;
	 		case "save":
					...
					foreach($addData['wastage']['product'] as $wkey => $wval) {

						$wast = array(
							'id_karikar'	=>	$id_karigar,
							'id_product' 	=>	$addData['wastage']['product'][$wkey],
							'id_design' 	=>	$addData['wastage']['design'][$wkey],
							'id_sub_design' =>	$addData['wastage']['sub_design'][$wkey],
							'wastage_per' 	=>	$addData['wastage']['value'][$wkey],
							'mc_type'       =>  $addData['wastage']['mc_type'][$wkey],
							'mc_value'      =>  $addData['wastage']['mc_value'][$wkey],
							'pur_touch'     =>  $addData['wastage']['pur_touch'][$wkey],
							'active'        =>  0,
							'status'        =>  0,
							'created_on'	=>  date("Y-m-d H:i:s"),
							'created_by'    =>  $this->session->userdata('uid')						
						...
						$this->$model->insertData($wast,'ret_karikar_items_wastage');
					}

					foreach($addData['stone']['stn_type'] as $wkey => $wval)
					{
						$stn = array(

							'id_karigar'         =>   $id_karigar,
							'stone_type'         =>   $addData['stone']['stn_type'][$wkey],
							'stone_id'           =>   $addData['stone']['stn_name'][$wkey],
							'stone_cal_type'     =>   $addData['stone']['stn_calc_type'][$wkey],
							'uom_id'             =>   $addData['stone']['uom_id'][$wkey],
							'rate_per_gram'      =>   $addData['stone']['stn_rate'][$wkey],
							'active'        =>  0,
							'status'        =>  0,
							'created_on'	=>  date("Y-m-d H:i:s"),
							'created_by'    =>  $this->session->userdata('uid')
					...
					foreach($addData['charges']['charge_name'] as $key => $val)
					{
						$ch = array(

							'id_karigar'         =>   $id_karigar,
							'charge_id'          =>   $addData['charges']['charge_name'][$key],
							'charge_value'       =>   $addData['charges']['charge_value'][$key],
							'active'             =>  0,
							'status'             =>  0,
							'created_on'	     =>  date("Y-m-d H:i:s"),
							'created_by'         =>  $this->session->userdata('uid')
						...
						$this->$model->insertData($ch,'ret_karigar_charges');
				...
				//echo"<pre>"; print_r($addData);exit;
				 $data=array(
					...
					'createdby'            => $this->session->userdata('uid')
						...
						$this->$model->deleteData('id_karigar',$id,'ret_karigar_stones');
						...
						$this->$model->deleteData('id_karigar',$id,'ret_karigar_charges');
...
/*	function old_metal_rate()
  ...
  public function reorder_settings($type="",$id="",$status=""){
	...
	function set_ret_prod_image($id)
   {
	...
	{
	    $model=self::CAT_MODEL;
        $data=$this->$model->get_karigar_wise_wastage();
        echo json_encode($data);
	}
	//Karigar V.A Details
		...
		switch($type) 
		{
			...
			case 'save':
				...
				foreach($_POST['approved_data'] as $key => $val)
				{
					...
					$this->$model->update_karigar_wastages($val['id_karigar'],$val['pro_id'],$val['des_id'],$val['sub_des_id']);
