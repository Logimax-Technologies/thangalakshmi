...
class Admin_ret_reports extends CI_Controller {
   ...
   //Green Tag Report
    ...
    function popayments($type="")
    {
		...
		switch($type)
		{
			...
			case 'ajax': 
					...
					$list=$this->$model->get_popaymentreport($_POST); 
    ...
    function file_upload_old_tags()
    {
			...
			foreach ($imp_data as $key => $value)
			{
				$key_name_tran_no = array_search('TRANNO', $imp_data[$key]);
				if($key_name_tran_no != '')
				{
					break;
				}
			...
			$unset_key_name_tran = array_search('TRANNO', $old_tran_no);
   ...
   function dashboard_btList()
   {
    ...
    function netbanking_collection_report($type="")
    {
        ...
        switch($type)
        {
            ...
            break;
            case 'ajax':
                $list=$this->$model->getNetbankingCollectionReport($_POST['from_date'],$_POST['to_date'],$_POST['id_branch'],$_POST['nb_type']);
                $data=array(
                    'list'=>$list
                );
                echo json_encode($data);
