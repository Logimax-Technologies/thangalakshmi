...
//							'tax_group_id'			=> (isset($addData['tax_group_id']) ? $addData['tax_group_id'] :NULL ),
	...
	{   
		...
		{
			case 'list': 
					$data['main_content'] = "tagging/retagging" ;
        			$this->load->view('layout/template', $data);
				break;
			case 'partly_sale': 
		...
		foreach($reqdata as $tag)
		{
		    if($tag_process==1)
		    {
        		   ...
        		   // print_r($insId);exit;
                    ...
                    {
                        $this->db->trans_commit();
                        $responseData=array('status'=>TRUE,'message'=>'Re-Tagging Created successfully.','tag_ids'=>'');
                    }	
                    else
                    {
                        $this->db->trans_rollback();
		    ...
		    else if($tag_process==3)
		    {
		        $this->db->trans_begin();
		        $this->$model->updateData(array('tag_process'=>$tag_process,'tag_process_date'=>date("Y-m-d H:i:s"),'tag_process_by'=>$this->session->userdata('uid')),'tag_id',$tag['tag_id'],'ret_taging');
		        
		        if($report_type==1) // SALES TRUEN
		        {
                    $salesReturnLog=array(
                    'tag_id'           =>$tag['tag_id'],
                    'date'			   =>$tag_datetime,
                ...
                {
                    $this->db->trans_commit();
                    $responseData=array('status'=>TRUE,'message'=>'Tag Other Issued successfully.','tag_ids'=>'');
                }	
                else
                {
                    $this->db->trans_rollback();
		    ...
		    else if($tag_process==4 && $report_type==3)
		    {
		        $this->db->trans_begin();
		        $this->$model->updateData(array('tag_process'=>$tag_process,'tag_process_date'=>date("Y-m-d H:i:s"),'tag_process_by'=>$this->session->userdata('uid')),'tag_id',$tag['tag_id'],'ret_taging');
		        
		        if($report_type==3) // PARTLY SALE
		        {
		             $partly_sale_log=array(
                    'tag_id'           =>$tag['tag_id'],
                    'gross_wt'         =>$tag['gross_wt'],
                ...
                {
                    $this->db->trans_commit();
                    $responseData=array('status'=>TRUE,'message'=>'Non Tag Updated successfully.','tag_ids'=>'');
                }	
                else
                {
                    $this->db->trans_rollback();
		    ...
		    else if($tag_process==5 && $report_type==4)
		    {
		        $this->db->trans_begin();
                ...
                $isExist = $this->$model->checkPurchaseItemsStockExist($id_product,$id_branch,$tag['purity']);
                    ...
                    if($isExist== TRUE)
                    {
                        ...
                        $this->$model->updateOldMetalStockData($oldMetaldata,'+');
                    }
                    else
                    {
                        $oldMetaldata=array(
                        ...
                        $this->$model->insertData($oldMetaldata,'ret_purchase_item_stock_summary'); 
                    }
                ...
                {
                    $this->db->trans_commit();
                    $responseData=array('status'=>TRUE,'message'=>'Updated To Accounts Stock.',''=>'');
                }	
                else
                {
                    $this->db->trans_rollback();
                    $responseData=array('status'=>false,'message'=>'Unable to Proceed');
                }
		    }
		    else if($tag_process==4 && $report_type==4)
		    {
		        $this->db->trans_begin();
		        
		        $this->$model->updateData(array('is_pocketed'=>3),'old_metal_sale_id',$tag['old_metal_sale_id'],'ret_bill_old_metal_sale_details');
