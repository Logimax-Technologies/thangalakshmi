			...
			case 'save':
				...
				foreach($_POST['approved_data'] as $key => $val)
				{
					...
					if($status==1)
					{
						...
						{
							$active_item_list = $this->$model->get_active_karigar_wastages($val['id_karigar'],$val['pro_id'],$val['des_id'],$val['sub_des_id']);
							foreach($active_item_list as $items)
							{
							    $this->$model->updateData(array("active"=>0,"status"=>0),'id_karikar_wast',$items['id_karikar_wast'],'ret_karigar_charges');
							}
					...
					if($approval_for==0) // Karigar Wastages Update
					{
						$status = $this->$model->updateData($statusData,'id_karikar_wast',$val['id_karikar_wast'],'ret_karikar_items_wastage');
						if($status)
						{
						    $this->$model->updateData(array("active"=>1,"status"=>1),'id_karikar_wast',$items['id_karikar_wast'],'ret_karigar_charges');
						}
					}
					else if($approval_for==1) // Karigar stones Update
					{
