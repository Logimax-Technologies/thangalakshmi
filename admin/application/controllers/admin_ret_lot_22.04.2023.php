	...
	function lot_split($type="")
	{
		...
		{		
			...
			case 'save':
				...
				$addData = $_POST['inward'];
				$data = array(
					'lot_received_at'	=> $addData['lot_received_at'],
					'id_employee'		=> $addData['id_employee'],
					'created_on'	    => date("Y-m-d H:i:s"),
					'created_by'        => $this->session->userdata('uid')
				); 
				$this->db->trans_begin();
				$insId = $this->$model->insertData($data,'ret_lot_split');
				//print_r($this->db->last_query());exit;
				if($insId > 0)
				{
					if(sizeof($_POST['split_item'])>0)
					{
						foreach($_POST['split_item'] as $itemData)
						{
							...
							$item_details = array(
								'id_lot_split'	            => $insId,
								'id_lot_inward_detail'      => ($itemData['id_lot_inward_detail']!='' ? $itemData['id_lot_inward_detail']:NULL),
								'id_category'				=> ($itemData['cat_id']!='' ? $itemData['cat_id']:NULL),
								'id_purity'				    => ($itemData['id_purity']!='' ? $itemData['id_purity']:NULL),
								'id_product'				=> ($itemData['pro_id']!='' ? $itemData['pro_id']:NULL),
								'split_pcs'				    => ($itemData['split_pcs']!='' ? $itemData['split_pcs']:0),
								'split_grs_wt'				=> ($itemData['split_wt']!='' ? $itemData['split_wt']:0),
								'created_on'	            => date("Y-m-d H:i:s"),
					            'created_by'                => $this->session->userdata('uid')
												   
							);
							$detail_insId = $this->$model->insertData($item_details,'ret_lot_split_details');
							if($detail_insId)
							{
								if(sizeof($itemData['stone_details'])>0)
								{							
									$stone_details = json_decode($itemData['stone_details'],true);								
									foreach($stone_details as $stn)
									{
										$stone_data = array(										
											"id_lot_split_details"  =>  $detail_insId,
											"id_stn_detail"         =>  $stn['id_stn_detail'],	
											"stone_id"              =>  $stn['stone_id'],
											"uom_id"                =>  $stn['uom_id'],
											"stone_pcs"             =>  $stn['split_stone_pcs'],
											"stone_wt"              =>  $stn['split_stone_wt'],
										);
										//echo "<pre>";print_r($stone_details);exit;
										$stoneInsert = $this->$model->insertData($stone_data,'ret_lot_split_stone_details');
									}
								}
							}
