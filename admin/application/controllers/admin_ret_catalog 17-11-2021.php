			        	...
			        	foreach($reqdata as $data)
        				{
							...
							else if($data['wastageType']==2)
							{
								foreach($data['wastag_value'] as $wkey => $wval)
								{
									...
									$update_status=$this->$model->insertData($wcweightranges,'ret_design_weight_range_wc');
								}
							}
							//echo "<pre>";print_r($upd_data);exit;
							$upd_data['wastage_type']   = $data['wastageType'];
        					$update_status=$this->$model->updateData($upd_data, 'id_sub_design_mapping', $data['id_sub_design_mapping'], 'ret_sub_design_mapping');
        				}
        				if($update_status)
        				{		
        					$this->db->trans_commit();
  ...
  public function reorder_settings($type="",$id="",$status=""){
	...
	function set_ret_prod_image($id)
   {
	...
	function update_product_mapping()
	{
	    ...
	    {
	        if($this->$model->check_design_products_maping($id_product,$id_design))
    	    {
                $insdata=array(
                'id_design'         =>$id_design,
                'pro_id'            =>$id_product,
			...
			case 'edit':
	 		...
	 		case "save":
					...
					$data=array(
						...
						'attr_group_type'      => $addData['attr_group_type'],
