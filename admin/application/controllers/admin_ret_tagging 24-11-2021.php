...
echo "<pre>";print_r($lot_no);exit;*/
					...
					if($this->db->trans_status()===TRUE)
					{
						...
						$responseData=array('status'=>true,'message'=>'Tagging added successfully','trans_code'=>$trans_code,'ref_no'=>$ref_no,'to_branch'=>$toBranch,'qrcodeUrl'=>$QrcodeUrl,'tag_id'=>$insId);
...
//							'tax_group_id'			=> (isset($addData['tax_group_id']) ? $addData['tax_group_id'] :NULL ),
			...
			case 'tag_update':
			            ...
			            if($this->db->trans_status()===TRUE)
    					{
    						...
    						$responseData=array('status'=>true,'message'=>'Tagging Updated successfully','trans_code'=>'','ref_no'=>$ref_no,'to_branch'=>'','qrcodeUrl'=>$QrcodeUrl,'tag_id'=>$tag_id);
