					...
					if($status==1)
					{
						if($approval_for==0) // Karigar Wastages Update
						{
							...
							foreach($wastage_list as $itm)
							{
								...
								//Same category 
								if($val['cat_id'] == $itm['id_category'])
								{
									//All product in list
									if($itm['id_product']==0)
									{
										$this->$model->update_karigar_wastages($val['id_karigar'],$val['cat_id'],$itm['id_product']);
									}

									// If post product is All  
									if($val['pro_id']==0)
									{
										$this->$model->update_karigar_wastages($val['id_karigar'],$val['cat_id']);
									}
								}

								//Same category,Same product
								if(($val['cat_id'] == $itm['id_category']) && ($val['pro_id'] == $itm['id_product']))
								{
									//All Design in List
									if($itm['id_design']==0)
									{
										$this->$model->update_karigar_wastages($val['id_karigar'],$val['cat_id'],$val['pro_id'],$itm['id_design']);
									}

									//if post Design is All
									if($val['des_id']==0)
									{
										$this->$model->update_karigar_wastages($val['id_karigar'],$val['cat_id'],$val['pro_id']);

									}
								}

								// Same category , Same product , Same Design
								if(($val['cat_id'] == $itm['id_category']) && ($val['pro_id'] == $itm['id_product']) && ($val['des_id'] == $itm['id_design']))
								{
									// All sub design in list
									if($itm['id_sub_design']==0)
									{
										$this->$model->update_karigar_wastages($val['id_karigar'],$val['cat_id'],$val['pro_id'],$val['des_id'],$itm['id_sub_design']);
									}

									// If post subdesign is All
									if($val['sub_des_id']==0)
									{
										$this->$model->update_karigar_wastages($val['id_karigar'],$val['cat_id'],$val['pro_id'],$val['des_id']);
									}
								}

								// Same category , Same product , Same Design , Same Sub Design
