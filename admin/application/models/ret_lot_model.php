...
/*	function ajax_getLotList($received_at,$from_date,$to_date)
	...
	function ajax_getLotList($from_date,$to_date)
    {
		...
		if(i.lot_from=2,'Supplier Entry',if(i.lot_from=3,'From Import',if(i.lot_from=4,'From Tag Process',if(i.lot_from=5,'From Old Metal Process',if(i.lot_from=6,'From Retaggnig',if(i.lot_from=7,'Lot Merge','Manual Lot')))))) as lotFrom,i.lot_from
		FROM ret_lot_inwards i
	...
	function getLotidsforMerge($data)
	{
		...
		WHERE l.is_lot_split = 0
		and lt_tag.tag_lot_id is null and lt_m.id_lot_inward_detail is null 
		and lm_lot.lot_no is null
		and l.stock_type = ".$data['stock_type']."
		GROUP BY lt.lot_no
