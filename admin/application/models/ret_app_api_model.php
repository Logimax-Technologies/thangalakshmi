...
function getAvailableCustomers($SearchTxt){
	...
	function getUserMenus($id_profile){ 
        ...
        $menus = array ( 
        			...
        			array("id" => 6, "parent_id" => 0, "title" => "Category", "icon" => "basket", "component" => "CategoryPage", "show"  => "1", "show_in_dash"  => "0"), //Disabled for ams
        			...
        			array("id" => 9, "parent_id" => 0, "title" => "Cart", "icon" => "cart", "component" => "CartPage", "show"  => 1, "show_in_dash"  => "1"), //Disabled for ams,
					...
					array("id" => 16, "parent_id" => 0, "title" => "Enquiry", "icon" => "ios-create", "component" => "FaqPage", "show"  => 1, "show_in_dash"  => "1"),
	...
	{ 
		...
		left join metal m on m.id_metal=c.id_metal".($type == 'active' ? ' where c.status = 1'.($last_id > 0? ' and c.id_ret_category >'.$last_id : '' ) : ($last_id > 0? ' where c.id_ret_category >'.$last_id : '' ) )." ".($last_id >= 0 ? 'ORDER BY c.id_ret_category ASC LIMIT 10' : '' ));  
	...
	{ 
		...
		$sql  = "SELECT p.pro_id,p.cat_id,c.name as category,p.product_name, p.product_name as label,p.product_short_code,p.product_status,p.image,
		        ...
		        c.tgrp_id as tax_group_id,tax.tax_percentage,tax.tgi_calculation,p.calculation_based_on 
    ...
    {	
        ...
        if($searchField == 'old_tag_id') {
			...
			$data = $this->db->query("SELECT tag.tag_id,
						...
						design_id, cost_center, tag.purity, IFNULL(tag.size,'') as size, uom, piece, IFNULL(tag.less_wt,0) as less_wt, tag.net_wt, tag.gross_wt,
						...
						c.tgrp_id as tax_group_id,IFNULL(tag.id_orderdetails,'') as id_orderdetails,
...

    {
        ...
        $products = $this->db->query("SELECT 
	        ...
	        mt.id_metal,c.tgrp_id as tax_group_id
    ...
    { 
        ...
        tag.tag_code, tag_datetime,c.tgrp_id as tax_group_id,
    ...
    /*function generateEstiNo($date)

	{

	  $sql = "SELECT max(esti_no) as lasEstiNo FROM ret_estimation WHERE date(estimation_datetime)='".$date."' ORDER BY estimation_id DESC "; 
    ...
    function applyFilter($data,$type)
    {
		...
		if($searchType == 1) {
			...
			$this->db->where('t.tag_code', $tagStr);
            ...
            if ( $query->num_rows() > 0 )
            {
				...
				foreach($result as $row) {
					...
					foreach($tagStone as $stn) {
						...
						$TagStoneDetails = array(
													"TagStoneId"   	=>  $stn['tag_stone_id'],
													"StoneName"   	=>  $stn['stone_name'],
													"Pieces"        =>  $stn['pieces'],
													"Weight"        =>  $stn['wt'],
					...
					$total = $this->get_totalprice($row, $rates, $rate_fields, $tax_details);
	...
	function get_totalprice($tag, $rates, $rate_fields, $tax_details) {
		...
		if($calculation_based_on == 0) { 
			...
			$mc_type 	  = $tag_mc_type == 1 ? ($tag_mc_value * $gross_wt) : ($tag_mc_value * $Pieces);

			// Metal Rate + Stone + OM + Wastage + MC

			$rate_with_mc = ($rate * ($wast_wgt + $net_wt)) + $mc_type + $stone_price + $certification_cost;
			...
			$mc_type 	  = $tag_mc_type == 1 ? ($tag_mc_value * $net_wt) : ($tag_mc_value * $Pieces);
	...
	function applySuppFilter($data,$type)
    {
		$searchType = 0;
		$tagStr = "";
		if(isset($data['TagNo'])) {
        ...
        if($query) 
        {
            ...
            if ( $query->num_rows() > 0 )
            {
				...
				foreach($result as $row) {
					...
					$file = self::SUPPIMG_PATH.$row['id_supp_catalogue']."-".$row['TagImage'];
					
					$supplier_cat_img = (file_exists($file)? base_url().$file : null );  
					//$row['TagImage'] = "";

					$row['img_details'][] = array(

						'id_desmap_img'  => $id_supp_catalogue,

						'id_tag_img'     => $id_supp_catalogue,

						'tag_id'   	  	 => $id_supp_catalogue,

						'image_name'     => ($row['TagImage'] != '' ? $supplier_cat_img  : base_url().'assets/img/no_image.png'),
					...
					$total_price = $this->get_totalprice($row, $rates, $rate_fields, $tax_details);*/
					...
					//if($from_price_validation && $to_price_validation && $stone_validation) {
						...
						//if($records >= $records_start) {
							...
							//$row['purities'] = $this->get_purities($purities, $cat_id);
	...
	function applyFilter_new($data,$type)
    {
		$date_time = date('Y-m-d H:i:s');
		$searchType = 0;
		$tagStr = "";
		...
		t.tag_id,
			...
			r.rate_field,
			r.market_rate_field,
			calc_totalamt(p.metal_type, st_tot.stone_amt, 1000, t.calculation_based_on, t.gross_wt, t.net_wt, t.retail_max_wastage_percent, t.tag_mc_value, t.tag_mc_type, t.sales_value, t.piece, m.tgrp_id, r.rate_field, '".$ratesjson."', '".$tax_details_json."') AS total_price
			"
		);
