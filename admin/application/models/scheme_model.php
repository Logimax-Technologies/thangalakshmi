...
class Scheme_model extends CI_Model
{
	...
	{ 
	    ...
	    if($this->session->userdata('uid')==1){
	       $sql ="select s.id_scheme,s.scheme_name as name,s.is_pan_required,s.pan_req_amt,s.is_nominee_required,s.set_as_min_from,s.set_as_max_from,s.disable_sch_payment,s.get_amt_in_schjoin,s.firstPayamt_as_payamt,s.firstPayamt_maxpayable,s.active,s.total_installments,s.maturity_type,s.has_gift,s.is_nominee_required
		 From scheme s
		  
		 where s.active = 1  AND (visible = 1 OR visible = 2) ";
	    }
	    else{
	        $sql ="select s.id_scheme,s.scheme_name as name,s.is_pan_required,s.pan_req_amt,s.is_nominee_required,s.disable_sch_payment,s.get_amt_in_schjoin,s.firstPayamt_as_payamt,s.firstPayamt_maxpayable,s.active,s.total_installments,s.maturity_type,s.has_gift,s.is_nominee_required
		 From scheme s
...
public function get_schemes_type($id)
	 ...
	 public function get_scheme($id)
	{
	   ...
	   $sql="SELECT s.amt_based_on,s.is_partial_payment,s.daily_pay_limit,s.restrict_payment,s.chit_detail_days,s.total_days_to_pay,s.is_digi,s.apply_interest_by,
	   s.disable_pay,s.disable_pay_amt,s.logo,sb.id_branch,s.gst_type,s.gst,s.hsn_code, s.id_scheme, s.scheme_name, 	s.code,s.wgt_convert,sync_scheme_code,s.ref_benifitadd_ins_type, s.ref_benifitadd_ins,
		...
		s.total_tax,s.is_pan_required,s.description,pan_req_amt,s.auto_debit_plan_type, s.is_enquiry,
