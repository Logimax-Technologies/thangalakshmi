  ...
  function get_weight($id)
   ...
   function getDeposit_empty_record() {
	...
	function get_karigar_approvalWastage_list($data)
	{
	    ...
	    kw.wastage_type,kw.calc_type,kw.karigar_calc_type,
	...
	function get_WastApprvKarigars($approval_for)
	{
		...
		WHERE ".($approval_for ==0 ? "k.status_karigar = 1 and kw.status=0" : "k.status_karigar = 1 and ks.status=0")."
	...
	public function update_karigar_stones($id_karigar)
	{
	...
	function get_sectionBranchwise($data)
	{
		...
		".($data['id_branch']!='' ? " and sect_br.id_branch=".$data['id_branch']."" :'')." ");
...
//Master:Diamond Quality rate Ends
	...
	{  
		...
		$sql=$this->db->query("SELECT * FROM `ret_selling_settings` WHERE id_product=".$id_product." AND id_design=".$id_deign." ".($id_sub_design!='' && $id_sub_design>0 ? " and id_sub_design=".$id_sub_design."" :'')." ".( $id_branch != '' ? " and id_branch=".$id_branch."" :'')."".( $id_selling_settings != '' ? " and id_selling_settings !=".$id_selling_settings."" :'')."");
	...
	{
	    $responseData=array();
	    $sql=$this->db->query("SELECT s.id_selling_settings,p.product_name,des.design_name,IFNULL(subDes.sub_design_name,'') as sub_design_name,if(s.mc_type=1,' / Gram',if(s.mc_type=2,' / Piece',' % Price')) as mc_type,if(s.type=1,'Fixed','Weight Range') as type,IFNULL(s.mc_value,'-') as mc,IFNULL(s.mc_min,'-') as min_mc,IFNULL(s.wastage_perc ,'-') as va,IFNULL(s.wastag_min,'-') as min_va,b.name,s.type as wastage_type,s.margin_mrp
	...
	{
	    $sql=$this->db->query("SELECT pro.cat_id,c.id_ret_category,rss.id_product,rss.id_sub_design,rss.id_design,rss.type,rss.wastage_perc,
		rss.mc_value,rss.mc_type,rss.wastag_min,rss.id_branch,rss.mc_min,rss.margin_mrp
