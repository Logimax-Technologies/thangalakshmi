...
class Scheme_model extends CI_Model
{
	 ...
	 public function get_scheme($id)
	{
	   $sql="SELECT s.logo,sb.id_branch,s.gst_type,s.gst,s.hsn_code, s.id_scheme, s.scheme_name, 	s.code,s.wgt_convert,sync_scheme_code,s.ref_benifitadd_ins_type, s.ref_benifitadd_ins,
		...
		s.visible, s.interest_value, s.total_interest, s.tax,s.tax_by,s.tax_value, 
		...
		s.firstPayDisc_by,s.firstPayDisc_value,s.sch_limit_value,s.all_pay_disc,s.allpay_disc_by,s.allpay_disc_value,s.discount_installment,s.discount_type,s.discount,otp_price_fixing,otp_price_fix_type
		FROM (scheme s)
...
/*-- / Coded by ARVK --*/	
	...
	function getFreeInsBySchId($id){
		$sql = $this->db->query("select free_payInstallments,discount_installment,ref_benifitadd_ins from scheme where  id_scheme='$id'");
