...
class ret_dashboard_model extends CI_Model
{
	...
	function get_dashboard_billings($from_date,$to_date,$id_branch)
	{
		   $sql = $this->db->query("select sum(bill_det.net_wt) as gold_wt from ret_billing as bill
	...
	{
		   $sql = $this->db->query("select sum(bill_det.net_wt) as silver_wt from ret_billing as bill
           left JOIN ret_bill_details  as bill_det on(bill_det.bill_id=bill.bill_id)
           left join ret_product_master as pro on(pro.pro_id=bill_det.product_id)
           left join ret_category as cat on(cat.id_ret_category=pro.cat_id)
           left join metal as m on(m.id_metal=cat.id_metal)
	...
	{
		   $sql = $this->db->query("select sum(bill_det.item_cost) as mrp from ret_billing as bill
         left JOIN ret_bill_details  as bill_det on(bill_det.bill_id=bill.bill_id)
         left join ret_product_master as pro on(pro.pro_id=bill_det.product_id)
         left join ret_category as cat on(cat.id_ret_category=pro.cat_id)
         left join metal as m on(m.id_metal=cat.id_metal)
	...
	function get_dashboard_lot_tag_details($from_date, $to_date)
	{
		$sql = $this->db->query("SELECT
		(SELECT ifnull(SUM(d.no_of_piece),0) FROM ret_lot_inwards l left JOIN ret_lot_inwards_detail d on d.lot_no=l.lot_no WHERE date(l.lot_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') as lot_pcs,
		(SELECT ifnull(SUM(d.gross_wt),0) FROM ret_lot_inwards l left JOIN ret_lot_inwards_detail d on d.lot_no=l.lot_no WHERE date(l.lot_date) BETWEEN  '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') as lot_wt,
		(SELECT ifnull(SUM(tag.piece),0)  FROM ret_taging tag WHERE tag.tag_status!=2 AND date(tag.tag_datetime) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') as tagged_pcs,
		(SELECT ifnull(SUM(tag.gross_wt),0)  FROM ret_taging tag WHERE tag.tag_status!=2 AND date(tag.tag_datetime) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') as tagged_wt");
		return $sql->row_array();
	}
	...
	function get_dashboard_virturaltag_details($from_date, $to_date)
	{
		$sql = $this->db->query("SELECT 
										...
										FROM (SELECT ifnull(SUM(d.gross_wt),0) as gross_wt, 
											  ...
											  AND e.item_type =2 and b.bill_status =1 AND date(b.bill_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') as homesale,
											(SELECT ifnull(SUM(tag.gross_wt - d.gross_wt),0) as gross_wt, 
												...
												b.bill_status = 1 AND date(b.bill_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') as tagsplit");
		return $sql->row_array();
	}
   ...
   function customer_orders($from_date,$to_date,$id_branch)
    {
        ...
        IFNULL(sum(recd_piece.recd_pcs),0) as received_piece,IFNULL(sum(allocated.allo_pcs),0) as allocated_piece,
        IFNULL(sum(pending.pen_pcs),0) as pending_piece,IFNULL(sum(ready.rdy_pcs),0) as ready_piece,
        IFNULL(sum(delivery.dely_pcs),0) as delivery_piece
        ...
        where c.order_from is not null ".($id_branch!='' && $id_branch!=0 ? " and c.order_from=".$id_branch."" :'')."");
        // print_r($this->db->last_query());exit;
        return $sql->row_array();
    }
	...
	function StockMetalWise($id_branch)
	{
	 ...
	 function AvailableStockDetails($from_date,$to_date,$id_branch,$id_metal)
	{
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(s.date)='".$op_date."' ".($id_branch!='' && $id_branch>0 ? " and s.id_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as opening_pcs,
        
        (SELECT IFNULL(SUM(s.closing_gwt),0) FROM ret_stock_balance s
        left join ret_product_master as pro on pro.pro_id=s.id_product
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(s.date)='".$op_date."' ".($id_branch!='' && $id_branch>0 ? " and s.id_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as opening_gwt,

        (SELECT IFNULL(SUM(s.closing_nwt),0) FROM ret_stock_balance s
        left join ret_product_master as pro on pro.pro_id=s.id_product
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(s.date)='".$op_date."' ".($id_branch!='' && $id_branch>0 ? " and s.id_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as opening_nwt,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE b.bill_status=1 and m.id_metal=".$id_metal." and date(b.bill_date)='".$from_date."' ".($id_branch!='' && $id_branch>0  ? " and b.id_branch=".$id_branch."" :'').") as tot_sales_pcs,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE d.bill_det_id is NOT null and m.id_metal=".$id_metal." and b.bill_status=1 and date(b.bill_date)='".$from_date."' ".($id_branch!='' && $id_branch>0 ? " and b.id_branch=".$id_branch."" :'').") as tot_sales_gwt,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE b.bill_status=1 and m.id_metal=".$id_metal." and date(b.bill_date)='".$from_date."' ".($id_branch!='' && $id_branch>0 ? " and b.id_branch=".$id_branch."" :'').") as tot_sales_nwt,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(l.date)='".$from_date."' and l.status=0 ".($id_branch!='' && $id_branch>0 ? " and l.to_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal." ) as inward_pcs,
            
        (SELECT IFNULL(SUM(tag.net_wt),0) FROM ret_taging tag 
        left join ret_product_master as pro on pro.pro_id=tag.product_id
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(tag.tag_datetime)='".$from_date."' ".($id_branch!='' && $id_branch>0 ? " and tag.current_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal." ) as inward_nwt,
            
        (SELECT IFNULL(SUM(tag.gross_wt),0) FROM ret_taging tag 
        left join ret_product_master as pro on pro.pro_id=tag.product_id
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(tag.tag_datetime)='".$from_date."' ".($id_branch!='' && $id_branch>0 ? " and tag.current_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as inward_gwt ,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(l.date)='".$from_date."' and (l.status=2 or l.status=3 or l.status=5 or l.status=4) ".($id_branch!='' && $id_branch>0 ? " and l.from_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as br_out_pcs,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(l.date)='".$from_date."' and (l.status=2 or l.status=3 or l.status=5 or l.status=4) ".($id_branch!='' && $id_branch>0 ? " and l.from_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as br_out_gwt,
        ...
        left join ret_category as cat on cat.id_ret_category=pro.cat_id
        left join metal as m on m.id_metal=cat.id_metal
        WHERE date(l.date)='".$from_date."' and (l.status=2 or l.status=3 or l.status=5 or l.status=4) ".($id_branch!='' && $id_branch>0 ? " and l.from_branch=".$id_branch."" :'')." and m.id_metal=".$id_metal.") as br_out_nwt ");
        
		//print_r($this->db->last_query());exit;
		$data=$sql->row_array();
		$return_data['opening_pcs']=$data['opening_pcs'];
		$return_data['opening_gwt']=$data['opening_gwt'];
		$return_data['opening_nwt']=$data['opening_nwt'];
		$return_data['tot_sales_pcs']=$data['tot_sales_pcs'];
		$return_data['tot_sales_gwt']=$data['tot_sales_gwt'];
		$return_data['tot_sales_nwt']=$data['tot_sales_nwt'];
		$return_data['inward_pcs']=$data['inward_pcs'];
		$return_data['inward_gwt']=$data['inward_gwt'];
		$return_data['br_out_pcs']=$data['br_out_pcs'];
		$return_data['available_pcs']=$data['opening_pcs']+$data['inward_pcs']-$data['tot_sales_pcs']-$data['br_out_pcs'];
		$return_data['available_gwt']=$data['opening_gwt']+$data['inward_gwt']-$data['tot_sales_gwt']-$data['br_out_gwt'];
		$return_data['available_nwt']=$data['opening_nwt']+$data['inward_nwt']-$data['tot_sales_nwt']-$data['br_out_nwt'];
		return $return_data;
	}
