...
class Ret_tag_model extends CI_Model
{
    ...
    { 
		 $sql = ("SELECT tag.tag_id,tag_code,
					...
					current_counter, id_branch,tag.created_time
	...
	function get_entry_records($tag_id)
	{
		$sql = $this->db->query("SELECT tag_id, tag_code, counter,
				...
				ifnull(design_code,'') as design_code ,created_by,des.design_name,b.name
				FROM ret_taging as tag 
	...
	function getAvailableLots()
	{
		...
		$lot_data=$this->db->query("SELECT lot_in.lot_no,date_format(lot_in.lot_date,'%Y-%m-%d')as lot_date,lot_in.id_category,lot_in.id_purity,c.name as category_name,p.purity as purity_name,
			...
			(lot.total_no_of_piece-ifnull(tag.tag_piece,0)) as lot_bal_pcs,m.metal
			...
			LEFT JOIN (
						...
						left join ret_taging t on t.tag_lot_id=d.lot_no
	...
	function get_lot_products($lot_no)
	{
		$data=array();
		$sql=$this->db->query("SELECT d.lot_product,d.lot_no,
							 p.product_name,p.hsn_code,p.product_short_code
							 from ret_lot_inwards_detail d
							 LEFT JOIN ret_product_master p on p.pro_id=d.lot_product
							 where d.lot_no=".$lot_no." group by d.lot_no");
		//echo $this->db->last_query();exit;
		$data=$sql->result_array();
		return $data;
	}

	function get_lot_designs($lot_no,$lot_product)
	{
		$data=array();
		$sql=$this->db->query("SELECT d.lot_id_design,d.lot_no,
							 des.design_name,des.design_no,des.design_code
							 from ret_lot_inwards_detail d
							 LEFT JOIN ret_design_master des on des.design_no=d.lot_id_design
							 where d.lot_no=".$lot_no." and d.lot_product=".$lot_product." group by d.lot_no");
		//echo $this->db->last_query();exit;
		$data=$sql->result_array();
		return $data;
	}
	...
	function get_lot_inward_details($lot_no,$lot_product,$lot_id_design)
	{
		$sql= $this->db->query("SELECT d.lot_no,d.id_lot_inward_detail,d.lot_product,d.lot_id_design,d.no_of_piece,
					des.design_name,p.product_name,p.hsn_code,p.product_short_code,
					d.gross_wt_uom,d.net_wt_uom,d.less_wt,d.less_wt_uom,
					d.wastage_percentage,d.mc_type,d.making_charge,d.precious_st_pcs,
					d.precious_st_wt,d.precious_st_uom,d.semi_precious_st_pcs,
				d.semi_precious_st_wt,d.normal_st_pcs,d.normal_st_wt,d.gross_wt,d.net_wt,
				(lot.total_gross_wt-ifnull(tag.tag_gross_wt,0)) as lot_bal_wt,tag.tag_gross_wt as tag_gross_wt,
			(lot.total_no_of_piece-ifnull(tag.tag_piece,0)) as lot_bal_pcs
					...
					LEFT JOIN (
						...
						sum(ifnull(t.piece,0)) as  tag_piece
						from ret_taging t
						where t.tag_lot_id=".$lot_no." and t.design_id=".$lot_id_design."
						group by t.tag_lot_id
					...
					LEFT JOIN (
						...
						sum(ifnull(d.no_of_piece,0)) as  total_no_of_piece
						from ret_lot_inwards_detail d
						where d.lot_no=".$lot_no." and d.lot_id_design=".$lot_id_design."
						group by d.lot_no
					) lot on lot.lot_no=d.lot_no
					where d.lot_no=".$lot_no." and d.lot_id_design=".$lot_id_design."
					".($lot_product!='' ? " and d.lot_product=".$lot_product."" : '')."");
			//echo $this->db->last_query();exit;
			$data['lot_inward_detail']=$sql->result_array();
