...
class Scheme_model extends CI_Model
{
	...
	{
		$this->db->select('id_scheme,scheme_name,code,scheme_type,amount,max_weight,payment_chances,flx_denomintion,min_amount,max_amount,total_installments,stop_payment_installment,interest,description,active,date_add');
		$data=$this->db->get(self::SCH_TABLE);
		return $data->result_array();
	}  	
  /*	public function get_schemes()
	{
		$this->db->select('id_scheme,scheme_name as name,is_pan_required,pan_req_amt,disable_sch_payment,get_amt_in_schjoin,firstPayamt_as_payamt,firstPayamt_maxpayable,active,total_installments,maturity_type,has_gift');
	...
	// based on selected branch && branchwise_scheme set  to schemes showed in add page -admin //HH
	public function get_schemes($id) 
	{ 
	    $data=$this->get_chitsettings();
	    
	    $sql ="select s.id_scheme,s.scheme_name as name,s.is_pan_required,s.pan_req_amt,s.disable_sch_payment,s.get_amt_in_schjoin,s.firstPayamt_as_payamt,s.firstPayamt_maxpayable,s.active,s.total_installments,s.maturity_type,s.has_gift
		 From scheme s 
		 left join scheme_branch sb on s.id_scheme=sb.id_scheme
		   where s.active=1 ".($id!='undefined' && $data['branchwise_scheme']==1 ? "and sb.id_branch=".$id." " :'')."";
	 ...
	 public function get_scheme($id)
	{
	   $sql="SELECT s.logo,sb.id_branch,s.gst_type,s.gst,s.hsn_code, s.id_scheme, s.scheme_name, 	s.code,s.wgt_convert,sync_scheme_code,s.ref_benifitadd_ins_type, s.ref_benifitadd_ins,
		...
		s.total_tax,s.is_pan_required,s.description,pan_req_amt,s.auto_debit_plan_type,
		...
		s.firstPayDisc_by,s.firstPayDisc_value,s.sch_limit_value,s.all_pay_disc,s.allpay_disc_by,s.allpay_disc_value,s.discount_installment,s.discount_type,s.discount,otp_price_fixing,otp_price_fix_type,one_time_premium,flexible_sch_type,maturity_days,closing_maturity_days,flx_denomintion,flexible_sch_type,noti_msg
		FROM (scheme s)
...
return $id_branch;
	...
	{
	    $sql="SELECT
				    s.id_scheme, s.scheme_name, s.code, m.metal, s.id_metal, s.scheme_type, s.min_amount,s.max_amount,
				    ...
				    s.allow_preclose,s.preclose_months,s.preclose_benefits,s.flx_denomintion,
				    ...
				    s.total_tax,s.is_pan_required,s.description,pan_req_amt
				FROM (scheme s)
    ...
    {
        //$chitSettings = $this->getChitSettings();
		$data=array(			
				'id_scheme'				=> 0,
				'scheme_name'			=> NULL,
				'code'					=> NULL,
			...
			//  'sch_limit'				=> $chitSettings['sch_limit'],
...
/*-- / Coded by ARVK --*/	
	...
	function get_fixweight_schemes()/* -- edited by ARVK -- */
	{
		$sql="SELECT
				    ...
				    s.total_tax,s.is_pan_required,s.description ,count(sa.id_scheme_account) as accounts,pan_req_amt,
	...
	function getFreeInsBySchId($id){
		$sql = $this->db->query("select free_payInstallments,discount_installment,stop_payment_installment,ref_benifitadd_ins from scheme where  id_scheme='$id'");
...
/*public function scheme_branch($data)
	...
	// preclose interest deduction base on scheme chart //HH
    ...
    // preclose interest deduction base on scheme chart //
