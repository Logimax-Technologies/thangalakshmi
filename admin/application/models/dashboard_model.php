	 ...
	 $id_branch=$this->input->post('id_branch');
	   /*$sql="Select count(id_scheme_account) as total,b.id_branch,b.name as  branch_name from ".self::ACC_TABLE." s 
	        ...
	        from ".self::ACC_TABLE." s 
	   		...
	   		where s.active=1 and s.is_closed=0 and s.scheme_acc_number IS NOT NULL AND p.payment_status = 1 AND (date(s.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
    ...
    function account_join_list($filterBy,$from_date,$to_date)
	{
	  ...
	  $result=0;	
	   $sql="Select COUNT(distinct sa.id_scheme_account) as joined_thro,IFNULL(sum(p.payment_amount),0) as amount,(Select cs.currency_symbol from chit_settings cs) as currency_symbol
		...
		switch($filterBy){
			case 'MOB': //Joined thro Mobile App
			         $sql=$sql." where sa.is_closed=0 and sa.added_by = 2 and (date(sa.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
		            and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
		            ".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and sa .id_branch=".$id_branch:'')." ";   
				break;
			case 'WEB': //Joined thro Web App
			         $sql=$sql." where sa.is_closed=0 and sa.added_by = 0 and (date(sa.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
		            and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
		            ".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and sa .id_branch=".$id_branch:'')." ";  
				break;
			case 'ADMIN': //Joined thro Admin
			          $sql=$sql." where sa.is_closed=0 and sa.added_by = 1 and (date(sa.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
		            and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
		           ".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and sa .id_branch=".$id_branch:'')." ";  
				break;	
			case 'COLLECTION': //Joined thro Admin
			          $sql=$sql." where sa.is_closed=0 and sa.added_by = 3 and (date(sa.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
		            and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
		            ".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and sa .id_branch=".$id_branch:'')." ";  
				break;	
		}	
	...
	function get_account_joined($from_date,$to_date,$type)
	{
	  ...
	  $result=0;	
	   $sql="select  CONCAT(e.firstname,' ',e.lastname) as emp_name,IFNULL(e.emp_code,'-') as emp_code,
			...
			LEFT JOIN employee e ON (e.id_employee = s.id_employee)
			left join payment p on (p.id_Scheme_account=s.id_Scheme_account AND p.payment_status = 1)
			where s.added_by=".$type." and s.active=1 and s.is_closed=0 and (date(s.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
			and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
			".($id_employee!='' ?  " and s.id_employee=".$id_employee  :'')." group by s.id_scheme_account";
			//print_r($sql);exit;
			$r=$this->db->query($sql);
		return $r->result_array();	
		}	
 ...
 function customer_detail_bydate($from_date="",$to_date="",$id_branch="")
	...
	public function account_bydate()
	{
		...
		$sql= "Select s.id_scheme_account from scheme_account s 
			   ...
			   ".($uid!=1 ? ($branchWiseLogin==1? ($login_branch!='' ?  " and( s.id_branch=".$login_branch." or b.show_to_all=1 )":''):''):'')."
			   ".($login_branch=='' && $id_branch!='' && $id_branch!=0  ? " and s.id_branch=".$id_branch :'')."" ;
			    
		if($from_date!='' && $to_date!='')
		{
			$sql=$sql." and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
			and (date(s.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') "; 
		}
