...
/*	function ajax_getLotList($received_at,$from_date,$to_date)
	...
	function getLotNoForMerge($data)
	{
		...
		sum(lt.no_of_piece) as no_of_piece,sum(lt.gross_wt) as gross_wt,ifnull(sum(lt.precious_st_pcs),0) as dia_pcs,
		ifnull(sum(lt.precious_st_wt),0) as dia_wt,ifnull(sum(lt.normal_st_pcs),0) as stn_pcs,ifnull(sum(lt.normal_st_wt),0) as stn_wt,
		lt_tag.tag_id
		...
		LEFT JOIN (SELECT lm.lot_no FROM ret_lot_merge lm 
		           LEFT JOIN ret_lot_inwards lt on lt.lot_no = lm.lot_no) as lt_m on lt_m.lot_no = l.lot_no
		WHERE lt.lot_product is not null and lt_tag.tag_lot_id is null and lt_m.lot_no is null
