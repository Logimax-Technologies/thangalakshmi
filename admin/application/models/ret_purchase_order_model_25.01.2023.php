...
class Ret_purchase_order_model extends CI_Model
{
	...
	function getReturnedRequestList()
	{
		/*$sql = $this->db->query("SELECT po.po_id,  poitm.po_item_id, po.po_ref_no, poitm.no_of_pcs as purchasedpcs, 
            ...
            (IFNULL(cat_item.ret_pcs,0)) as ret_pcs,IFNULL(cat_item.ret_wt,0) as ret_wt,
	...
	function getPurchaseOrderItemDet($po_id)
	{
        ...
        p.po_order_no,p.id_purity,pur.purity as purity_name,ifnull(dia_stn.stn_wt,0) as dia_wt,pro.purchase_mode
        FROM ret_purchase_order_items p 
        LEFT JOIN ret_purchase_order ord ON ord.po_id=p.po_item_po_id
	...
	{
	    $sql=$this->db->query("SELECT po_stn.po_stone_id,po_stn.is_apply_in_lwt,po_stn.po_stone_pcs,po_stn.po_stone_wt,

        po_stn.po_stone_uom,po_stn.po_stone_calc_based_on,po_stn.po_stone_rate,po_stn.po_stone_amount,s.stone_type
	...
	{
	    $sql = $this->db->query("SELECT p.po_ref_no,p.po_id,p.po_karigar_id,(oi.item_pure_wt) as tot_purchase_wt,
	                                IFNULL(ratefix.fixed_wt,0) as total_fixed_wt,
                            	    date_format(p.po_date,'%d-%m-%Y') as podate
                                    FROM ret_purchase_order p 
                                    LEFT JOIN ret_purchase_order_items as oi ON oi.po_item_po_id = p.po_id 
   ...
   function get_po_paymentDetails($pay_id)
    ...
    
    {
        ...
        pur_itms.pur_ret_gwt as grs_wt,tag.tag_code,tag.product_id,p.product_name
        ...
        pur_itm.pur_ret_pcs as pcs,pur_itm.pur_ret_gwt as grs_wt,pro.product_name
        ...
        pr_itms.pur_ret_pcs as pcs,pr_itms.pur_ret_gwt as grs_wt,p.product_name
