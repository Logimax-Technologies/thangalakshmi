...
class Ret_estimation_model extends CI_Model
{
	...
	function get_customer($id_customer)
    {
		...
		IFNULL(c.pan,'') as pan_no,IFNULL(c.aadharid,'') as aadharid,IFNULL(c.gst_number,'') as gst_number,IFNULL(c.id_village,'') as id_village,c.title,c.id_profession,c.gender,IFNULL(date_format(c.date_of_birth,'%d-%m-%Y'),'') as date_of_birth,IFNULL(date_format(c.date_of_wed,'%d-%m-%Y'),'') as date_of_wed
		FROM customer c 
	...
	function get_entry_records($est_id)
	{
		$sql = $this->db->query("SELECT estimation_id,esti_no,
				...
				IFNULL(est.silverrate_1gm,0) as silverrate_1gm,a.address1,a.address2,a.address3,c.name as city_name
				FROM ret_estimation as est 
				LEFT JOIN customer as cus ON cus.id_customer = est.cus_id 
	...
	function createNewCustomer($cusname,$cusmobile,$mail,$branch,$id_village,$cus_type,$id_country,$id_state,$id_city,$address1,$address2,$address3,$pincode,$gst_number,$title,$id_profession,$gender,$date_of_birth,$date_of_wed)
	{
	...
	function updateCustomer($id_customer,$cusname,$cusmobile,$mail,$branch,$cus_type,$id_country,$id_state,$id_city,$address1,$address2,$address3,$pincode,$gst_number,$title,$id_profession,$gender,$date_of_birth,$date_of_wed,$id_village)
	{
            if($date_of_birth!=''){
                $d1 = date_create($date_of_birth);
                $dateofbirth = date_format($d1,"Y-m-d");
    ...
    function getTaggingScanBySearch($SearchTxt,$searchField,$branch)
    {
        ...
        IFNULL(coll.id_tag_mapping,'') as id_tag_mapping,IFNULL(coll.id_mapping_details,'') as id_mapping_details,IFNULL(tagOtherMetal.tag_other_itm_amount,0) as tag_other_itm_amount,c.scheme_closure_benefit, CONCAT(s.value, ' ', s.name) as size_name,
        IFNULL(tag.tag_purchase_cost,0) as tag_purchase_cost
        FROM ret_taging as tag
        Left join ret_lot_inwards_detail lot_det ON tag.id_lot_inward_detail = lot_det.id_lot_inward_detail
        LEFT JOIN ret_lot_inwards as lot_inw ON lot_inw.lot_no = lot_det.lot_no
        LEFT JOIN ret_product_master as pro ON pro.pro_id = tag.product_id
   ...
   function getdesigndetails($cat_product,$cat_design,$cat_sub_design)
   {
    ...
    {
        $sql=$this->db->query("SELECT * FROM ret_product_master where stock_type=2");
        return $sql->result_array();
    }
	...
	*/
	function getOrderBySearch($SearchTxt,$branch,$fin_year_code)
    {
        ...
        IFNULL(tagOtherMetal.tag_other_itm_amount,0) as tag_other_itm_amount,cus.rate_per_gram as order_rate, CONCAT(s.value, ' ', s.name) as size_name
        FROM ret_taging tag
    ...
    {
		$data=$this->db->query("SELECT tag.tag_id as value,
        tag.tag_code as label, tag_datetime,prod.tgrp_id as tax_group_id,
   ...
   function get_non_tag_stock_details($data)
   {
        ...
        FROM  ret_nontag_item rn 
        where rn.product is not null 
        ".($data['id_product']!='' && $data['id_product'] > 0 ? "AND rn.product=".$data['id_product']."":"")."
        ".($data['id_branch']!='' && $data['id_branch'] > 0 ? "AND rn.branch=".$data['id_branch']."":"")."
        ".($data['id_design']!='' && $data['id_design'] > 0 ? "AND rn.design=".$data['id_design']."":"")."
        ".($data['id_sub_design']!='' && $data['id_sub_design'] > 0 ? "AND rn.id_sub_design=".$data['id_sub_design']."":"")."");
        return $sql->row_array();
   }
   ...
   function advance_details_order_details($orderno,$id_branch)
    ...
    function get_chit_details($est_id){
		$data = $this->db->query("SELECT rn.scheme_account_id,c.mobile, sa.id_scheme_account as value, sa.id_scheme_account as label, 
		...
		IFNULL(sa.additional_benefits,0) as additional_benefits, IFNULL(sa.benefit,0) as sch_benefit, IFNULL(pay.cash_pay, 0) as cash_pay,s.is_wast_and_mc_benefit_apply
                FROM ret_estimation_items as est 
                left join ret_est_chit_utilization rn on rn.est_id = est.esti_id
				...
				WHERE sa.id_scheme_account and sa.is_closed = 1 and sa.is_utilized = 0 and est.esti_id ='".$est_id."'
                GROUP by est.est_item_id");
		//echo $this->db->last_query();exit;
		return $data->result_array();
	}
    function get_est_tag_details($est_id)
    { 
		$items_query = $this->db->query("SELECT est_item_id, esti_id, item_type, item_emp_id,c.scheme_closure_benefit,
                       ...
                       sub.sub_design_name,mt.metal as metal_name,IFNULL(est.est_rate_per_grm,0) as est_rate_per_grm,c.id_metal as metal_type,c.is_916,c.name as cat_name,
                       IFNULL(concat(sz.value,'',sz.name),'') as size,sz.id_size,pro.tgrp_id as tax_group_id,IFNULL(tag.tag_purchase_cost,0) as tag_purchase_cost
					   FROM ret_estimation_items as est 
					   ...
					   LEFT JOIN (select i.tgi_taxcode,i.tgi_tgrpcode,
						...
						FROM ret_taxgroupitems i
						LEFT JOIN ret_taxmaster m on m.tax_id=i.tgi_taxcode) as tax on tax.tgi_tgrpcode=pro.tgrp_id
					    WHERE est.esti_id ='".$est_id."' and est.item_type = 0 and est.purchase_status = 0
					    GROUP by est.est_item_id");
		//print_r($this->db->last_query());exit;
		$item_details = $items_query->result_array();	
        $return_data=[];	
    ...
    { 
		$items_query = $this->db->query("SELECT est_item_id, esti_id, item_type, est.id_sub_design,c.scheme_closure_benefit,
					   est.product_id, est.tag_id, est.design_id, est.purity as purid, est.size, est.uom, est.piece, 
    ...
    function est_home_bill($est_id)
    {
        $items_query = $this->db->query("SELECT est_item_id, esti_id, item_type,est.id_sub_design,c.scheme_closure_benefit,
                       ...
                       LEFT JOIN (select i.tgi_taxcode,i.tgi_tgrpcode,
                        ...
                        FROM ret_taxgroupitems i
                        LEFT JOIN ret_taxmaster m on m.tax_id=i.tgi_taxcode) as tax on tax.tgi_tgrpcode=pro.tgrp_id
                        WHERE est.esti_id ='".$est_id."' and est.item_type = 2 and est.purchase_status = 0
                        GROUP by est.est_item_id");
        //print_r($this->db->last_query());exit;
        $item_details = $items_query->result_array();	
        $return_data=[];		
        foreach ($item_details as $items) {
            $return_data["item_details"][]=array(
                            ...
                            'material_details'	=>$this->get_other_material_details($items['est_item_id']),
                            'charge_value'		=> $items['charge_value'],
                            //'charges'			=> $this->create_new_empty_est_cus_charges_item($items['est_item_id']),
                            'charges'			=> [],
                            'is_916'			=>$items['is_916'],
                            'cat_name'		    =>$items['cat_name'],
                            'cat_id'		    =>$items['cat_id'],
    ...
    {
        $old_matel_query = $this->db->query("SELECT est_old.old_metal_sale_id, est_id,piece,
    						   id_category, type, item_type, gross_wt,net_wt,est_old.remark as remark,
    						   round((gross_wt - stone_wt),3) as ls_wt,
                               if(type = 1, 'Melting', 'Retag') as reusetype,
                               if(item_type = 1, 'Ornament', if(item_type = 2, 'Coin', if(item_type = 3, 'Bar',''))) as receiveditem,
    						   IFNULL(stone_wt,0) as stone_wt, IFNULL(dust_wt,0) as dust_wt , est_old.purity as purid, wastage_percent,
    ...
    {
        $sql=$this->db->query("SELECT tag_id,tag_code,tag_status,(if(tag_status =0 ,'Not yet sale',if(tag_status =1 ,'Sold out',if(tag_status =2 ,'Deleted',if(tag_status =3 ,'Other Issue,',if(tag_status =4 ,' In Transit',if(tag_status =5 ,'Deleted for Stock',if(tag_status =6 ,'Sales Return',if(tag_status =7 ,'Tag Issued',if(tag_status =10 ,'Delivery Ready','')))))))))) as status from ret_taging WHERE current_branch = '".$data['id_branch']."' and ".$data['searchField']." =  '".$data['searchTxt']."' ");
