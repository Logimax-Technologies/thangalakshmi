...

{
...

    {
   	...
   	c.comments,c.username,c.passwd,c.is_new,c.active,c.`date_add`,c.`date_upd`, IFNULL(c.gender,'') AS gender, c.cus_type
...

    {
        ...
        if(emp_bill.emp_name!='',emp_bill.emp_name,if(emp_bill_old.emp_name!='',emp_bill_old.emp_name,e.firstname)) as emp_name,bill.billing_for,bill.id_branch
...

	{
		...
		$data['adjusted_in_sales']=$this->get_adjust_in_sales($bill_id);

		$data['sales_rtn_bill_no']=$this->get_sales_return_bill_no($bill_id);
...

	{
...

		{
			...
			IFNULL(d.wastage_discount,0) as wastage_discount,IFNULL(d.rate_per_grm,0) as rate_per_grm,IFNULL(d.item_blc_discount,0) as item_blc_discount,c.cat_code as cat_code,b.fin_year_code
...

		{
			...
			tax.tax_percentage,d.rate_per_grm,c.cat_code as cat_code,b.fin_year_code,d.bill_det_id
		...
		$order_adj=$this->db->query("SELECT a.received_amount,a.received_weight,a.rate_per_gram,b.bill_id,a.store_as,a.advance_amount,date_format(b.bill_date,'%d-%m-%Y') as bill_date,a.advance_type
...

        {
        	...
        	od.total_sgst,od.total_cgst,od.total_igst,od.rate_per_gram,od.rate,ifnull(cat.hsn_code,'') as hsn_code
	...
	function get_order_stones($order_no) {
		...
		$order_stones=$this->db->query("SELECT os.pieces, IFNULL(os.price,0) as st_price, os.stone_id, os.stone_id, os.wt, st.stone_name, st.stone_type, st.stone_code,um.uom_short_code,os.rate_per_gram                        
	...
	function createNewCustomer($cusname, $cusmobile, $branch,$id_village,$country,$state,$city,$address1,$address2,$address3,$pincode,$mail,$cus_type,$pan_no,$aadharid,$gst_no,$title,$id_profession,$gender,$date_of_birth,$date_of_wed)
	...
	function updateNewCustomer($id_customer,$cusname, $cusmobile, $branch,$id_village,$country,$state,$city,$address1,$address2,$address3,$pincode,$mail,$cus_type,$pan_no,$aadharid,$gst_no,$title,$id_profession,$gender,$date_of_birth,$date_of_wed)
...

	{
...

		{
            ...
            a.id_state as cus_state,a.id_state as cmp_state,a.id_country as cmp_country,a.id_country as cus_country,IFNULL(cus.pan,'') as pan_no,0 as est_rate_per_grm,cat.scheme_closure_benefit
		...
		c.scheme_closure_benefit,IFNULL(tag.net_wt,0) as tag_net_wt
		...
		$item_details = $items_query->result_array();
    		...
    		ifnull(tag.tag_code,'') as tag_code,est_itms.est_rate_per_grm,IFNULL(est.discount,0) as est_discount,c.scheme_closure_benefit,IFNULL(tag.net_wt,0) as tag_net_wt,tag.sales_value as item_cost
		...
		IFNULL(est_old.touch,0) as touch
...

		{
			...
			$return_data['item_details'][]=array(
						...
						'mc_va_limit'			=> ($item['product_id']!=''  && $item['design_id']!='' && $item['id_sub_design']!=''?  $this->get_mc_va_limit($item['product_id'], $item['design_id'], $item['id_sub_design'], $grossWt , $lot_no) :'')
...

	{
			...
			$est_stone_query=$this->db->query("SELECT est_item_stone_id,est_item_id, 
                           ...
                           um.uom_short_code,est_st.uom_id
...

	{
			...
			$est_stone_query=$this->db->query("SELECT bill_item_stone_id,bill_id,bill_det_id,
                           ...
                           stone_name, stone_code, uom_name, uom_short_code,rate_per_gram,bill_st.price as amount
	...
	function get_closed_accounts($SearchTxt,$id_customer,$id_scheme_acc){
   ...
   function get_advance_details($bill_cus_id,$is_eda)

	{

	    

        $data=$this->db->query("SELECT ir.amount as total_amount, IFNULL(irp.cash_pay,0) AS total_cash_pay, 
        (ir.amount-IFNULL(advance.amount,0)-IFNULL(chit_adj.chit_utilized_amount,0)-IFNULL(advance_adjusted.amount,0)-IFNULL(refund.refund_amount,0)-IFNULL(adv_trns.transfer_amount,0))as amount,
        ir.id_issue_receipt,ir.bill_no, (IFNULL(irp.cash_pay,0)-IFNULL(advance.amount,0)-IFNULL(chit_adj.chit_utilized_amount,0)-IFNULL(advance_adjusted.amount,0)-IFNULL(refund.refund_amount,0))as cash_pay,
        adv_trns.transfer_amount
	...
	//Adv Adj
		...
		function getBillData($bill_no, $billType, $id_branch,$fin_year,$metal_type,$is_eda)
	...
	function get_borrower_details($SearchTxt,$id_branch,$issue_to,$is_eda)
	...
	*/
    ...
    function getAvailableIndCustomers($SearchTxt){
		...
		$data = $this->db->query("SELECT c.id_customer as value, concat(firstname,'-',mobile) as label, mobile,firstname, id_branch,v.village_name,if(c.is_vip=1,'Yes','No') as vip,c.id_village,
			...
			IFNULL(c.id_profession,'') as id_profession,c.gender,IFNULL(date_format(c.date_of_birth,'%d-%m-%Y'),'') as date_of_birth,IFNULL(date_format(c.date_of_wed,'%d-%m-%Y'),'') as date_of_wed
...

	{
        ...
        IFNULL(r.amount-IFNULL(coll.paid_amt,0),0) as balance_amount,IFNULL(coll.discount_amt,0) as discount_amt
