...
class Ret_catalog_model extends CI_Model
{
 ...
 /*product functions here */ 	
	...
	*/ 
	...
	{
		$data = array();
			$sql = "SELECT design_no,design_code,design_name,d.created_time,design_status,p.product_name
			        from ret_design_master d
			        left join ret_product_master p on p.pro_id=d.product_id
			        where d.product_id=".$id_product."
			        ".($from_date!='' && $to_date!='' ? " and date(d.created_time) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."'" :'')."";
		//print_r($sql);exit;
		$result = $this->db->query($sql);
		$data = $result->result_array();
		return $data;
