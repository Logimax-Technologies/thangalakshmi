...

{
	...
	//Bulk edit
...

	 {
			...
			m.tgrp_id as tax_group_id,IF(t.tag_mc_type=1,'Per Gram','Per Piece') as mc_type
...

	{
...

		{
		    ...
		    Date_format(m.updatetime,'%d-%m%-%Y %h:%i %p')as updatetime,b.enable_gift_voucher
...

	{
		...
		$sql = $this->db->query("SELECT tag.tag_id,tag.tag_code,tag.counter,
				...
				ifnull(tag_stn_detail.stn_wt,'0') as stn_wt, ifnull(tag_stn_detail.uom_name, '') as stuom 
                ...
                LEFT JOIN (SELECT tag_id,sum(amount) as stn_amount,sum(wt) as stn_wt, uom_name
...

	{
		...
		$sql = $this->db->query("SELECT tag.tag_id,tag.tag_code,tag.counter,
				...
				ifnull(tag_stn_detail.stn_wt,'0') as stn_wt
				...
				LEFT JOIN (SELECT tag_id,sum(amount) as stn_amount,sum(wt) as stn_wt

                FROM `ret_taging_stone` 

                GROUP by tag_id) as tag_stn_detail ON tag_stn_detail.tag_id = tag.tag_id

				WHERE tag_lot_id='".$lot_id."' and tag.tot_print_taken=0");
...

	{
	    ...
	    $sql=$this->db->query("SELECT tag.tag_code,IFNULL(tag.gross_wt,0) as gross_wt,IFNULL(tag.net_wt,0) as net_wt,tag.tag_id,
			...
			concat(s.value,'-',s.name) as size_name
...

    {
        ...
        IFNULL(lot_inw.order_no,'') as order_no,des.design_name
