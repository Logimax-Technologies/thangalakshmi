 ...
 /*product functions here */ 	
  ...
  function get_weight($id)
   ...
   /*Delivery functions here */ 	
	...
	{
	    $sql=$this->db->query("SELECT m.id_sub_design_mapping,cat.name as category_name,p.product_name,des.design_name,s.sub_design_name,IFNULL(m.mc_cal_type,'-') as mc_cal_type,IFNULL(m.mc_cal_value,'-') as mc_cal_value,IFNULL(m.wastage_type,'') as wastage_type,IFNULL(m.wastag_value,'-') as wastag_value
        FROM ret_sub_design_mapping m 
        LEFT JOIN ret_product_master p ON p.pro_id=m.id_product
        LEFT JOIN ret_design_master des ON des.design_no=m.id_design
	...
	function ajax_getattribute($from_date,$to_date)
	{
		...
		$sql = "SELECT attr_id, attr_name, IF(attr_group_type = 1, 'Drop Down Box', 'Radio Buttons') AS attr_type, attr_group_type, attr_status, created_on, created_by, updated_on, updated_by FROM ret_attribute";
	...
	function getActiveAttribute()
	{
		...
		$sql = "SELECT attr_id, attr_name, IF(attr_group_type = 1, 'Drop Down Box', 'Radio Buttons') AS attr_type, attr_group_type, attr_status, created_on, created_by, updated_on, updated_by FROM ret_attribute WHERE attr_status = 1";
	...
	{
		$sql = "SELECT  attr_id, attr_name, IF(attr_group_type = 1, 'Drop Down Box', 'Radio Buttons') AS attr_type, attr_group_type, attr_status, created_on, created_by, updated_on, updated_by FROM ret_attribute WHERE attr_id =".$attr_id;
