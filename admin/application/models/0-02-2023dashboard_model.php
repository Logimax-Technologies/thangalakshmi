...
/*Function added by ARVK with KCP's query*/
	...
	{
	  $result=0;	
	   $sql="Select count(d.next_due) as due_count, sum(d.payment_amount) as amount	From scheme_account sa
  Left Join
     (Select
...
Left Join payment p on(p.id_scheme_account=sa.id_scheme_account and sa.active=1 and sa.is_closed=0 and p.payment_status='1')
		...
		switch($filterBy){
			case 'T': //Today Due
			         $sql=$sql." where date(d.next_due)= CURDATE() AND sa.is_closed!=1";   
				break;
			case 'Y': //yesterday Due
			         $sql=$sql." where date(d.next_due)= CURDATE()- INTERVAL 1 DAY AND sa.is_closed!=1";   
				break;
			case 'TW': //This Week Due
			         //$sql=$sql." where date(d.next_due)= CURDATE() - INTERVAL 1 WEEK ";   
			         $sql=$sql." where Date(d.`next_due`) BETWEEN DATE_ADD(CURDATE(), INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) AND DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY) AND sa.is_closed!=1";   
				break;
			case 'ALL': //all dues 
			        $sql=$sql." ";   
				break;
			case 'TM': //This Month Due
			         //$sql=$sql." where date(d.next_due)= CURDATE() - INTERVAL 1 WEEK ";   
			         $sql=$sql." where Date(d.`next_due`) BETWEEN DATE_SUB(CURDATE(),INTERVAL (DAY(CURDATE())-1) DAY) AND LAST_DAY(NOW()) AND sa.is_closed!=1";   
				break;			
		}	
		// print_r($sql);exit;
		return $this->db->query($sql)->row_array();		
...
LEFT JOIN scheme s ON (sa.id_scheme = s.id_scheme)";	
		switch($filterBy){
			case 'T': //Today Due
			         $sql=$sql." where date(d.next_due)= CURDATE() ";   
				break;
			case 'Y': //Yesterday Due
			         $sql=$sql." where date(d.next_due)= CURDATE()- INTERVAL 1 DAY ";   
				break;
			case 'TW': //This Week Due
			         //$sql=$sql." where date(d.next_due)= CURDATE() - INTERVAL 1 WEEK ";   
			         $sql=$sql." where Date(d.`next_due`) BETWEEN DATE_ADD(CURDATE(), INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) AND DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY) ";   
				break;
			case 'ALL': //all dues 
			        $sql=$sql." ";   
				break;
			case 'TM': //This Month
			          $sql=$sql." where Date(d.next_due) BETWEEN DATE_SUB(CURDATE(),INTERVAL (DAY(CURDATE())-1) DAY) AND LAST_DAY(NOW()) ";   
				break;	
		}	
    ...
    function get_account_list($id_branch,$from_date,$to_date,$type="")
	{
	  ...
	  $result=0;	
	   $sql="select CONCAT(e.firstname,' ',e.lastname) as emp_name,IFNULL(e.emp_code,'-') as emp_code,
			  ...
			  sc.scheme_name,sc.code,sc.total_installments,sc.max_weight,sc.max_chance,c.mobile,s.date_add,
			CAST(if(sc.scheme_type=1,sc.max_weight,if(sc.scheme_type=3 && sc.max_amount!=0,sc.max_amount,if(sc.scheme_type=3 && sc.max_amount=0,(sc.max_weight*(SELECT m.goldrate_22ct FROM metal_rates m  order by id_metalrates Desc LIMIT 1)),sc.amount))) as int) as amount,ifnull(p.payment_amount,'0') as payment_amount,cs.currency_symbol
			from
			  ".self::ACC_TABLE." s
			left join ".self::CUS_TABLE." c on (s.id_customer=c.id_customer)
			join chit_settings cs
	  ...
	  $result=0;	
	   $sql="select  CONCAT(e.firstname,' ',e.lastname) as emp_name,IFNULL(e.emp_code,'-') as emp_code,
			  s.id_scheme_account,if(s.scheme_acc_number is not null,s.scheme_acc_number,'Not Allocated')as scheme_acc_number,sc.code,cs.has_lucky_draw,IFNULL(s.group_code,'')as group_code,concat (c.firstname,' ',if(c.lastname!=Null,c.lastname,'')) as name,s.ref_no,s.account_name,s.start_date,if(s.added_by=0,'Web App',if(s.added_by=1,'Admin','Mobile App'))as added_by,			  
			    if(sc.scheme_type=0,'Amount',if(sc.scheme_type=1,'Weight',if(sc.scheme_type=3,'FLEXIBLE_AMOUNT','Amount To Weight')))as scheme_type,
			  sc.scheme_name,sc.code,sc.total_installments,sc.max_weight,sc.max_chance,c.mobile,s.date_add,
			FORMAT(if(sc.scheme_type=1,sc.max_weight,if(sc.scheme_type=3 && sc.max_amount!=0,sc.max_amount,if(sc.scheme_type=3 && sc.max_amount=0,(sc.max_weight*(SELECT m.goldrate_22ct FROM metal_rates m  order by id_metalrates Desc LIMIT 1)),sc.amount))),2) as amount,ifnull(p.payment_amount,'-') as payment_amount,cs.currency_symbol
			from
			  ".self::ACC_TABLE." s
			left join ".self::CUS_TABLE." c on (s.id_customer=c.id_customer)
			join chit_settings cs
	...
	function get_customer_joined($from_date,$to_date,$type)
{
    $id_branch = $this->input->post('id_branch');
      ...
      $sql = "Select c.id_customer,c.cus_type,p.profile_name,if(c.added_by=0,'Web App',if(c.added_by=1,'Admin','Mobile App'))as added_by,concat(firstname,' ',if(lastname!=NULL,lastname,'')) as name,c.mobile, count(sa.id_scheme_account) as accounts,c.is_new,c.profile_complete,c.`date_add`,c.active From customer c  left join scheme_account sa on (c.id_customer=sa.id_customer and sa.active=1 and sa.is_closed=0) left join profile p on c.profile_complete=p.id_profile
       where c.added_by=".$type." and (date(c.date_add) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
       ".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and c.id_branch=".$id_branch:'')." 
