...

{
...

	{
		...
		br.short_name as branch_code,d.name as delivery_location,IFNULL(v.village_name,'') as village_name,c.gst_number,a.address1,a.address2,a.pincode,c.cus_type,c.pan as pan_number,
		...
		b.bill_status,IFNULL(b.credit_disc_amt,0) as credit_disc_amt
...

		{
			...
			if($data['bill_type']==8)
			{
			    $data['tot_bill_amount']=$this->get_due_bill($data['ref_bill_id']);
...

	{
		...
		$items_query = $this->db->query("SELECT b.tot_bill_amount

		FROM ret_billing b

		where b.bill_id=".$bill_id."");

		return $items_query->row('tot_bill_amount');
