...

{
...

	{
		...
		$sql = $this->db->query("SELECT estimation_id,esti_no,
				...
				cus.firstname as customer_name,e.emp_code,b.short_name 
...

	{
		...
		$items_query = $this->db->query("SELECT est_item_id, esti_id, item_type, 
                       ...
                       concat(design_name,'-',design_code) as design_name,design_code, pur.purity as purname,
                       ...
                       est.item_total_tax,est.market_rate_cost,tag.net_wt as tag_net_wt,txgrp.tgrp_name,tag.tag_code,IFNULL(est.id_orderdetails,'') as id_orderdetails
		...
		$old_matel_query = $this->db->query("SELECT est_old.old_metal_sale_id, est_id, 
						   ...
						   pur.purity as purname, met.metal ,est_old.purpose,est_old.id_old_metal_type, est_old.id_old_metal_category
	...
	function createNewCustomer($cusname, $cusmobile, $branch,$id_village,$cus_type)

	{
		...
		if($customer_check_query->num_rows() == 0){
            ...
            $id_country = (!empty($_POST['id_country']) ? $_POST['id_country'] :NULL);
            $id_state   = (!empty($_POST['id_state']) ? $_POST['id_state'] :NULL);
            $id_city    = (!empty($_POST['id_city']) ? $_POST['id_city'] :NULL);
            $id_village  = (!empty($id_village) ? $id_village :NULL);
            
			$insert_data = array("firstname" => strtoupper($cusname), "id_branch" => $branch, "mobile" => $cusmobile, "username" => $cusmobile, "passwd" => $this->encrypt($cusmobile),'id_village'=>$id_village,'cus_type'=>$cus_type,"date_add"=>date("Y-m-d H:i:s"));
			...
			if(!empty($cus_insert_id)){
				...
				$address_data=array('id_country'=>$id_country,'id_state'=>$id_state,'id_city'=>$id_city,"id_customer"=>$cus_insert_id);
				
				$cus_insert_id = $this->insertData($address_data, "address");
	...
	function updateCustomer($id_customer,$cusname, $cusmobile, $branch,$id_village,$cus_type)
	{
			$update_Data = array("firstname" => strtoupper($cusname),'id_village'=>($id_village!='' ?$id_village :1),'cus_type'=>$cus_type);
	...
	function getAvailableCustomers($SearchTxt,$esti_for){
...

    {
        ...
        IFNULL(lot_inw.order_no,'') as order_no,des.design_name,tag.tag_mark,r.rate_field,r.market_rate_field
    ...
    function getProductBySearch($SearchTxt,$category)
...

    {
...

        {
			...
			WHERE pro.stock_type=1 and (product_short_code LIKE '%".$SearchTxt."%' OR product_name LIKE '%".$SearchTxt."%') ".(empty($category) ? ' ' : 'AND pro.cat_id ='.$category)." GROUP BY pro.pro_id");
	...
	function get_closed_accounts($SearchTxt){
		...
		$data = $this->db->query("SELECT  sa.id_scheme_account as value,sa.scheme_acc_number as label,sa.closing_balance,sa.is_closed,s.scheme_type
				...
				WHERE sa.scheme_acc_number LIKE '%".$SearchTxt."%' and sa.is_closed=1");
	...
	*/
...

    {
        ...
        product_name, product_short_code,tag.product_id, pur.purity as purname,tag.id_orderdetails,mt.id_metal,mt.tgrp_id as tax_group_id,tag.tag_mc_type,c.order_no,r.rate_field,r.market_rate_field,tag_stn_detail.stn_amount as stn_amt,
        d.design_name
        FROM ret_taging tag
...

    {
		...
		WHERE c.fin_year_code=".$fin_year." and c.order_no='".$order_no."' ".($id_branch!='' ? "and c.order_from=".$id_branch."" :'')." ");
...

    {
        ...
        tag.tag_code as label, tag_datetime,mt.tgrp_id as tax_group_id,
