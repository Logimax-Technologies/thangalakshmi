...

{
        ...
        if($data['branch_settings']==1){ // Branch Enabled
            if($data['scheme_wise_receipt'] == 2 && $branch >0){  // 2 - branch wise Receipt number
                $sql = "Select max(p.receipt_no) as receipt_no
    				...
    				Where  p.payment_status=1   and p.id_branch=".$branch." 
                ...
                $sql = "Select max(p.receipt_no) as receipt_no
    				From payment p
        				...
        				left join customer c on c.id_customer= sa.id_customer
        				left join scheme s on s.id_scheme=sa.id_scheme
    				Where  p.payment_status=1 and sa.id_scheme=".$id_scheme." AND p.id_branch=".$branch."
               ...
               $financial_year = $res->row()->fin_date;
                 ...
                 $sql = "Select p.receipt_no
    				...
    				left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
        			left join customer c on c.id_customer= sa.id_customer
    				Where  date(date_payment) BETWEEN '".$financial_year."' AND DATE(CURDATE()) and p.payment_status=1 ".($id_company!=0 && $company_settings == 1? "and c.id_company=".$id_company."" :'')." order by receipt_no desc limit 0,1 ";
            }
               ...
               $financial_year = $res->row()->fin_date;
                $sql = "Select max(p.receipt_no) as receipt_no
    				From payment p
        				...
        				left join customer c on c.id_customer= sa.id_customer
        				left join scheme s on s.id_scheme=sa.id_scheme
    				Where  date(date_payment) BETWEEN '".$financial_year."' AND DATE(CURDATE()) and p.payment_status=1 and sa.id_scheme=".$id_scheme." AND p.id_branch=".$branch."
    				".($id_company!=0 && $company_settings == 1? "and c.id_company=".$id_company."" :'')." group by sa.id_scheme,p.id_branch ";
            }
            else  // If other cases fails,generate 1 - common Receipt number
            {
                $sql = "Select p.receipt_no
    				...
    				left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
        			left join customer c on c.id_customer= sa.id_customer
    				Where  p.payment_status=1 ".($id_company!=0 && $company_settings == 1? "and c.id_company=".$id_company."" :'')." order by receipt_no desc limit 0,1 "; 
            }
        } else{
            if($data['scheme_wise_receipt'] == 3){   // 3 - Scheme-wise Receipt number
		        $sql = "Select max(p.receipt_no) as receipt_no
    				From payment p
        				...
        				left join customer c on c.id_customer= sa.id_customer
        				left join scheme s on s.id_scheme=sa.id_scheme
    				Where  p.payment_status=1 and sa.id_scheme=".$id_scheme." ".($id_company!=0 && $company_settings == 1? "and c.id_company=".$id_company."" :'')." group by sa.id_scheme ";
            }
            else if($data['scheme_wise_receipt'] == 5){// financial year wise
                $res = $this->db->query("SELECT date(fin_year_from) as fin_date FROM `ret_financial_year` where fin_status = 1");
               $financial_year = $res->row()->fin_date;
...

   {
...

	{
                    ...
                    //old code 11/05/2023 concat(ifnull(concat(p.receipt_year,'-'),''),p.receipt_no) as receipt_no,
		...
		$sql="SELECT  IFNULL(sa.start_year,'') as start_year,IFNULL(p.receipt_year,'') as receipt_year,(select b.short_name from branch b where b.id_branch = sa.id_branch) as acc_branch, (select b.short_name from branch b where b.id_branch = p.id_branch) as payment_branch, b.short_name, s.code,cs.schemeaccNo_displayFrmt,cs.receiptNo_displayFrmt,s.is_lucky_draw,ifnull(sa.scheme_acc_number,'Not Allocated') as scheme_acc_number,ifnull(p.receipt_no,'No Receipt') as receipt_no,cs.scheme_wise_receipt,cs.scheme_wise_acc_no,cs.scheme_wise_acc_no,
					  ...
					  c.mobile,cs.scheme_wise_acc_no,b.short_name,
					  ...
					  IFNULL(concat(if(cs.scheme_wise_acc_no=4 or scheme_wise_acc_no=5, IFNULL(concat(sa.start_year,'-'),'') ,''),sa.scheme_acc_number),'Not Allocated') as oldscheme_acc_number,
					  ...
					  cs.receipt_no_set,p.receipt_year,(select IFNULL(short_name,'') from branch where id_branch=sa.id_branch) as branch_code,


                     if(cs.scheme_wise_receipt=2,p.receipt_no,concat(ifnull(concat(p.receipt_year,'-'),''),p.receipt_no)) as oldreceipt_no,
       ...
       Where  sa.active = 1 and sa.is_closed = 0 and (date(".($date_type!='' ? ($date_type==2 ?"p.custom_entry_date":"p.date_payment") : "p.date_payment").") BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') ".($id_employee!=NULL||$id_employee!=''?' and p.id_employee ='.$id_employee:'')."
	...
	if($this->branch_settings==1){
		...
		$sql="SELECT
					  ...
					  group_concat(IFNULL(sa.scheme_acc_number,'Not Allocated')) as scheme_acc_number,



					  group_concat(s.code) as code,p.is_print_taken,
...
function checkSettings()
...

	{
		...
		$branchWiseLogin=$this->session->userdata('branchWiseLogin');
			...
			$sql = "Select s.is_lucky_draw,IFNULL(sa.start_year,'') as start_year,(select b.short_name from branch b where b.id_branch = sa.id_branch) as acc_branch,s.code,cs.schemeaccNo_displayFrmt,s.is_lucky_draw,ifnull(sa.scheme_acc_number,'Not Allocated') as scheme_acc_number,cs.scheme_wise_acc_no,
 sa.id_scheme_account,
...
function get_paymentContent($id_scheme_account)
	...
	{  
	   ...
	   $schemeAcc = array();
		...
		$sql="Select s.amt_based_on,s.id_metal,c.firstname, s.is_digi,IFNULL((SELECT SUM(p.payment_amount) FROM payment p WHERE p.id_scheme_account = sa.id_scheme_account AND p.payment_status = 1 AND date(p.date_payment) = curdate()),0) as curday_total_paid,
...
//if(Date_Format(max(p.date_add),'%Y%m') = Date_Format(curdate(),'%Y%m'), SUM(p.no_of_dues),0)  as currentmonthpaycount,	 
	...
	//allow pay for advance settings...
	    ...
	    if($record->allow_advance == 1 && $record->advance_months > 0 &&  $allowed_due != 0 && ($allowed_due <= $record->advance_months)){
...
//if(Date_Format(max(p.date_add),'%Y%m') = Date_Format(curdate(),'%Y%m'), SUM(p.no_of_dues),0)  as currentmonthpaycount,	
		...
		function get_payment_report()
	{
		...
		$sql = "SELECT IF(s.scheme_type=0 OR s.scheme_type=2,s.amount,IF(s.scheme_type=1 ,s.max_weight,if(s.scheme_type=3,s.min_amount,0))) as payable,IF(s.scheme_type=0 OR s.scheme_type=2,s.amount,IF(s.scheme_type=1 ,s.max_weight,if(s.scheme_type=3,s.min_amount,0))) as payable, s.scheme_name,ifnull(sa.account_name,'-') as account_name,
...
/* / Coded by ARVK */
...

	{
        ...
        //old code 11/05/2023 	if(c.has_lucky_draw=1 && sch.is_lucky_draw = 1,concat(ifnull(sch_acc.group_code,''),'  ',ifnull(sch_acc.scheme_acc_number,'Not Allocated')),concat(sch.code,'  ',IFNULL(sch_acc.scheme_acc_number,'Not Allocated')))as scheme_acc_number,

		$records = array();



		$query_invoice = $this->db->query("SELECT IFNULL(sch_acc.start_year,'') as start_year,(select b.short_name from branch b where b.id_branch = sch_acc.id_branch) as acc_branch,sch.code,c.schemeaccNo_displayFrmt,sch.is_lucky_draw,ifnull(sch_acc.scheme_acc_number,'Not Allocated') as scheme_acc_number,c.scheme_wise_acc_no,
                            ...
                            if(c.has_lucky_draw=1 && sch.is_lucky_draw = 1,concat(ifnull(sch_acc.group_code,''),'  ',ifnull(sch_acc.scheme_acc_number,'Not Allocated')),if(c.scheme_wise_acc_no=3,concat(b.short_name,sch.code,'-',IFNULL(sch_acc.scheme_acc_number,'Not Allocated')),concat(sch.code,'  ',IFNULL(sch_acc.scheme_acc_number,'Not Allocated'))))as oldscheme_acc_number,
		...
		if($query_invoice->num_rows() > 0)
...

			{
...

				{
					...
					'id_scheme_account' => $row->id_scheme_account,'scheme_acc_number' => 
...
function get_online_payment($id_payment)

	{
		...
		$sql="SELECT 
		  ...
		  IF(payment_mode = 'MULTI', concat('MULTI | ',(SELECT group_concat(concat(payment_mode,'-',payment_amount)) as multi_mode FROM `payment_mode_details` where id_payment=".$id_payment." and payment_status=1)),IFNULL(p.payment_mode,'-') )as payment_mode,
		  ...
		  IFNULL((select  
			...
			where (pay.bank_name is not null or pay.id_bank  is not null) and pay.id_payment = p.id_payment and pay.payment_status=1),'-') as bank_name,
			IFNULL((select GROUP_CONCAT(IFNULL(card_no,'')) from payment_mode_details where (card_no is not null) and id_payment=p.id_payment and payment_status=1),'-') as card_no
...
//if(Date_Format(max(p.date_add),'%Y%m') = Date_Format(curdate(),'%Y%m'), SUM(p.no_of_dues),0)  as currentmonthpaycount,	
	...
	{
       $sql="SELECT id_pg,pg_name,pg_code,type
			 FROM gateway g 
			 where active=1";
 ...
 function firstPayamt_payable()
    ...
    function getBranchGatewayData($branch_id,$pg_code)
...

	 {
   		...
   		$sql="SELECT param_1,param_2,param_3,param_4,pg_code,api_url,id_pg from gateway where active =1 and is_default=1 and  pg_code=".$pg_code." ".($branch_id!='' ?"and id_branch=".$branch_id."" :'')."";
...
//Kyc Approval Data status filter with date picker//	
   ...
   /*Functions for pay settled payments Begins */
...

    {
        ...
        IFNULL(s.closing_weight,0)as closing_weight
        ...
        $acc_details=$accounts->result_array();



				//print_r($acc_details);exit;



				foreach($acc_details as $r)
...
/*	function get_today_collection_details($from_date,$to_date,$id_scheme,$id_branch)
...

   {
...

	{
        ...
        $sql = $this->db->query("SELECT p.ref_trans_id as txn_ids,payment_status,sum(p.payment_amount) as payment_amount,c.email,c.mobile,p.date_payment FROM payment p
                                left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
                                left join customer c on c.id_customer=sa.id_customer
                                ...
                                AND is_gateway_verified = 0 GROUP BY ref_trans_id LIMIT 50");
                                
        /*$sql = $this->db->query("SELECT p.ref_trans_id as txn_ids,payment_status,p.payment_amount,c.email,c.mobile,p.date_payment FROM payment p
                                left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
                                left join customer c on c.id_customer=sa.id_customer
                                WHERE ".($id_pg > 0 ? ' p.id_payGateway ='.$id_pg.' and ' : '' )." 
                                ".($id_branch > 0 ? ' p.id_branch ='.$id_branch.' and ' : '' )." 
                                id_payment = 1907
                                AND is_gateway_verified = 1 LIMIT 1");*/
...

	{
    	...
    	cs.custom_entry_date,p.payment_amount,s.one_time_premium,sa.id_branch as branch,cs.allow_referral,cs.gent_clientid,s.firstPayamt_maxpayable,is_lucky_draw,c.mobile,c.email
...

	{
    	...
    	cs.custom_entry_date,p.payment_amount,s.one_time_premium,sa.id_branch as branch,cs.allow_referral,cs.gent_clientid,s.firstPayamt_maxpayable,is_lucky_draw,c.mobile,c.email,sum(p.payment_amount) as pay_amt
...
// Scheme source wise report  --- scheme wise payment details report with mode wise + online & showroom collection report   --> START 
  ...
  function sheme_payment_list_daterange($from_date,$to_date,$id_classfication,$id_scheme,$pay_mode,$id_branch,$mode)
    ...
    {   
        ...
        //oldcode 11/05/2023 CONCAT(s.code,' - ',sa.scheme_acc_number) as scheme_acc_number,
        ...
        $sql = $this->db->query("SELECT IFNULL(sa.start_year,'') as start_year,(select b.short_name from branch b where b.id_branch = sa.id_branch) as acc_branch,s.code,chit.schemeaccNo_displayFrmt,s.is_lucky_draw,ifnull(sa.scheme_acc_number,'Not Allocated') as scheme_acc_number,chit.scheme_wise_acc_no,
		
		                    IFNULL(p.receipt_year,'') as receipt_year, (select b.short_name from branch b where b.id_branch = p.id_branch) as payment_branch, s.code,chit.receiptNo_displayFrmt,ifnull(p.receipt_no,'') as receipt_no,chit.scheme_wise_receipt,


                c.mobile, concat(if(chit.scheme_wise_receipt=4 or chit.scheme_wise_receipt=5,ifnull(concat(p.receipt_year,'-'),''),''),p.receipt_no) as oldreceipt_no, CONCAT(c.firstname,' ',IFNULL(c.lastname,'')) as cusname, IFNULL(sa.account_name,'-') as name,
        
                if(chit.scheme_wise_acc_no=3,if(sa.scheme_acc_number is not null,concat(IFNULL(b.short_name,''),s.code,'-',sa.scheme_acc_number),concat(IFNULL(b.short_name,''),s.code,'-NotAllocated')),CONCAT(s.code,' - ',sa.scheme_acc_number)) as oldscheme_acc_number,
                ...
                IFNULL(p.metal_rate,'-') as metal_rate,IF(p.metal_weight!='0' && p.metal_weight!='' ,p.metal_weight,'0') as tot_metal_weight, IF(p.metal_weight!='0' && p.metal_weight!='' ,(pmd.payment_amount / p.metal_rate),'0') as metal_weight,  



                IF(p.payment_mode='FP','0',pmd.payment_amount) as payment_amount,b.name as pay_branch,if(p.added_by=0,'Admin',if(p.added_by=1,'Web App',if(p.added_by=2,'Mobile App',if(p.added_by=3,'Collection App',if(p.added_by=4,'Retail',if(p.added_by=5,'Sync',if(p.added_by=6,'Import','-'))))))) as payment_through,s.scheme_name,s.firstPayDisc_value as discountAmt,
                ...
                Where sa.active = 1 and sa.is_closed = 0 and s.active = 1 and pmd.payment_status = 1 and p.payment_status = 1 and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') And p.payment_status=1
 ...
 */
	...
	function payment_summary_modewise_data($from_date="",$to_date="",$id_classfication="",$id_scheme="",$pay_mode="",$id_branch="",$mode=""){
	  ...
	  //  print_r($_POST);exit;
	    ...
	    $sql1 = "SELECT  SUM(pmd.payment_amount) as offline_amt, 
               ...
               Where sa.active = 1 and sa.is_closed = 0  and s.active = 1 and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
                ...
                ".($id_scheme!='' ? " and s.id_scheme=".$id_scheme."" :'')."
                 ...
                 ".($pay_mode!='' ? (($pay_mode == 0 || $pay_mode == 3) ? " and p.added_by=".$pay_mode."" :" " ) : "AND p.added_by IN (0,3,6)")."
  ...
  //echo $sql1;exit;
        ...
        if(($pay_mode == 0 || $pay_mode == 3 ) && $pay_mode!=''){
    ...
    {
        $sql = $this->db->query("SELECT id_scheme_account,date_payment,payment_amount,round(metal_rate,0) as metal_rate,payment_status,round(metal_weight,3) as metal_weight,receipt_no from payment where id_payment=".$id_payment);
        return $sql->row_array();
    }
    ...
    function update_dueMonYear($id_scheme_account){
        ...
        if(due_type = 'AD',  Date_Format((date_add(date(p.date_payment), INTERVAL @months month)),'%Y-%m')  , if(due_type != 'PD',Date_Format(date(p.date_payment),'%Y-%m'),null)) as paidmonth,
