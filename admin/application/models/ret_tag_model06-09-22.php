...

{
...

	{
        ...
        IFNULL(d.semiprecious_st_certif,'') as semiprecious_st_certif,m.id_metal,c.cat_code,p.sales_mode,c.tgrp_id as tax_group_id,l.id_purity,c.is_multimetal,p.has_size
...

	{
...

	 {
	    ...
	    function getTagDetails($tag_id)
        {
        	...
        	IFNULL(tag_dia_detail.dia_uom_short_code, '') as dia_uom_short_code,pd.div_value AS product_division
        	FROM ret_taging as tag 
        	join company c
        	LEFT JOIN ret_design_master as des ON des.design_no = tag.design_id
	...
	function get_retagging_details($data)
	{
        ...
        /*$non_tag_sql = $this->db->query("SELECT d.ret_bill_det_id as tag_id,p.product_name,des.design_name,bill.sales_ref_no,det.gross_wt,det.net_wt,det.item_cost as sales_value,
        br.name as branch_name,date_format(b.bill_date,'%d-%m-%Y') as bill_date,m.metal_code,'' as tag_code,p.stock_type,det.piece
        FROM ret_bill_return_details d 
        LEFT JOIN ret_billing b ON b.bill_id = d.bill_id
        LEFT JOIN ret_bill_details det ON det.bill_det_id = d.ret_bill_det_id
        LEFT JOIN ret_billing bill ON bill.bill_id = det.bill_id
        LEFT JOIN ret_product_master p ON p.pro_id = det.product_id
        LEFT JOIN ret_design_master des ON des.design_no = det.design_id
        LEFT JOIN branch br on br.id_branch=b.id_branch
        LEFT JOIN ret_category c on c.id_ret_category=p.cat_id
        LEFT JOIN metal m on m.id_metal=c.id_metal 
        WHERE d.ret_bill_det_id IS NOT NULL AND det.is_non_tag = 1
        ".($data['id_branch']!='' && $data['id_branch']>0 ? " and det.current_branch=".$data['id_branch']."" :'' )."
         AND det.non_tag_process = 0");
        
        $non_tagged_items =  $non_tag_sql->result_array();*/
        
        //$returnData = array_merge($tagged_items,$non_tagged_items);
