...
class Ret_billing_model extends CI_Model
{
    ...
    function getBillingDetails($bill_id)
	{
		...
		IFNULL(b.credit_due_amt,0) as credit_due_amt,IFNULL(b.credit_ret_amt,0) as credit_ret_amt,b.to_branch,b.from_branch,created_time
	...
	function getOtherEstimateItemsDetails($bill_id,$bill_type)
	{
        ...
        if($bill_type==5)
        {
            ...
            subDes.sub_design_name,concat(s.value,' ',s.name) as size_name,c.balance_type,
			od.total_sgst,od.total_cgst,od.total_igst,od.rate_per_gram,od.rate
            FROM customerorderdetails od
            LEFT JOIN customerorder c ON c.id_customerorder=od.id_customerorder
            LEFT JOIN ret_product_master p ON p.pro_id=od.id_product
            LEFT JOIN ret_design_master des ON des.design_no=od.design_no
	...
	function get_order_stones($order_no) {
		...
		$order_stones=$this->db->query("SELECT os.pieces, IFNULL(os.price,0) as st_price, os.stone_id, os.stone_id, os.wt, st.stone_name, st.stone_type, st.stone_code
	...
	function code_number_generator($id_branch,$metal_type,$ret_sales_type)
	{
		$lastno = $this->get_last_code_no($id_branch,$metal_type,$ret_sales_type);
	...
	function get_last_code_no($id_branch,$metal_type,$ret_sales_type)
    {
        $fin_year = $this->get_FinancialYear();
   ...
   function get_advance_details($bill_cus_id)
	...
	//Adv Adj
		...
		WHERE ".(!empty($id_branch)? "bill.id_branch=".$id_branch.' AND' :'')." bill.sales_ref_no ='".$bill_no."'  AND bill_items.bill_det_id IS NOT NULL and bill.bill_status=1
		".($metal_type!='' ? " and mt.id_metal=".$metal_type."" :'')."
	...
	function get_previous_order_details($order_no, $bill_id, $branch_id, $bill_date)
	{
		...
		$sql=$this->db->query("SELECT 
							...
							WHERE rba.order_no = '".$order_no."' AND 
								  ...
								  rba.advance_date <= '".$bill_date."' AND
