...
class Ret_order_model extends CI_Model
{
	...
	function generateOrderNo($id_branch, $ordertype)
	{
		$branch_code = "";
		$lastno = NULL;
		$fin_year_code = "";
		...
		if($isBranch){
			...
			$branch_sql->free_result();
				$sql = "SELECT MAX(RIGHT(order_no,5)) as lastorder_no
					FROM customerorder o
			...
			$sql = "SELECT order_no as lastorder_no FROM customerorder fin_year_code=".$fin_year['fin_year_code']." ".($isBranch ? " and id_branch=".$id_branch : '')." ORDER BY id_customerorder DESC limit 1";
			$result = $this->db->query($sql); 
			if( $result->num_rows() > 0){
				$lastno = $result->row()->lastorder_no;
			} 
		...
		return $branch_code."-".$order_for."-".$order_number;
	}
	...
	function ajax_getOrders($data){
		...
		co.order_items,if(o.order_status=6,m.order_status,'') as status
		from  customerorder o
		LEFT JOIN (
		SELECT id_customerorder, count(id_customerorder) AS order_items 
	...
	//New order
	function get_new_orderlist($from_date,$to_date,$branch,$id_karigar)
	{
	...
	{
	    $order_advance = 0;
	    $sql = $this->db->query("SELECT sum(advance_amount) as advance_amount FROM ret_billing_advance WHERE id_customerorder = '".$orderid."'");
	    if($sql->num_rows() > 0){
