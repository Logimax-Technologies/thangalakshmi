    ...
    <div>
			...
			<div style="width: 100%; text-transform:uppercase;">
				...
				<div style="display: inline-block; width: 50%; padding-left:0px;margin-top:20px;">
                        ...
                        <label><?php echo (isset($billing['address1']) && $billing['address1']!='' ?$billing['address1'].',' :''); ?></label>
                        <label><?php echo (isset($billing['address2']) && $billing['address2']!='' ? "<br>".$billing['address2'].',' :''); ?></label>
                        <label><?php echo (isset($billing['village_name']) && $billing['village_name']!='' ? "<br>".$billing['village_name'].',' :''); ?></label>
                        <label><?php echo ($billing['city']!='' ?  "<br>".$billing['city'].($billing['pincode']!='' ? ' - '.$billing['pincode'].'.' :''):''); ?></label>
                        <label><?php echo ($billing['cus_state']!='' ?  "<br>".$billing['cus_state'].'.' :''); ?></label>
                        <br>
                        <label><?php echo (isset($billing['pan_no']) && $billing['pan_no']!='' ? ".<br>.".  'PAN : '.$billing['pan_no'] :''); ?></label>
                        <?php 
                        if($billing['billing_for']==2)
                        {?>
                            <label><?php echo (isset($billing['gst_number']) && $billing['gst_number']!='' ? ".<br>.".  'GST IN : '.$billing['gst_number'] :''); ?></label>
                        <?php }
                        ?>
                        
                        <br>
                        <label><?php echo '<div class="addr_labels">place of supply</div><div class="addr_values">:&nbsp;&nbsp;'.$billing['cus_state'].'-'.$comp_details['state_code']."</div>"; ?></label><br>
                        <label><?php echo '<div class="addr_labels">reverse charge</div><div class="addr_values">:&nbsp;&nbsp;No</div>'; ?></label><br>
                        
				</div>
				
                <?php }else if($billing['billing_for']==3){?>
			...
			<div style="text-align: left;width: 100%; text-transform:uppercase;margin-top:-8px;">
		        <div style="text-align: left; width:100%;height: 18px; ">
					...
					<div style="width: 15%; display: inline-block; margin-top: -1px"> <?php echo number_format($gold_metal_rate,2,'.','').'/'.'Gm'; ?></div>
				</div>
				
				<div style="text-align: left; width:100%;height: 18px; ">
					<div style="width: 10%; display: inline-block"> SILVER &nbsp; : &nbsp; </div>
					<div style="width: 15%; display: inline-block; margin-top: -1px"> <?php echo $silver_metal_rate.'/'.'Gm'; ?> </div>
				</div>
	    	...
	    	<div style="width: 100%; text-align: right; text-transform:uppercase;display: inline-block;margin-top:-40px;">
			    <div style="text-align: right; width:100%;height: 18px;">
					...
					<div style="width: 10%; display: inline-block; margin-top: -2px;text-align: left;"> <b><?php echo $billing['bill_date']; ?></b></div>
				</div>
				<!--<div style="text-align: right; width:100%;height: 18px;">
					<div style="width: 80%; display: inline-block"> Invoice no &nbsp; : &nbsp; </div>
					<div style="width: 18%; display: inline-block; margin-top: -2px"> <?php echo ($billing['branch_code']!='' ? $billing['branch_code'].'/':'').($billing['fin_year_code']!='' ?$billing['fin_year_code'].'/' :'').(($billing['bill_type']==1 || $billing['bill_type']==2 || $billing['bill_type']==3 || $billing['bill_type']==7 || $billing['bill_type']==9) ? 'SA/': ($billing['bill_type']==4 ? 'PUR/' :($billing['bill_type']==5 ? 'OD/' :''))).$billing['bill_no']; ?> </div>
				</div>-->
				
					<div style="text-align: right; width:100%;height: 18px;">
					<div style="width: 80%; display: inline-block"> Invoice no &nbsp; : &nbsp; </div>
					<div style="width: 15%; display: inline-block; margin-top: -2px;text-align: left;">
					    <?php 
					    if(sizeof($est_other_item['item_details'])>0) //SALES BILL 
					    {
					        echo $billing['branch_code'].'-SA-'.$billing['metal_code'].'-'.$billing['sales_ref_no'];
					    }else if(sizeof($est_other_item['old_matel_details'])>0) // OLD METAL ITEMS
					    {
					        echo $billing['branch_code'].'-PU-'.$billing['metal_code'].'-'.$billing['pur_ref_no'];
					    }
					    else if(sizeof($est_other_item['return_details'])>0) //SALES RETURN
					    {
					        echo $billing['branch_code'].'SR-'.$billing['metal_code'].'-'.$billing['s_ret_refno'];
					    }
					    else if($billing['bill_type']==5) //ORDER ADVANCE
					    {
					        echo $billing['branch_code'].'-OD-'.$billing['order_adv_ref_no'];
					    }
					    else if($billing['bill_type']==8)   //CREDIT COLLECTION
					    {
					        echo $billing['branch_code'].'-CC-'.$billing['credit_coll_refno'];
					    }
					    else if($billing['bill_type']==10)   //CHIT PRE CLOSE
					    {
					        echo $billing['branch_code'].'-'.$billing['chit_preclose_refno'];
					    }else
					    {
					        echo $billing['bill_no'];
					    }
					    ?>
					</div>
				</div>
		 	...
		 	<div style="width: 100%; text-align: center; margin-top:-67px; font-weight: bold; text-transform:uppercase;">
				<label>
					<?php echo "TAX INVOICE - ".($billing['bill_type']==1 ? 'Sales Bill': ($billing['bill_type']==2 ? 'Sales And Purchase Bill':($billing['bill_type']==3 ? 'Sales and purchase bill' :($billing['bill_type']==4 ? ' URD Purchase Bill':($billing['bill_type']==5 ? 'Order Advance' :($billing['bill_type']==6 ? 'Advance' :($billing['bill_type']==7 ? 'Sales Return':($billing['bill_type']==9 ? 'Order Delivery' :($billing['bill_type']==10 ? 'Chit PreClose': ($billing['bill_type']==11 ? 'Repair Order Delivery' : 'Credit Collection') ))))))))).($billing['bill_status']==2 ? '- Cancelled' : '');?>
				</label>
			</div>
												...
												<!--</tbody> -->
														...
														{?>
    														...
    														<tr>
    															...
    															<td class="alignRight">Rs.</td>
    															<td class="alignRight"><?php echo moneyFormatIndia(number_format(round($tot_sales_amt+$billing['handling_charges']+$billing['round_off_amt']),2,'.',''));?></td>
    														</tr>
