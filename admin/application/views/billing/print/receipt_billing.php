				...
				<div style="width: 100%; text-transform:uppercase;">
					...
					<div align="center" style="display: inline-block; width: 22%;padding-left:40px;font-weight: bold; text-align:center !important; padding-top:70px">
						<label style="white-space:nowrap;">
							...
							<!--<?php echo $billing['bill_type'] != 15 ? "  " . ($billing['bill_type'] == 1 ? 'Sales Bill' : ($billing['bill_type'] == 2   ? ($purchase_old_metal_type == 1 ? 'Sales & Old Gold  Purchase ' : ($purchase_old_metal_type == 2 ? 'Sales & Old Silver Purchase' : 'Sales & Purchase')) : ($billing['bill_type'] == 14 ? "Sales Return Transfer" : ($billing['bill_type'] == 3 ?   'Sales & Credit Sales ' : ($billing['bill_type'] == 4 ? ($purchase_old_metal_type == 1 ? 'Old Gold Purchase Bill' : $purchase_old_metal_type == 2 ? 'Old Silver Purchase Bill' : 'Purchase Bill') : ($billing['bill_type'] == 5 ? 'Order Advance' : ($billing['bill_type'] == 6 ? 'Advance' : ($billing['bill_type'] == 7 ? 'Sales Credit' : ($billing['bill_type'] == 9 ? 'Order Delivery' : ($billing['bill_type'] == 10 ? 'Chit PreClose' : ($billing['bill_type'] == 11 ? 'Repair Order' : ($billing['bill_type'] == 13 ? 'Sales Transfer' : ($billing['bill_type'] == 15 ? 'Approval Bill Acknowledgement' : 'Credit Collection'))))))))))))) . "" : ($billing['bill_status'] == 2 ? '- Cancelled' : '') ?>
						-->

							<?php echo $billing['bill_type'] != 15 ? "  " .
								($billing['bill_type'] == 1 ? ($billing['is_to_be'] == 1 ? 'To Be Sales Bill' : ($billing['is_credit'] == 1 ? 'Credit Sales Bill' : 'Sales Bill')) : ($billing['bill_type'] == 2   ? ($metal_name != '' ? 'Sales & Old ' . $metal_name . ' Purchase ' : 'Sales & Purchase') : ($billing['bill_type'] == 14 ? "Sales Return Transfer" : ($billing['bill_type'] == 3 ?   'Sales & Credit Sales ' : ($billing['bill_type'] == 4 ? ($metal_name != '' ? 'Old ' . $metal_name . ' Purchase Bill' : 'Purchase Bill') : ($billing['bill_type'] == 5 ? 'Order Advance' : ($billing['bill_type'] == 6 ? 'Advance' : ($billing['bill_type'] == 7 ? 'Sales Credit' : ($billing['bill_type'] == 9 ? 'Sales Bill' : ($billing['bill_type'] == 10 ? 'Chit PreClose' : ($billing['bill_type'] == 11 ? 'Repair Order' : ($billing['bill_type'] == 13 ? 'Sales Transfer' : ($billing['bill_type'] == 15 ? 'Approval Bill Acknowledgement' : 'Credit Collection'))))))))))))) . "" : ($billing['bill_status'] == 2 ? '- Cancelled' : '') ?>
							...
							<tbody>
								...
								<?php if ($card_amt > 0) { ?>
									...
									<?php foreach ($payment['pay_details'] as $cardItem) {
										if ($cardItem['payment_mode'] == 'DC' || $cardItem['payment_mode'] == 'CC') { ?>
											<tr>
												<td style="width:24%"><?php echo $cardItem['payment_mode'] == 'DC' ? 'DEBIT CARD' : ($cardItem['payment_mode'] == 'CC' ? 'CREDIT CARD' : '') ?>(<?php echo $cardItem['card_no'] ?>)</td>
								...
								<?php if ($rtgs_amt > 0) { ?>
									<tr>
										...
										<td style="width:10%"class="alignRight"><?php echo moneyFormatIndia(number_format($rtgs_amt, 2, '.', '')); ?></td>
										<td style="width:66%"></td>
								...
								<?php if ($imps_amt > 0) { ?>
									<tr>
										...
										<td style="width:10%"class="alignRight"><?php echo moneyFormatIndia(number_format($imps_amt, 2, '.', '')); ?></td>
										<td style="width:66%"></td>
								...
								<?php if ($upi_amt > 0) { ?>
									<tr>
										...
										<td style="width:10%"class="alignRight"><?php echo moneyFormatIndia(number_format($upi_amt, 2, '.', '')); ?></td>
										<td style="width:66%"></td>
								...
								<?php if (sizeof($billing['adjusted_in_sales']) > 0) {
									...
									foreach ($billing['adjusted_in_sales'] as $adjusted_amt) {
										...
										if ($adjusted_amt['deposit_bill_id'] != '') { ?>
											...
											<tr>
												<td style="width:24%"><?php echo $adjusted_amt['deposit_type'] == 1 ? ($adjusted_amt['metal'] != '' ? 'BY CUST OWN ' . $adjusted_amt['metal'] . ' EX' : 'BY CUST OWN EX') : ($adjusted_amt['deposit_type'] == 2 ? 'Credit Note ' : 'ADV ADJ')  ?></td>
												<td style="width:10%"class="alignRight"><?php echo moneyFormatIndia(number_format($adjusted_amt['utilized_amt'], 2, '.', '')); ?></td>
												<td style="width:66%"><?php echo  $adjusted_amt['deposit_type'] == 1 ? ($adjusted_amt['adjusted_bill_no'] != '' ? '&nbsp;&nbsp; Ref Pur No : ' . $adjusted_amt['adjusted_bill_no'] : '') : ($adjusted_amt['deposit_type'] == 2 ? ($adjusted_amt['adjusted_bill_no'] != '' ? '&nbsp;&nbsp; Ref  No : ' . $adjusted_amt['adjusted_bill_no'] : '') : '') ?>;</td>
