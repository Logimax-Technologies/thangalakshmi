...
$(document).ready(function() {
				...
				switch(ctrl_page[2]){				 	
					...
					break;
				 	...
				 	case 'add':
                        ...
                        $("#date_of_wed").datepicker({format: 'yyyy-mm-dd'});
					...
					else if ($('#allow_bill_type').val() == 1) //Only Normal Bills
	...
	$('#pay_submit').on('click',function(){
...

	    {
        		...
        		var is_store_delivery=$("input[name='billing[delivered_at]']:checked").val();

        		var bill_type = $("input[name='billing[bill_type]']:checked").val();

        		var is_credit=$("input[name='billing[is_credit]']:checked").val();

        		var is_aganist_order    = $("input[name='billing[is_aganist_order]']:checked").val();
...

        		{
        		    ...
        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Select The Bill Type.."});
        		...
        		else if(bill_type==5  && ($('#billing_order_adv_details >tbody>tr').length==0))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Order Details Not Found.."});

        		    allow_submit=false;

        		}

        		else if((bill_type == 1 || bill_type==15) && ($('#billing_sale_details >tbody>tr').length==0))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Sales Details Not Found.."});

        		    allow_submit=false;

        		}

        		else if((bill_type==2) && ($('#billing_sale_details >tbody>tr').length==0 || $('#purchase_item_details >tbody>tr').length==0))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Sales Or Old Metal Items Details Not Found.."});

        		    allow_submit=false;

        		}

        		else if((bill_type==3) && ($('#billing_sale_details >tbody>tr').length==0 || $('#sale_return_details >tbody>tr').length==0 ))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Sales Or Sales Return Items Details Not Found.."});

        		    allow_submit=false;

        		}

        		else if((bill_type==4) && ($('#purchase_item_details >tbody>tr').length==0 ))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Old Metal Items Details Not Found.."});

        		    allow_submit=false;

        		}

        		else if((bill_type==6) && ($('#sale_return_details >tbody>tr').length==0 ))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Sales Return Items Details Not Found.."});
        		...
        		else if(bill_type==7  && ($('#sale_return_details >tbody>tr').length==0 ))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Return Details Not Found.."});
...

        		{
        		    ...
        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Select Bill No.."});

        		    allow_submit=false;

        		}

        		else if((bill_type==9) && ($('#billing_sale_details >tbody>tr').length==0))
        		...
        		else if((bill_type==11) && ($('#billing_repair_order_details >tbody>tr').length==0) && (is_aganist_order == 1))

        		{

        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Repair Item Details Not Found.."});

        		    allow_submit=false;

        		}

        		else if((bill_type==11) && ($('#billing_repair_order_other_details >tbody>tr').length==0) && (is_aganist_order == 2))
...

        		{
        		    ...
        		    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Select Credit Due Date.."});

        		    allow_submit=false;

        		}

        		else if(($('#billing_sale_details >tbody>tr').length>0) && bill_type!=11)

        		{

        		    if(($('#cus_del_state').val()=='' || $('#cus_del_country').val()=='') && (bill_type!=12))

            	    {

            	        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Check The Delivery Address.."});
            	    ...
            	    if(($('.sales_sgst').html()==0 && $('.sales_cgst').html()==0) && ($('.sales_igst').html()==0 && $('#is_eda').val()==1))

        		    {

        		        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>GST Not Calculated..Please Check The Delivery Address.."});
        		...
        		else if(($('#billing_sale_details >tbody>tr').length>0))

        		{
        		    ...
        		    $('#billing_sale_details > tbody tr').each(function(idx, row){
        		        ...
        		        if((curRow.find('.bill_net_val').val()!=curRow.find('.bill_tag_net_wt_val').val()) && (curRow.find('.is_partial').val()==0))

        		        {

        		            $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Tag Net Weight and Sale weight is difference.."});
...

        		{
        			...
        			if(is_credit==1 && credit_Otp_approval==1)
        		...
        		if($('#billing_emp_select_req').val()==1 && ($('#emp_select').val()=='' || $('#emp_select').val()==null)){
        		        ...
        		        success:function(data){
...

        					{
...

        					    {
        					        ...
        					        window.open( base_url+'index.php/admin_ret_billing/repair_order_thermal_print/'+data['id'],'_blank');
        						...
        						window.location.href= base_url+'index.php/admin_ret_billing/billing/add';
	...
	$("input[name='billing[bill_type]']:radio").on('change',function(){
        ...
        $(".purchase_details.order_adv_details,.sale_details,.return_details,.custom_details,.advance_amt,.total_summary_details,.summary_adv_details,.repair_order_details").hide();  

		

        if(this.value == 1 || this.value == 15) { // Sales
        ...
        else if(this.value == 11) // repair order
        ...
        else if(this.value==12) // SUPPLIER SALES BILL
	...
	$("#search_est_no").on('click', function(){
		...
		if($('#filter_est_no').val() != "" && $('#id_branch').val() != "" && bill_type != ""){
			...
			if($('#id_branch').val() == ""){
				...
				$.toaster({ priority : 'danger', title : 'Warning!', settings : {'timeout': 4000}, message : ''+"</br>"+'Select Branch'});

				$('#id_branch').focus();

			}else if($('#filter_est_no').val() == ""){

			    $.toaster({ priority : 'danger', title : 'Warning!', settings : {'timeout': 4000}, message : ''+"</br>"+'Enter Estimation No'});
					...
					if(!rowExist){
						...
						if(est_sale_row.find('.select_est_partial').is(':checked')){
							...
							var row = '<tr id="'+tot_length+'">'
							    ...
							    +'<td><span class="est_wastage_wt">'+est_sale_row.find('.est_wastage_wt').html()+'</span><input type="hidden" class="bill_wastage_wt" value="'+est_sale_row.find('.est_wastage_wt').html()+'"><input type="hidden" name="sale[wastage_discount][]" class="wastage_discount" value=""><input type="hidden" name="sale[mc_discount][]" class="mc_discount" value=""><input type="hidden" name="sale[item_blc_discount][]" class="item_blc_discount" value=""></td>'
...
function getEstimationDetails(estId, orderNo, billType){
    ...
    var metal_type = ($('#isMetal').val()==1 ? $('#select_metal_type').val() : '');
	...
	$.ajax({
        ...
        data: {'estId' : $('#filter_est_no').val(), 'order_no' : $('#filter_order_no').val(), 'billType' : billType, 'id_branch' : $("#id_branch").val(),'fin_year':$('#order_fin_year_select').val(),'metal_type':metal_type,'tag_code':$('#filter_tag_no').val()}, //Need to update login branch id here from session

        success: function (data) { 

			if(data.success == true){

				$("#validate_max_cash").val(data.responsedata.other_details.max_cash.validate_cash_amt);

				$("#max_cash_amt").val(data.responsedata.other_details.max_cash.max_cash_amt);

				var mc_edit_in_bill = data.responsedata.other_details.mc_edit_in_bill;
				
				var wastage_edit_in_bill = data.responsedata.other_details.wastage_edit_in_bill;
				...
				$.each(chit_details,function(key,item){
					...
					total_chit_wt +=(item.scheme_type==2 || item.scheme_type==3 ? parseFloat(item.closing_balance) :0);
				...
				if(data.responsedata.item_details.length > 0){
                   ...
                   total_est_discount = data.responsedata.item_details[0].est_discount;
                    ...
                    if(data.responsedata.item_details[0].id_customer!='' && data.responsedata.item_details[0].id_customer!=undefined)
					...
					var tag_other_itm_amount=0;
...

						{
							...
							$.each(estval.stone_details,function(key,item){
								...
								stone_details.push({'stone_id' : item.stone_id,'uom_id':item.uom_id,'stone_pcs' :item.pieces,'stone_wt':item.wt,'stone_price':item.amount,'certification_cost':item.certification_cost,'stone_name':item.stone_name,'is_apply_in_lwt':item.is_apply_in_lwt,'stone_cal_type':item.stone_cal_type,'rate_per_gram':item.rate_per_gram});
...

					{
...

					    {
					        ...
					        if(parseFloat(estval.net_wt)!=parseFloat(estval.tag_net_wt))

					        {

					            rowExist = true; 

					            $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br> Tag Net Weight and Estimation Net weight is difference.."});
					...
					if(estval.is_non_tag==1)
...

					    {
					        ...
					        let va_wt_limit = typeof estval.mc_va_limit != 'undefined' && estval.mc_va_limit != null && estval.mc_va_limit.length != 0 ? (!(estval.mc_va_limit.wastag_min >= 0) || estval.mc_va_limit.wastag_min == null ? 0 : parseFloat(parseFloat(estval.net_wt * estval.mc_va_limit.wastag_min/100)).toFixed(3)) : 0;	

							let mc_limit = typeof estval.mc_va_limit != 'undefined' && estval.mc_va_limit != null && estval.mc_va_limit.length != 0 ? (!(estval.mc_va_limit.mc_min >= 0) || estval.mc_va_limit.mc_min == null ? 0 : estval.mc_va_limit.mc_min)  : 0;

							let va_limit = typeof estval.mc_va_limit != 'undefined' && estval.mc_va_limit != null && estval.mc_va_limit.length != 0 ? (!(estval.mc_va_limit.wastag_min >= 0) || estval.mc_va_limit.wastag_min == null ? 0 : estval.mc_va_limit.wastag_min) : 0;	

							let margin_mrp = typeof estval.mc_va_limit != 'undefined' && estval.mc_va_limit != null && estval.mc_va_limit.length != 0 ? (!(estval.mc_va_limit.margin_mrp >= 0) || estval.mc_va_limit.margin_mrp == null ? 0 : estval.mc_va_limit.margin_mrp) : 0;							

							let purchase_cost = typeof estval.esti_purchase_cost != 'undefined' && estval.esti_purchase_cost != null ? estval.esti_purchase_cost : 0

					        var row = '<tr id="'+estkey+'">'

                            +'<td><span>'+estval.hsn_code+'</span><input type="hidden" name="sale[id_collecion_maping_det][]" class="id_collecion_maping_det" value="'+estval.id_collecion_maping_det+'"><input type="hidden" name="sale[order_no][]" class="order_no" value="'+estval.order_no+'"><input type="hidden" name="sale[id_orderdetails][]" class="id_orderdetails" value="'+estval.id_orderdetails+'"><input type="hidden" name="sale[id_customerorder][]" class="id_customerorder" value="'+estval.id_customerorder+'"><input type="hidden" class="sale_pro_hsn" name="sale[hsn]" value="'+estval.hsn_code+'" /><input type="hidden" class="sale_type" name="sale[sourcetype][]" value="1" /><input type="hidden" class="sale_item_type" name="sale[itemtype][]" value="'+estval.item_type+'" /><input type="hidden" class="is_est_details" value="1" name="sale[is_est_details][]" /><input type="hidden" class="est_itm_id" name="sale[est_itm_id][]" value="'+estval.est_item_id+'" /><input type="hidden" class="esti_no" name="sale[esti_no][]" value="'+estval.esti_no+'" /><input type="hidden" class="sale_cal_type" name="sale[calltype][]" value="'+estval.calculation_based_on+'" /><input type="hidden" class="sale_metal_type" value="'+estval.metal_type+'" /><input type="hidden" class="sale_purity" value="'+estval.purid+'"  name="sale[purity][]" /><input type="hidden" class="sale_size" value="'+estval.size+'"  name="sale[size][]" /><input type="hidden" class="sale_uom" value="'+estval.uom+'"  name="sale[uom][]" /><input type="hidden" class="total_tax" name="sale[item_total_tax][]"><input type="hidden" class="is_partial" value="'+estval.is_partial+'"  name="sale[is_partial][]" /><input type="hidden" class="min_wastage" value="'+estval.min_wastage+'"  name="sale[min_wastage][]" /><input type="hidden" class="max_wastage" value="'+estval.max_wastage+'"  name="sale[max_wastage][]" /><input type="hidden" class="stock_type" name="sale[stock_type][]" value="'+estval.stock_type+'" /><input type="hidden" class="is_non_tag" name="sale[is_non_tag][]" value="'+estval.is_non_tag+'" /><input type="hidden" class="rate_field"  value="'+estval.rate_field+'" /><input type="hidden" class="mc_limit"  value="'+mc_limit+'" /><input type="hidden" class="va_limit"  value="'+va_limit+'" /><input type="hidden" class="margin_mrp"  value="'+margin_mrp+'" /><input type="hidden" class="purchase_cost" value="'+purchase_cost+'" /></td>'

                            +'<td><span class="product_name">'+estval.product_name+'</span><input class="sale_product_id" type="hidden" name="sale[product][]" value="'+estval.product_id+'" /><input class="scheme_closure_benefit" type="hidden" name="sale[scheme_closure_benefit][]" value="'+estval.scheme_closure_benefit+'" /></td>'

                            +'<td><span class="design_name">'+estval.design_name+'</span><input type="hidden" class="sale_design_id" name="sale[design][]" value="'+estval.design_id+'" /><input type="hidden" class="id_sub_design" name="sale[id_sub_design][]" value="'+estval.id_sub_design+'" /></td>'
                            ...
                            +'<td><span class="bill_sale_net_wt"></span><input type="hidden" class="bill_net_val" name="sale[net][]" value="'+estval.net_wt+'" /><input type="hidden" class="bill_tag_net_wt_val"  value="'+estval.tag_net_wt+'" /></td>'

                            +'<td><span class="est_wastage">'+estval.wastage_percent+'</span><input type="'+(wastage_edit_in_bill == 1 ? "text" : "hidden")+'"  class="bill_wastage" value="'+estval.wastage_percent+'" /><input type="hidden" class="bill_wastage_per" name="sale[wastage][]" value="'+estval.wastage_percent+'" /></td>'

                            +'<td><span class="est_wastage_wt"></span><input type="'+(wastage_edit_in_bill == 1 ? "text" : "hidden")+'"  class="bill_wastage_wt" value=""><input type="hidden" name="sale[wastage_discount][]" class="wastage_discount" value=""><input type="hidden" name="sale[mc_discount][]" class="mc_discount" value=""><input type="hidden" name="sale[item_blc_discount][]" class="item_blc_discount" value=""></td>'

                            +'<td><select class="bill_mctype"><option value="1" '+(estval.mc_type==1 ? 'selected':'')+'>Pcs</option><option value="2" '+(estval.mc_type==2 ? 'selected':'')+'>Gram</option></select></td>'

                            +'<td><span class="making_charge">'+estval.mc_value+'</span><input type="hidden" class="mc_dis" value="0"><input type="'+(mc_edit_in_bill == 1 ? "text" : "hidden")+'"  class="bill_mc" name="sale[mc][]" value="'+estval.mc_value+'" /><input type="hidden" class="discount"  name="sale[adjusted_dis][]" value="" /><input type="hidden" class="bill_mc_value" value="'+estval.mc_value+'" /></td>'

                            +'<td><input type="number" class="bill_discount" name="sale[discount][]"  step="any" readonly/></td>'

                            +'<td><span class="bill_taxable_amt"></span></td>'

                            +'<td><span class="tgrp_name">'+estval.tgrp_name+'</span><input type="hidden" class="sale_tax_group" name="sale[taxgroup][]" value="'+estval.tax_group_id+'" /></td>'
                            ...
                            +'<td><input type="hidden" class="bill_stone_price" value="'+stone_price+'" /><input type="number" class="bill_amount" name="sale[billamount][]" value="" step="any" readonly /><input type="hidden" class="per_grm_amount" name="sale[per_grm][]" value="'+estval.est_rate_per_grm+'" step="any" /></td>'

                            +'<td>'+(estval.is_partial==1 ?'YES' :'NO')+'</td>'

                            +'<td><span class="tag_code">'+estval.tag_code+'</span><input type="hidden" class="sale_tag_id" name="sale[tag][]" value="'+estval.tag_id+'" /><input type="hidden" name="sale[total_cgst][]" class="sale_cgst"/><input type="hidden" class="sale_sgst" name="sale[total_sgst][]"/><input type="hidden" class="sale_igst" name="sale[total_igst][]"/></td>'

                            +'<td>'+estval.order_no+'</td>'

                            +'<td><span class="esti_number">'+estval.esti_no+'</span><input type="hidden" class="sale_est_itm_id" name="sale[estid][]" value="'+estval.est_item_id+'" /><input type="hidden" class="tag_sales_value"  value="'+estval.item_cost+'" /><input type="hidden" class="gift_applicable"  value="'+estval.gift_applicable+'" /></td>'
				...
				else if(billType==11 && (data.responsedata.order_details).length>0)
...
$('#disc_apply').on('click',function(){
	...
	var otp_dis_approval   = $('#otp_dis_approval').val();
	...
	//if($('#summary_discount_amt').val()>parseFloat(emp_disc_limit) || disc_status.status == false)

	if(parseFloat($('#summary_discount_amt').val())>parseFloat(emp_disc_limit))

	{

		// 1 -> Both employee discount and MC, VA limit discount failed in validation. 2 -> Employee discount failed, 3-> MC,VA discount failed

		//let discount_on = $('#summary_discount_amt').val() > parseFloat(emp_disc_limit) && disc_status.status == false ? 1 :  ( $('#summary_discount_amt').val() > parseFloat(emp_disc_limit) && disc_status.status == true ? 2 : 3);

		//if(otp_dis_approval==1 && discount_on == 2) // No Need approval for Giving Discount

		if(otp_dis_approval==1) // No Need approval for Giving Discount
...

		{
			...
			let show_disc_amt = parseFloat(emp_disc_limit) <= parseFloat(disc_status.max_discount_limit) ? emp_disc_limit : disc_status.max_discount_limit;

			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br> Maximum Discount Limit is Rs."+show_disc_amt});	
...

{
...

	{
		...
		disc_status = discount_validation_for_salebill();	
	...
	if(disc_status.status == true) {
	 ...
	 var max_discount_amt   =0;
		...
		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br> Maximum Discount Limit is Rs."+disc_status.max_discount_limit});	
...

{
	...
	$('#billing_sale_details > tbody tr').each(function(idx, row){
		...
		var pcs = (isNaN(curRow.find('.sale_pcs').val()) || curRow.find('.sale_pcs').val() == '')  ? 0 : curRow.find('.sale_pcs').val();
	    ...
	    if(isNaN(parseFloat(curRow.find('.per_grm_amount').val())) || curRow.find('.per_grm_amount').val() == 0 || curRow.find('.per_grm_amount').val() == ''){

		    var rate_per_grm = (isNaN($('#'+rate_field).val()) ||$('#'+rate_field).val() == '')  ? 0 : parseFloat($('#'+rate_field).val());

		}else{

		    var rate_per_grm = (isNaN(parseFloat(curRow.find('.per_grm_amount').val())) || curRow.find('.per_grm_amount').val() == '')  ? 0 : parseFloat(curRow.find('.per_grm_amount').val());
		...
		if(calculation_type == 0){ 
			...
			wast_wgt_amt      = parseFloat(parseFloat(wast_wgt)*parseFloat(rate_per_grm)).toFixed(2);

			if(curRow.find('.bill_mctype').val() != 3){

    			var mc_type       =  parseFloat(curRow.find('.bill_mctype').val() == 2 ? parseFloat(retail_max_mc * gross_wt ) : parseFloat(retail_max_mc * 1));
			...
			if(curRow.find('.bill_mctype').val() != 3){
    			...
    			var mc_type       =  parseFloat(curRow.find('.bill_mctype').val() == 2 ? parseFloat(retail_max_mc * net_wt ) : parseFloat(retail_max_mc * 1));
    	    ...
    	    rate_with_mc  = parseFloat((isNaN(curRow.find('.tag_sales_value').val()) || curRow.find('.tag_sales_value').val() == '')  ? 0 : curRow.find('.tag_sales_value').val()* gross_wt); 

    		tag_sales_value  = parseFloat((isNaN(curRow.find('.tag_sales_value').val()) || curRow.find('.tag_sales_value').val() == '')  ? 0 : curRow.find('.tag_sales_value').val()* gross_wt); 

    	}

		let charge_value = isNaN(parseFloat(curRow.find('.charge_value').val())) ? 0 : curRow.find('.charge_value').val();

		rate_with_mc = parseFloat(parseFloat(rate_with_mc) + parseFloat(charge_value)+parseFloat(tag_other_itm_amount)).toFixed(2);
...

                {
                    ...
                    */
...

        	        {
        	            ...
        	            blc_discount = parseFloat(parseFloat(discount) - parseFloat(mc_type)).toFixed(2);
...

        	            {
...

        	                {
...

        	                    {
        	                        ...
        	                        wastage_discount = parseFloat(parseFloat(wast_wgt_amt)).toFixed(2);
        	                ...
        	                var total_applied_discount = parseFloat(parseFloat(mc_discount)+parseFloat(wastage_discount)).toFixed(2);

        	                if(total_applied_discount < discount)

        	                {

        	                    item_blc_discount  = parseFloat(parseFloat(discount)-parseFloat(total_applied_discount)).toFixed(2);
...

        	        {
        	            ...
        	            discount_weight = parseFloat(parseFloat(wastage_discount)/parseFloat(rate_per_grm)).toFixed(3);
...

	    {
    		...
    		var base_value_tax=parseFloat(calculate_base_value_tax(rate_with_mc,tax_group));

    		var base_value_amt=parseFloat(parseFloat(rate_with_mc)+parseFloat(base_value_tax));

    		var arrived_value_tax=parseFloat(calculate_arrived_value_tax(base_value_amt,tax_group));

    		var arrived_value_amt=parseFloat(parseFloat(base_value_amt)+parseFloat(arrived_value_tax));

    		

    		var total_tax_rate=parseFloat(parseFloat(base_value_tax)+parseFloat(arrived_value_tax));
...

            {
                ...
                rate_with_mc = parseFloat(parseFloat(rate_with_mc)-parseFloat(total_tax_rate)).toFixed(2);

            }

    		

    		/*if(is_store_delivery==1) // SHOW ROOM DELIVERY
    		...
    		/*if(is_store_delivery==2) // Customer Delivery
	   ...
	   curRow.find('.mc_discount').val(parseFloat(mc_discount).toFixed(2));
    	...
    	curRow.find('.wastage_discount').val(parseFloat(wastage_discount).toFixed(2));

    	curRow.find('.item_blc_discount').val(parseFloat(item_blc_discount).toFixed(2));
		...
		curRow.find('.est_wastage_wt').html(parseFloat(parseFloat(wast_wgt)-parseFloat(discount_weight)).toFixed(3));

		curRow.find('.making_charge').html(parseFloat(mc_type-mc_discount).toFixed(2));
...

{
	...
	let wastage_min = isNaN(parseFloat($(this).closest('tr').find(".va_limit").val())) ? 0 : parseFloat($(this).closest('tr').find(".va_limit").val());

	if(wastage < wastage_min)

	{

		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Minimum wastage Percentage is "+wastage_min+"%",settings : {'timeout': 5000}});
...

{
	...
	let wastage_wt_min = isNaN(parseFloat($(this).closest('tr').find(".va_wt_limit").val())) ? 0 : parseFloat($(this).closest('tr').find(".va_wt_limit").val());
...

	{
		...
		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Minimum wastage Wt is "+wastage_wt_min,settings : {'timeout': 5000}});
...

	{
		...
		let wast_per = parseFloat(parseFloat(wastage_wt * 100) / parseFloat($(this).closest('tr').find('.bill_net_val').val())).toFixed(2);
...

{
	...
	let mc_min = isNaN(parseFloat($(this).closest('tr').find(".mc_limit").val())) ? 0 : parseFloat($(this).closest('tr').find(".mc_limit").val());

	if(mc < mc_min)

	{

		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Minimum MC is "+mc_min,settings : {'timeout': 5000}});
...

{
	...
	$('#billing_sale_details > tbody tr').each(function(idx, row){
		...
		if(isNaN(parseFloat(curRow.find('.per_grm_amount').val())) || curRow.find('.per_grm_amount').val() == 0 || curRow.find('.per_grm_amount').val() == ''){
		    ...
		    rate_per_grm = (isNaN(parseFloat(curRow.find('.per_grm_amount').val())) || curRow.find('.per_grm_amount').val() == '')  ? 0 : parseFloat(curRow.find('.per_grm_amount').val());
...
function getSearchCustomers(searchTxt){
	...
	$.ajax({
        ...
        data: {'searchTxt': searchTxt,'billing_for':billing_for,'esti_for':(billing_for==1 ? 1 :3)}, 
			...
			if(data.length == 0){
				...
				if(!rowExist){
					...
					var row = '<tr id="'+tot_length+'">'
								...
								+'<td><input type="number" class="bill_amount" name="sale[billamount][]" value="" step="any" readonly /><input type="hidden" class="per_grm_amount" name="sale[per_grm][]" value="'+data[0].est_rate_per_grm+'" step="any" /><input type="hidden" class="tag_sales_value" value="'+data[0].sales_value+'" step="any" /></td>'
...
function calculate_sales_details(){
	...
	$('#billing_sale_details > tbody  > tr').each(function(index, tr) {
		...
		if($(this).find('.bill_gross_val').val() != "" && $(this).find('.gwt').html() != "" && $(this).find('.sales_value').val() != ""){
			...
			total_sale_pcs += parseFloat((isNaN($(this).find('.sale_pcs').val()) || $(this).find('.sale_pcs').val() == '')  ? 0 : $(this).find('.sale_pcs').val());

			total_sale_gwt += parseFloat((isNaN($(this).find('.bill_gross_val').val()) || $(this).find('.bill_gross_val').val() == '')  ? 0 : $(this).find('.bill_gross_val').val());

			total_sale_lwt += parseFloat((isNaN($(this).find('.bill_less_val').val()) || $(this).find('.bill_less_val').val() == '')  ? 0 : $(this).find('.bill_less_val').val());

			total_sale_nwt += parseFloat((isNaN($(this).find('.bill_net_val').val()) || $(this).find('.bill_net_val').val() == '')  ? 0 : $(this).find('.bill_net_val').val());

			total_sale_amt += parseFloat((isNaN($(this).find('.bill_amount').val()) || $(this).find('.bill_amount').val() == '')  ? 0 : $(this).find('.bill_amount').val());
	...
	$('#return_item_detail > tbody  > tr').each(function(index, tr) {
	    ...
	    sales_amt += parseFloat((isNaN($(this).find('.returnitemcost').val()) || $(this).find('.returnitemcost').val() == '')  ? 0 : $(this).find('.returnitemcost').val());

	    cgst += parseFloat((isNaN($(this).find('.return_item_tax_cgst_value').val()) || $(this).find('.return_item_tax_cgst_value').val() == '')  ? 0 : $(this).find('.return_item_tax_cgst_value').val());

		sgst += parseFloat((isNaN($(this).find('.return_item_tax_sgst_value').val()) || $(this).find('.return_item_tax_sgst_value').val() == '')  ? 0 : $(this).find('.return_item_tax_sgst_value').val());

		igst += parseFloat((isNaN($(this).find('.return_item_tax_igst_value').val()) || $(this).find('.return_item_tax_igst_value').val() == '')  ? 0 : $(this).find('.return_item_tax_igst_value').val());

	});

	

	$('#billing_supplier_sales_bill_details > tbody  > tr').each(function(index, tr) {
	 ...
	 item_cost=parseFloat((isNaN($(this).find('.item_cost').val()) || $(this).find('.item_cost').val() == '')  ? 0 : $(this).find('.item_cost').val());
		...
		if($(this).find('.weight').val() != "" && $(this).find('.pur_weight').val() != "" && $(this).find('.item_cost').val() != "")

		{

			sales_weight += parseFloat((isNaN($(this).find('.pur_weight').val()) || $(this).find('.pur_weight').val() == '')  ? 0 : $(this).find('.pur_weight').val());

			item_taxable_amt += parseFloat((isNaN($(this).find('.item_cost').val()) || $(this).find('.item_cost').val() == '')  ? 0 : $(this).find('.item_cost').val());

			item_tax_cost += parseFloat((isNaN($(this).find('.item_total_tax').val()) || $(this).find('.item_total_tax').val() == '')  ? 0 : $(this).find('.item_total_tax').val());

			sales_amt+=parseFloat($(this).find('.item_cost').val()-$(this).find('.item_total_tax').val());

			cgst += parseFloat((isNaN($(this).find('.item_total_cgst').val()) || $(this).find('.item_total_cgst').val() == '')  ? 0 : $(this).find('.item_total_cgst').val());

			sgst += parseFloat((isNaN($(this).find('.item_total_sgst').val()) || $(this).find('.item_total_sgst').val() == '')  ? 0 : $(this).find('.item_total_sgst').val());

			igst += parseFloat((isNaN($(this).find('.item_total_igst').val()) || $(this).find('.item_total_igst').val() == '')  ? 0 : $(this).find('.item_total_igst').val());
	...
	$('#billing_repair_order_other_details > tbody  > tr').each(function(index, tr) 

	{
		...
		if($(this).find('.rapair_amount').val() != ""){
			...
			amount          = parseFloat((isNaN(curRow.find('.rapair_amount').val()) || curRow.find('.rapair_amount').val() == '')  ? 0 : curRow.find('.rapair_amount').val());
			...
			tot_repair_amount+=parseFloat(parseFloat(amount)+parseFloat(total_other_item_amount));

			curRow.find('.item_total_amount').html(parseFloat(parseFloat(amount)+parseFloat(total_other_item_amount)).toFixed(2));
...
function calculateFinalCost(){
	...
	repair_charges		= parseFloat((isNaN($('.summary_repair_amt').html()) || $('.summary_repair_amt').html() == '')  ? 0 : $('.summary_repair_amt').html()).toFixed(2);
	...
	total_chit_amt		= parseFloat((isNaN($('.summary_chit_paid_amt').html()) || $('.summary_chit_paid_amt').html() == '')  ? 0 : $('.summary_chit_paid_amt').html()).toFixed(2);

	sale_return			= parseFloat((isNaN($('.summary_sale_ret_amt').html()) || $('.summary_sale_ret_amt').html() == '')  ? 0 : $('.summary_sale_ret_amt').html()).toFixed(2);

	taxable_amount		= parseFloat((isNaN($('.summary_sale_amt').html()) || $('.summary_sale_amt').html() == '')  ? 0 : $('.summary_sale_amt').html()).toFixed(2);

	var tds_percent    = ($('.tds_percent ').val()!='' ? $('.tds_percent ').val():0);

	var tcs_percent    = ($('.tcs_percent ').val()!='' ? $('.tcs_percent ').val():0);
...

	{
	    ...
	    tds_tax_value = parseFloat(parseFloat(parseFloat(taxable_amount)*parseFloat(tds_percent))/100).toFixed(2);
	...
	tot_cost 		     = parseFloat(parseFloat(sales_amt)+parseFloat(repair_charges)+parseFloat(tcs_tax_amt)+parseFloat(credit_amt)-parseFloat(purchase_amt)-parseFloat(adv_paid_amt)-parseFloat(total_chit_amt)-parseFloat(sale_return)+parseFloat(handling_charges)-parseFloat(credit_discount)-parseFloat(tds_tax_value)).toFixed(2);
...

    {
        ...
        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>The Total Return Amount is"+$(".pay_to_cus").val()});
...
$('#make_pay_cash,.receive_amount,#advance_amount').on('keyup',function(){
...

{
	...
	var cc              =($('.CC').html()!='' && $('.CC').html()!=undefined ? $('.CC').html():0);

	var dc              =($('.DC').html()!='' && $('.DC').html()!=undefined ? $('.DC').html():0);

	var chq             =($('.CHQ').html()!='' && $('.CHQ').html()!=undefined ? $('.CHQ').html():0);

	var NB              =($('.NB').html()!='' && $('.NB').html()!=undefined ? $('.NB').html():0);
	...
	var make_as_advance = $("input[name='billing[make_as_advance]']:checked").val();

	var sales_amount    = $('.sale_amt_with_tax').html();

	var purchase_amt    = $('.summary_pur_amt').html();

	var benifits        = ($('.benifit_amount').val()!='' ? $('.benifit_amount').val():0);

	var sale_return		= parseFloat((isNaN($('.summary_sale_ret_amt').html()) || $('.summary_sale_ret_amt').html() == '')  ? 0 : $('.summary_sale_ret_amt').html()).toFixed(2);
...

	{
...

    	{
    		...
    		total_amount=parseFloat(parseFloat(make_pay_cash)+parseFloat(tot_chit_amt)+parseFloat(tot_voucher_amt)+parseFloat(cc)+parseFloat(dc)+parseFloat(tot_adv_adj)+parseFloat(chq)+parseFloat(NB)+parseFloat(advance_amount)).toFixed(2);
...

    	{
    		...
    		if(benifits > receive_amount){
    	        ...
    	        if((make_pay_cash>0 || NB>0 || chq>0 || cc>0 || dc>0)){
    	            ...
    	            total_amount = parseFloat(parseFloat(make_pay_cash)+parseFloat(NB)+parseFloat(chq)+parseFloat(advance_amount)).toFixed(2);
...

    	{
    		...
    		total_amount=parseFloat(parseFloat(make_pay_cash)+parseFloat(tot_chit_amt)+parseFloat(tot_voucher_amt)+parseFloat(cc)+parseFloat(dc)+parseFloat(chq)+parseFloat(tot_adv_adj)+parseFloat(advance_amount)).toFixed(2);
...

    	{
    		...
    		if((make_pay_cash<10000 && allowsubmit) || ($('#is_eda').val()==2))
...

    {
        ...
        total_amount=parseFloat(parseFloat(make_pay_cash)+parseFloat(NB)+parseFloat($('#advance_amount').val())).toFixed(2);

        

        

        	if(make_pay_cash<10000 || ($('#is_eda').val()==2))
    ...
    if(((parseFloat(sales_amount))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#is_eda').val()==1))

	{

        if($('#pan_no').val()=='' || $('#pan_no').val()==0)
	...
	if(((parseFloat(purchase_amt))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#is_eda').val()==1))
	...
	if(((parseFloat(sale_return))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#is_eda').val()==1))
	...
	if(((parseFloat(order_advance_recd_amt))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#is_eda').val()==1))
...

{
	...
	$.ajax({
		 ...
		 url:base_url+"index.php/admin_ret_billing/billing/ajaxapprovallist?nocache=" + my_Date.getUTCSeconds(),

		 dataType:"JSON",

		 data:{'dt_range' :$("#dt_range").val(),'bill_no':$('#filter_bill_no').val(),'id_branch':($('#branch_filter').val()!='' && $('#branch_filter').val()!=undefined ? $('#branch_filter').val(): $("#branch_select").val()),'order_status':$('#order_status').val()},
...

{
...

    {
        ...
        if(row.billing_type==11)

        {

            print_url=base_url+'index.php/admin_ret_billing/repair_order_thermal_print/'+id;
        ...
        action_content='<a href="'+print_url+'" target="_blank" class="btn btn-info btn-print" data-toggle="tooltip" title="Billing Receipt"><i class="fa fa-print" ></i></a>'+(row.allow_cancel==1 && profile.allow_bill_cancel ==1 && access.edit=='1'?'<button class="btn btn-warning" onclick="confirm_delete('+id+','+profile.bill_cancel_otp+','+row.id_branch+')"><i class="fa fa-close" ></i></button>' :'');
            ...
            { "mDataProp": function ( row, type, val, meta ) {
...

                {
...

                     {
                         ...
                         return '<input type="checkbox" class="approval_tags"  name="approva[tag_id][]" value="'+row.tag_id+'" /><input type="hidden"  class="id_karigar"  value="'+row.id_karigar+'" />' + row.id_issue_receipt;
...

{
	...
	$.each(data,function(key,old_est_sale_row){
            ...
            $('#purchase_item_details > tbody tr').each(function(bidx, brow){
                ...
                if(old_est_sale_row.old_metal_sale_id == bill_pur_row.find('.est_itm_id').val())
...

{
	...
	$.ajax({
        ...
        data: {'searchTxt': searchTxt,'id_customer':bill_cus_id,'id_scheme_account':id_scheme_acc}, 
...

		{
...

			{
				...
				let cash_pay = isNaN(data[0].cash_pay) && $.trim(data[0].cash_pay) == "" ? 0 : data[0].cash_pay;
				...
				curRow.find(".chit_cash_pay").val(cash_pay);
				...
				if(data[0].scheme_type!=0 &&(data[0].total_installments==data[0].paid_installments))
...
function getSearchAcc(searchTxt, curRow){
	...
	$.ajax({
        ...
        data: {'searchTxt': searchTxt,'id_customer':bill_cus_id}, 
				...
				{ 
                    ...
                    if (index === -1) {
                        ...
                        if(i.item.scheme_type != 0 && bill_type!=10){
                            ...
                            i.item.closing_amount = parseFloat(i.item.closing_balance * parseFloat($('#goldrate_22ct').val())).toFixed(2);
					...
					let cash_pay = isNaN(i.item.cash_pay) && $.trim(i.item.cash_pay) == "" ? 0 : i.item.cash_pay;
    ...
    $('#estimation_chit_details > tbody  > tr').each(function(index, tr) {
        ...
        $('#billing_sale_details > tbody tr').each(function(bidx, brow){ 
...

	        {
	            ...
	            sales_details.push({'wastage_per':tagcurRow.find('.bill_wastage_per').val(),'mc_value':(tagcurRow.find('.making_charge').html()),'item_gross_wt':tagcurRow.find('.bill_gross_val').val()});
...

		{
		    ...
		    {       
		            ...
		            console.log(sales_details);
		             ...
		             var amount = parseFloat(savings_in_wast_amt)+parseFloat(savings_in_mcvalue)+parseFloat(additional_benefits)-parseFloat(closing_add_chgs);
		...
		curRow.find('.chit_amount').html(parseFloat(chit_amount).toFixed(2));			

		curRow.find('.chit_amt').val(parseFloat(chit_amount).toFixed(2));			
...

{
    ...
    $('.summary_chit_paid_amt').html(parseFloat(Math.round(total_chit_amount)).toFixed(2))
            ...
            var closing_weight = 0;
                ...
                $.each(chit_details,function(key,items){
...

    				{
    		            ...
    		            closing_weight = parseFloat(chit_amount/$('#goldrate_22ct').val()).toFixed(2);
                ...
                +'<td><div><input class="scheme_account" type="number" value="'+items.scheme_account_id+'" style="width: 100px;height:35px;"/><span ><button type="button" id="scheme_search" class="btn btn-default btn-flat"><i class="fa fa-search"></i></button></span></div><input type="hidden" class="scheme_account_id" name="chit_uti[scheme_account_id][]" value="'+items.scheme_account_id+'" ><input type="hidden" name="chit_uti[scheme_type][]" class="form-control scheme_type" id="scheme_type"  value="'+items.scheme_type+'" ><input type="hidden" name="chit_uti[paid_installments][]" class="form-control paid_installments" value="'+items.paid_installments+'"><input type="hidden" name="chit_uti[total_installments][]" class="form-control total_installments" value="'+items.total_installments+'" ><input type="hidden" name="chit_uti[is_wast_and_mc_benefit_apply][]" class="form-control is_wast_and_mc_benefit_apply" value="'+items.is_wast_and_mc_benefit_apply+'" ></td>'

                +'<td><span class="chit_amount">'+items.utl_amount+'</span><input type="hidden" class="form-control closing_amount" value="'+items.closing_amount+'"><input type="hidden" class="chit_amt" name="chit_uti[chit_amt][]" value="'+items.utl_amount+'"><input type="hidden" class="chit_cash_pay" name="chit_uti[chit_cash_pay][]"><input type="hidden" name="chit_uti[wastage_per][]" class="form-control wastage_per"><input type="hidden" name="chit_uti[mc_value][]" class="form-control mc_value"><input type="hidden" name="chit_uti[savings_in_wastage][]" class="form-control savings_in_wastage"><input type="hidden" name="chit_uti[savings_in_mcvalue][]" class="form-control savings_in_mcvalue"><input type="hidden" name="chit_uti[closing_weight][]" class="form-control closing_weight" value="'+closing_weight+'"></td>'

                +'<td><span class="saved_weight">'+closing_weight+'</span></td>'
...
function validateCardDetailRow(){
	...
	$('#card_details > tbody  > tr').each(function(index, tr) {
		...
		if($(this).find('.card_name').val() == "" || $(this).find('.card_type').val() == "" || $(this).find('.card_no').val() == "" || $(this).find('.ref_no').val() == "" || $(this).find('.card_amt').val() == "" || $(this).find('.id_device').val() == ""){
...

{
	...
	$('#card-detail-modal .modal-body #card_details > tbody  > tr').each(function(index, tr) {
		...
		$('#cc_billed_cash').val(parseFloat(parseFloat(cc_amount)+parseFloat(dc_amount)).toFixed(2));
...

{
	...
	$('.cheque_deposit_date').datepicker({ dateFormat: 'yyyy-mm-dd', })
...
function validateNBDetailRow(){
	...
	$('#net_bank_details > tbody  > tr').each(function(index, tr) {
		...
		if($(this).find('.nb_type').val() == "" || $(this).find('.ref_no').val() == "" || $(this).find('.amount').val() == "" || $(this).find('.id_bank').val() == ""){
...

{
	...
	row += '<tr>'
			...
			+'<td><select name="nb_details[nb_type][]" class="form-control nb_type"><option value=1>RTGS</option><option value=2>IMPS</option><option value=3>UPI</option></select></td>'
			...
			+'<td><input type="number" step="any" class="form-control ref_no" name="nb_details[ref_no][]"/></td>'
...

{
...

    {
    	...
    	$.ajax({
            ...
            data: {'bill_cus_id':bill_cus_id,'is_eda':$('#is_eda').val()}, 
            ...
            success: function (data) {
...

                        {
                            ...
                            $.each(data,function(key,items){
                                ...
                                +'<td><input type="checkbox" class="bill_id_issue_receipt"  name="adv_adj[id_issue_receipt]" value="'+items.id_issue_receipt+'"><input type="hidden" class="id_ret_wallet" value="'+items.id_ret_wallet+'"><input type="hidden" class="adv_cash_paid" value="'+items.cash_pay+'"></td>'
                            ...
                            if(bill_type!=5){
                                $('.total_bill_amt').html(parseFloat($('.total_cost').val()).toFixed(2));
                            }else if(bill_type==5){
                                $('.total_bill_amt').html(parseFloat($('.adv_amt').val()).toFixed(2));
                            }
...
$(document).on('keyup',".refund_amount", function(e){
...

     {
          ...
          $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"The Entered Refund Amount Is more than the Balance Amount"});
...
$(document).on('keyup',".adj_amount", function(e){
    ...
    var total_bill_amt          = $('.total_bill_amt').html();
     ...
     $('#adv-adj-confirm-add .modal-body #bill_adv_adj > tbody  > tr').each(function(index, tr) {
...

        {
            ...
            total_adjusted_amt+=(isNaN(row.find('.adj_amount').val()) || (row.find('.adj_amount').val()=='') ? 0 :parseFloat(row.find('.adj_amount').val()));
...

    {
...

        {
             ...
             $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"The Entered Amount Is more than the bill value"});

        }

        else if(parseFloat(advance_amount)<parseFloat(row.find('.adj_amount').val()))

        {
            ...
            balance_amount = parseFloat(advance_amount)-parseFloat(row.find('.adj_amount').val());
...
$('#add_adv_adj').on('click',function(e){
...

	{
...

	    {
...

            {
                ...
                $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"The Selected Amount should be more than the bill value."});
        		...
        		if(max_cash_valid == true) {
        			...
        			$('#payment_modes > tbody >tr').each(function(bidx, brow){
        				...
        				bill_card_pay_row.find('#tot_adv_adj').html(parseFloat(adj_amount).toFixed(2));
...

	    {
	        ...
	        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"Please Enter The Adjusted Amount.."});
...

    {
        ...
        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"Please Select The Receipt Details"});
...
function getCreditBillDetails(billNo, billType){
	...
	$.ajax({
        ...
        data: {'billNo' : billNo, 'billType' : billType, 'id_branch' : $("#id_branch").val(),'fin_year':$('#bill_fin_year_select').val(),'is_eda':$('#is_eda').val()},
...
function getBillDetails(billNo, billType){
	...
	$.ajax({
        ...
        data: {'billNo' : billNo, 'billType' : billType, 'id_branch' : $("#id_branch").val(),'fin_year':$('#bill_fin_year_select').val(),'metal_type':metal_type,'is_eda':$('#is_eda').val()},
			...
			if(data.success == true){
				...
				if(data.responsedata.item_details.length > 0){
...

				    {
				        ...
				        $(".summary_credit_due_ret_amt").html(parseFloat(data.responsedata.due_amount).toFixed(2));

					    $("#summary_credit_due_amt").val(parseFloat(data.responsedata.due_amount).toFixed(2));

					    $("#return_paid_amount").val(parseFloat(data.responsedata.paid_amount).toFixed(2));
...

{
	...
	var credit_due_ret_amt      = parseFloat((isNaN($('.summary_credit_due_ret_amt').html()) || $('.summary_credit_due_ret_amt').html() == '')  ? 0 : $('.summary_credit_due_ret_amt').html()).toFixed(2);

	var summary_credit_due_amt  = ($('#summary_credit_due_amt').val()=='' ? 0:$('#summary_credit_due_amt').val());

	var return_paid_amount      = ($('#return_paid_amount').val()=='' ? 0:$('#return_paid_amount').val());
	...
	$(".summary_sale_ret_amt").html(parseFloat(parseFloat(saleRet_rate)).toFixed(2)); 
...
function create_new_empty_stone_item(stn_data=[]) {
	...
	$.each(uom_details, function (pkey, pitem) {
		...
		uom_list += "<option value='"+pitem.uom_id+"' "+(stn_data ? (pitem.uom_id == stn_data.uom_id ? 'selected' : '') : '')+">"+pitem.uom_name+"</option>";
	...
	var rate = (stn_data ? (stn_data.rate_per_gram == undefined ? 0:stn_data.rate_per_gram): 0);
	...
	row='<tr id="'+$('#estimation_stone_item_details tbody tr').length+'" class="st_'+$('#estimation_stone_item_details tbody tr').length+'">'

	+'<td><select '+(is_apply_in_lwt!='' ?'disabled !important' : '')+' class="is_apply_in_lwt form-control" name="est_stones_item[is_apply_in_lwt][]" style="width:100px;"><option value="">-Select-</option><option value=1 '+(is_apply_in_lwt==1 ? 'selected' :'')

	+'>Yes</option><option value=0 '+(is_apply_in_lwt==0 ? 'selected' :'')+'>No</option> </select></td>'

	+'<td><select '+(stones_list!='' ?'disabled !important' : '')+' class="stone_id form-control" name="est_stones_item[stone_id][]" style="width:100px;">'+stones_list+'</select><input type="hidden" class="stone_type" value=""></td>'

	+'<td><input '+(stone_pcs!='' ?'disabled !important' : '')+'type="number" class="stone_pcs form-control" name="est_stones_item[stone_pcs][]" value="'+stone_pcs+'" style="width: 60px;"/></td>'

	+'<td><div class="input-group test" style="width:159px;" ><input '+(stone_wt!='' ?'disabled !important' : '')+' class="stone_wt form-control" type="number" name="est_stones_item[stone_wt][]" value="'+stone_wt+'" style="width: 78px;"/><span class="input-group-btn" style="width: 138px;"><select  '+(stone_wt!='' ?'disabled !important' : '')+'  class="stone_uom_id form-control" name="est_stones_item[uom_id][]">'+uom_list+'</select></span></div></td>'

	+'<td ><div class="form-group" style="width: 100px;">	<input  '+(cal_type!='' ?'disabled' : '')+' class="stone_cal_type" type="radio" name="est_stones_item[cal_type]['+row_cls+']" value="1" '+(cal_type == 1 ? 'checked' : '')+'>Wt&nbsp;<input  '+(cal_type!='' ?'disabled' : '')+'  type="radio" name="est_stones_item[cal_type]['+row_cls+']" class="stone_cal_type" value="2" '+(cal_type == 2 ? 'checked' : '')+'>Pcs</div></td>'

	

	+'<td><input '+(rate!='' ?'disabled !important' : '')+'type="number" class="rate_per_gram form-control" name="est_stones_item[rate_per_gram][]" value="'+rate+'" /></td>'
...
$('#stoneModal  #update_stone_details').on('click', function(){
	...
	$('#stoneModal .modal-body #estimation_stone_item_details> tbody  > tr').each(function(index, tr) {
		...
		stone_details.push({'stone_id' : $(this).find('.stone_id').val(),'is_apply_in_lwt':$(this).find('.is_apply_in_lwt ').val(),'stone_pcs' :$(this).find('.stone_pcs').val(),'stone_wt':$(this).find('.stone_wt').val(),'uom_id':$(this).find('.stone_uom_id').val(),'stone_cal_type':$(this).find('input[type=radio]:checked').val(),'rate_per_gram':$(this).find('.rate_per_gram').val(),'stone_price':$(this).find('.stone_price').val()});
...
function validateStoneItemDetailRow() {
	...
	$('#stoneModal .modal-body #estimation_stone_item_details> tbody  > tr').each(function(index, tr) {
		...
		let rate_per_gram = $.isNumeric($.trim($(this).find('.rate_per_gram').val())) ? $.trim($(this).find('.rate_per_gram').val()) : 0;

		let stone_price = $.isNumeric($.trim($(this).find('.stone_price').val())) ? $.trim($(this).find('.stone_price').val()) : 0;
		...
		if(is_apply_in_lwt == "") {
			...
			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please select LWT for stone"});
			...
			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Stone required"});

		} else if (!(stone_pcs > 0)) {

			row_validate = false;

			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Stone pieces required"});
			...
			if (!(stone_uom_id > 0)) {
				...
				$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Unit for stone weight required"});
			...
			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Stone calc type required"});
			...
			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Stone price should be greater than zero"});
...
$('#cancel_payment').on('click',function(){
	...
	$.ajax({
		...
		url:base_url+ "index.php/admin_ret_billing/issue/cancel?nocache=" + my_Date.getUTCSeconds(),
		...
		success:function(data){
			...
			$.toaster({ priority : 'success', title : 'Success!', message : ''+"</br> Payment Cancelled SuccessFully..."});
...
// issue 
	...
	$.ajax({
        ...
        data: {'searchTxt': searchTxt,'id_branch':id_branch,'issue_to':issue_to,'issue_type':issue_type,'is_eda':$('#is_eda').val()}, 
        ...
        success: function (data) {
...

			{
				...
				{ 
					...
					if(ctrl_page[1]=='receipt' && receipt_type==1 && ($("#id_customer").val()!=''))
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/get_customer_credit_details/?nocache=' + my_Date.getUTCSeconds(),             
        ...
        success: function (data) { 
...

			{
			    ...
			    $.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br> No Records Found..."});
...

{
	...
	$.each(issue_details,function(key,items){
		...
		+'<td><input type="checkbox" class="id_issue_receipt"  name="refund[id_issue_receipt]" value="'+items.id_issue_receipt+'">'

		+'<td><div value="'+items.bill_no+'">'+items.bill_no+'</div></td>'

		+'<td><input type="hidden" class="issue_amt" value="'+items.issue_amt+'" >'+items.issue_amt+'</td>'

		+'<td><input type="hidden" class="paid_amt" value="'+items.paid_amt+'" >'+items.paid_amt+'</td>'

		+'<td><span class="receipt_blc_amt">'+parseFloat(items.issue_amt - items.paid_amt - items.discount_amt)+'</span></td>'

		+'<td><input type="number" class="form-control discount_amt" value=""></td>'
...
$(document).on('change',".id_issue_receipt",function(){
...

   {
       ...
       discount_amount  = (row.find('.discount_amt').val()!='' ? row.find('.discount_amt').val() :0);

       balance_amount   = parseFloat(row.find('.issue_amt').val())-parseFloat(row.find('.paid_amt').val())-parseFloat(discount_amount);
...
$(document).on('change',".discount_amt",function(){
    ...
    var discount_amt        = parseFloat(row.find('.discount_amt').val()!='' ? row.find('.discount_amt').val():0).toFixed(2);

    var receipt_blc_amt     = parseFloat(row.find('.receipt_blc_amt').html()!='' ? row.find('.receipt_blc_amt').html():0).toFixed(2);

    var act_recd_amt        =  parseFloat(row.find('.receipt_blc_amt').html())-parseFloat(row.find('.discount_amt').val());

    if(parseFloat(receipt_blc_amt)<parseFloat(discount_amt))

    {

        $.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br> Please Enter The Valid Amount"});
...
$('#save_credit_collection').on('click',function(e){
	...
	$('#credit_collection .modal-body #issue_list > tbody  > tr').each(function(index, tr) {
	...
	$('#multiple_receipt_id').val(credit_details.length>0 ? JSON.stringify(credit_details):'');
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/get_customer_advance_details/?nocache=' + my_Date.getUTCSeconds(),             
...

{
    ...
    $('#receipt_refund .modal-body #refund_list > tbody  > tr').each(function(index, tr) {
...

       {
    	   ...
    	   refund_amt+=(isNaN(row.find('.issue_amount').val()) || (row.find('.issue_amount').val()=='') ? 0 :parseFloat(row.find('.issue_amount').val()));
...
$('#add_receipt_refund').on('click',function(e){
	...
	var advance_refund=[];
		...
		$('#multiple_receipt_id').val(advance_refund.length>0 ? JSON.stringify(advance_refund):'');
...
//advance refund
	...
	$('#cash_pay').on('keyup',function(e){
	    ...
	    if($('#cash_pay').val()>=10000 && ($('#is_eda').val()==1))
...

	{
		...
		balance_amount          =parseFloat(parseFloat(issue_amount)-parseFloat(final_price)).toFixed(2);
		...
		if(validateCardDetailRow()){
		    ...
		    $('#card-detail-modal .modal-body #card_details > tbody  > tr').each(function(index, tr) {
				...
				if($(this).find('.card_amt').val() != ""){
					...
					card_payment.push({'card_name':$(this).find('.card_name').val(),'card_type':$(this).find('.card_type').val(),'card_no':$(this).find('.card_no').val(),'card_amt':$(this).find('.card_amt').val(),'ref_no':$(this).find('.ref_no').val()});
...
$('#save_issue_net_banking').on('click',function(){
		...
		if(validateNBDetailRow()){
            ...
            $('#net_banking_modal .modal-body #net_bank_details > tbody  > tr').each(function(index, tr) {
                ...
                if($(this).find('.amount').val() != "" && $(this).find('.amount').val() != undefined ){

                nb_payment.push({'nb_type':$(this).find('.nb_type').val(),'nb_date':$(this).find('.nb_date').val(),'id_bank':$(this).find('.id_bank').val(),'amount':$(this).find('.amount').val(),'ref_no':$(this).find('.ref_no').val(),'payment_date':$(this).find('.nb_payment_date').val()});
...
$('#cancel_receipt').on('click',function(){
	...
	$.ajax({
		...
		url:base_url+ "index.php/admin_ret_billing/receipt/cancel?nocache=" + my_Date.getUTCSeconds(),
		...
		success:function(data){
			...
			$.toaster({ priority : 'success', title : 'Success!', message : ''+"</br> Receipt Cancelled SuccessFully..."});
...
$("#est_search").on('click', function(){
...

		{
			...
			$.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br> Please Select Branch..."});
        ...
        success: function (data) { 
			...
			if(data.success == true){
				...
				if(data.responsedata.old_matel_details.length > 0){
						...
						+'<td><span>'+(estval.purpose == 1 ? "Cash" : "Exchange")+'</span><input type="hidden" class="est_purpose" name="purchase[purpose][]" value="'+estval.purpose+'" /><input type="hidden" class="esti_detail_id" name="purchase[old_metal_sale_id][]" value="'+estval.old_metal_sale_id+'" /><input type="hidden" class="est_id_metal" name="purchase[id_metal][]" value="'+estval.id_metal+'" /><input type="hidden" class="est_item_type" name="purchase[item_type][]" value="'+estval.item_type+'" /></td>'
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/getEstimationDetailsTags/?nocache='+my_Date.getUTCSeconds(),             

        dataType: "json", 

        method: "POST", 

        data: {'estId' : esti_no,'billType':5,'id_branch':$('#branch_select').val(),'order_no':''}, //Need to update login branch id here from session
				...
				if(data.responsedata.item_details.length > 0){
    					...
    					$.each(data.responsedata.item_details, function (estkey, estval) {
        					...
        					if(!rowExist)
        						...
        						+'<td><input type="hidden" class="esti_sale_id" name="estsales[esti_detail_id][]" value="'+estval.esti_id+'" /><input type="hidden" class="est_item_type" name="estsales[item_type][]" value="'+estval.item_type+'" />'+estval.tag_code+'</td>'

        

        						+'<td><input type="hidden" class="esti_sale_tag_id" name="estsales[tag_id][]" value="'+estval.tag_id+'"  />'+estval.product_name+'</td>'

        

        						+'<td><span class="esti_sale_grs_wt">'+estval.gross_wt+'</span><input type="hidden" class="esti_sale_gross_val" name="estsales[gross_wt][]" value="'+estval.gross_wt+'"  /></td>'

        

        						+'<td><span class="esti_sale_ls_wt">'+estval.less_wt+'</span><input type="hidden" class="esti_sale_less_wt_val" value="'+estval.less_wt+'" name="estsales[less_wt][]" /></td>'

        

        						+'<td><span class="esti_sale_net_wt">'+estval.net_wt+'</span><input type="hidden" class="esti_sale_net_val" value="'+estval.net_wt+'"  name="estsales[net_wt][]"/></td>'

        

        						+'<td><span class="est_old_wastage">'+parseFloat(estval.wastage_percent)+'</span><input type="hidden" class="est_old_wastage_percent" name="estsales[wastage_percent][]" value="'+parseFloat(estval.wastage_percent)+'"  /></td>'

        

        						+'<td><span class="est_cost">'+estval.est_cost+'</span><input type="hidden" class="est_old_item_amount_val" value="'+estval.est_cost+'" name="estsales[amount][]" /><input type="hidden" value='+(JSON.stringify(stone_details))+' class="est_item_stone_dt" /><input type="hidden" class="est_stone_price" value="'+estval.stone_price+'"  /></td>'
...

{
	...
	var advance_adj_amt         =($('#tot_adv_adj').html()!='' ? $('#tot_adv_adj').html():0);

	var cash=($('#make_pay_cash').val()!='' ? $('#make_pay_cash').val():0);

	var pay_amount              =parseFloat(cash)+parseFloat(cc)+parseFloat(dc)+parseFloat(chq)+parseFloat(NB)+parseFloat(advance_adj_amt);	
	...
	if($('#is_pan_required').val()==1 && (parseFloat($('#min_pan_amt').val())<=parseFloat(receive_amount)))
...
/*	if($('#amount').val()=='' || $('#amount').val()==0)
	...
	else if($('#id_branch').val()=='' || $('#id_branch').val()==0 || $('#id_branch').val()==null)

	{

	    $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"Invalid Branch.."});
		...
		if(validateCardDetailRow()){
		    ...
		    $('#card-detail-modal .modal-body #card_details > tbody  > tr').each(function(index, tr) {
				...
				if($(this).find('.card_amt').val() != ""){
					...
					card_payment.push({'card_name':$(this).find('.card_name').val(),'card_type':$(this).find('.card_type').val(),'id_device':$(this).find('.id_device').val(),'card_no':$(this).find('.card_no').val(),'card_amt':$(this).find('.card_amt').val(),'ref_no':$(this).find('.ref_no').val()});
...
$('#save_net_banking').on('click',function(){
		...
		if(validateNBDetailRow()){
            ...
            $('#net_banking_modal .modal-body #net_bank_details > tbody  > tr').each(function(index, tr) {
                ...
                nb_payment.push({'nb_type':$(this).find('.nb_type').val(),'nb_date':$(this).find('.nb_date').val(),'id_bank':$(this).find('.id_bank').val(),'id_device':$(this).find('.id_device').val(),'amount':$(this).find('.amount').val(),'ref_no':$(this).find('.ref_no').val(),'payment_date':$(this).find('.nb_payment_date').val()});
        ...
        success: function (data) {
...

                    {
                        ...
                        $.each(data,function(key,items){
                            ...
                            +'<td><input type="checkbox" class="bill_id_issue_receipt"  name="adv_adj[id_issue_receipt]" value="'+items.id_issue_receipt+'"><input type="hidden" class="id_ret_wallet" value="'+items.id_ret_wallet+'"></td>'
                        ...
                        $('.total_bill_amt').html(parseFloat($('.receive_amount ').val()).toFixed(2));
...

{
	...
	$.ajax({
		...
		url: base_url+'index.php/admin_ret_billing/send_bill_cancel_otp/?nocache='+my_Date.getUTCSeconds(),             
...
$('#verify_otp').on('click',function(){
...

    {
    	...
    	$.ajax({
            ...
            url: base_url+'index.php/admin_ret_billing/verify_otp_for_billcancel/?nocache='+my_Date.getUTCSeconds(), 
...

    {
        ...
        $.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br>Please Enter The OTP"});
...

    {
        ...
        $.each(packing_items,function(key,items){
            ...
            $.each(other_inventory_item,function(key,pitem){
...

                {
...

                    {
                        ...
                        image_src =base_url+'assets/img/other_inventory/'+pitem.sku_id+'/'+pitem.item_image;
                ...
                item += "<option value='"+pitem.id_other_item+"' "+selected+">"+pitem.item_name+"</option>";
            ...
            trHtml+='<tr>'
                  ...
                  +'<td><input type="number" name="est_oth_inv[no_of_pcs][]" class="form-control no_of_pcs" value="'+items.no_of_piece+'"></td>'

                  +'<td><img class="img_src" src="'+image_src+'" width="30" height="30"></td>'
...

{
    ...
    var item = "<option value=''>-Select Item</option>";
        ...
        $.each(other_inventory_item,function(key,pitem){
            ...
            item += "<option value='"+pitem.id_other_item+"'>"+pitem.item_name+"</option>";
	...
	$('#bill_cus_name').attr("readonly",true);
		...
		$.each(data,function(key,item){
...

			{
				...
				if(item.id_orderdetails == bill_sale_row.find('.id_orderdetails').val())
...

			{
				...
				row+= '<tr id='+key+'>'
					...
					+'<td><span>'+item.hsn_code+'</span><input type="hidden" class="sale_pro_hsn" name="order[hsn]" value="'+item.hsn_code+'" /><input type="hidden" class="sale_type" name="order[sourcetype][]" value="1" /><input type="hidden" class="is_est_details" value="1" name="order[is_est_details][]" /><input type="hidden" class="sale_cal_type" name="order[calltype][]" value="'+item.calculation_based_on+'" /><input type="hidden" class="sale_metal_type" value="'+item.metal_type+'" /><input type="hidden" class="sale_purity" value="'+item.id_purity+'"  name="order[purity][]" /><input type="hidden" class="sale_size" value="'+item.size+'"  name="order[size][]" /><input type="hidden" class="total_tax" name="order[total_tax][]" /><input type="hidden" class="id_orderdetails" name="order[id_orderdetails][]" value="'+item.id_orderdetails+'"></td>'
					...
					+'<td><span>'+repair_percentage+'</span><input type="hidden" class="repair_percent" name="order[repair_percent][]" value="'+repair_percentage+'" /><input type="hidden" class="repair_tot_tax" name="order[repair_tot_tax][]" value="" /><input type="hidden" class="sale_tax_group" name="order[taxgroup][]" value="'+item.tax_group_id+'" /><input type="hidden" class="sale_cgst" name="order[cgst][]" value="" /><input type="hidden" class="tax_type" name="order[tax_type][]" value="" /><input type="hidden" class="sale_sgst" name="order[sgst][]" value="" /><input type="hidden" class="sale_igst" name="order[igst][]" value="" /><input type="hidden" class="sale_cgst_per" name="order[cgst_per][]" value="'+repair_percentage/2+'" /><input type="hidden" class="sale_sgst_per" name="order[sgst_per][]" value="'+repair_percentage/2+'" /></td>'

					+'<td><span>'+item.order_no+'</span><input type="hidden" class="order_no" name="order[order_no][]" value="'+item.order_no+'" /></td>'

					+'<td><span>'+item.completed_weight+'</span><input type="hidden" class="completed_weight" name="order[completed_weight][]" value="'+item.completed_weight+'" /></td>'

					+'<td><span>'+item.amount+'</span><input type="hidden" class="amount" name="order[amount][]" value="'+item.amount+'" /></td>'
...

{
...

	{
		...
		if($(this).find('.amount').val() != ""){
			...
			amount = parseFloat((isNaN(curRow.find('.amount').val()) || curRow.find('.amount').val() == '')  ? 0 : curRow.find('.amount').val());
			...
			cgst = parseFloat((isNaN(curRow.find('.sale_cgst').val()) || curRow.find('.sale_cgst').val() == '')  ? 0 : curRow.find('.sale_cgst').val());

			sgst = parseFloat((isNaN(curRow.find('.sale_sgst').val()) || curRow.find('.sale_sgst').val() == '')  ? 0 : curRow.find('.sale_sgst').val());

			igst = parseFloat((isNaN(curRow.find('.sale_igst').val()) || curRow.find('.sale_igst').val() == '')  ? 0 : curRow.find('.sale_igst').val());
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/admin_approval/?nocache='+my_Date.getUTCSeconds(),             
        ...
        success: function (data) {
...

				{
					...
					$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br> Admin Access Denied..."});
...
$('#otp_submit_validation').on('click',function(){
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/verify_otp/?nocache='+my_Date.getUTCSeconds(), 
...
$("input[name='billing[make_as_advance]']:radio").on('change',function(){
...

    {
...

        {
            ...
            $.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br>Please Select Customer.."});
        ...
        var trHtml='';
            ...
            setTimeout(function(){
                	...
                	$.each(scheme_details, function (pkey, pitem) {
                		...
                		scheme_list += "<option value='"+pitem.id_scheme+"'>"+pitem.scheme_name+"</option>";

                	});	

                    trHtml += '<tr>'

                              +'<td><select class="id_scheme" name="chit_details[material_id][]">'+scheme_list+'</select></td>'

                              +'<td><select class="select_scheme_account" name="chit_details[id_scheme_account][]">'+scheme_acc_list+'</select></td>'
                    ...
                    $('#chit_deposit_details > tbody').find('.select_scheme_account').select2();
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/get_customer_weight_scheme_details/?nocache='+my_Date.getUTCSeconds(), 
...

        {
              ...
              $.each(data,function(key,items){
                  ...
                  scheme_acc_list += "<option value='"+items.id_scheme_account+"'>"+items.account_name+"</option>";
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/get_one_time_pre_weight_scheme/?nocache='+my_Date.getUTCSeconds(), 
...
$('#save_chit_deposit').on('click',function(){
    ...
    chit_deposit_acc_details=[];
   	...
   	$('#chit_deposit_modal .modal-body #chit_deposit_details> tbody  > tr').each(function(index, tr) {

		chit_deposit_acc_details.push({'id_scheme' : $(this).find('.id_scheme').val(),'id_scheme_account' :$(this).find('.select_scheme_account').val()});

	});

	$('#chit_deposit_modal').modal('toggle');

	$('#chit_deposit_acc_details').val(chit_deposit_acc_details.length>0 ? JSON.stringify(chit_deposit_acc_details):'');
...

{
    ...
    trHtml+='<tr>'
                 ...
                 +'<td><input type="hidden" class="product" name="metal_details[id_product][]" value="'+$('#select_product').val()+'"><input type="hidden" class="id_design" name="metal_details[id_design][]" value="'+$('#select_design').val()+'"><input type="hidden" class="id_sub_design" name="metal_details[id_sub_design][]" value="'+$('#select_sub_design').val()+'">'+$('#select_product option:selected').text()+'</td>'

                 +'<td><input type="hidden" class="purity" name="metal_details[purity][]" value="'+$('#purity').val()+'">'+$('#purity').val()+'</td>'
                 ...
                 +'<td><input type="hidden" class="rate_per_gram" name="metal_details[rate_per_gram][]" value="'+$('#rate_per_gram').val()+'">'+$('#rate_per_gram').val()+'</td>'

                 +'<td><input type="hidden" class="item_cost" name="metal_details[item_cost][]" value="'+$('#item_cost').val()+'"><input type="hidden" class="item_total_tax" name="metal_details[item_total_tax][]" value=""><input type="hidden" class="item_total_sgst" name="metal_details[item_total_sgst][]" value=""><input type="hidden" class="item_total_cgst" name="metal_details[item_total_cgst][]" value=""><input type="hidden" class="item_total_igst" name="metal_details[item_total_igst][]" value="">'+$('#item_cost').val()+'</td>'
...
$('#issue_weight,#purchase_touch,#rate_per_gram').on('keyup',function(){
    ...
    var purity              = (isNaN($('#purity option:selected').text()) || $('#purity option:selected').text()=='' ? 0:$('#purity option:selected').text());
    ...
    item_cost               = parseFloat(parseFloat(purewt) * parseFloat(rate_per_gram)).toFixed(2);
...
$("input[name='billing[delivered_at]']:radio").on('change',function(){
...

    {
...

        {
            ...
            $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Verify The Address"});
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/get_customer_address/?nocache=' + my_Date.getUTCSeconds(),             
        ...
        success: function (data) {
...

            {
               ...
               $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Update Customer Address"});
...

            {
                ...
                if(registered_address.id_state!='' && registered_address.id_country!='')

                {
                    ...
                    var addressHtml = '<span>'
                                        ...
                                        +registered_address.address1+'</br>'

                                        +(registered_address.address2!='' ? registered_address.address2+'</br>' :'')

                                        +(registered_address.address3!='' ? registered_address.address3+'</br>' :'')

                                        +(registered_address.city_name!='' ? registered_address.city_name+' '+(registered_address.pincode!='' ? ' - '+registered_address.pincode :'') :'')+'</br>'

                                        +(registered_address.state_name!='' ? registered_address.state_name+'</br>' :'')

                                        +(registered_address.country_name!='' ? registered_address.country_name :'')+
...

{
    ...
    $.ajax({
        ...
        type: 'POST',
         ...
         url: base_url+'index.php/admin_ret_billing/get_mydelivery_address/?nocache=' + my_Date.getUTCSeconds(),             
        ...
        success: function(data) {
            ...
            $("#select_myaddr").select2("val", (id_delivery!=null && id_delivery!=''? id_delivery:''));
...
$('#select_myaddr').on('change',function(){
	...
	$.each(delivered_address,function(key,data){
...

		{
            ...
            +(data.city_name!='' ? data.city_name+' - '+(data.pincode!='' ? data.pincode :'') :'')+'</br>'

            +(data.state_name!='' ? data.state_name :'')+'</br>'

            +(data.country_name!='' ? data.country_name :'')+'</br>'
...

{
    ...
    $.ajax({
        ...
        success: function(country) {
            ...
            $("#del_country").select2("val", (id_country!=null && id_country!=''? id_country:''));
...

{
    ...
    $.ajax({
        ...
        $("#del_state").select2("val", (id_state!=null && id_state!=''? id_state:''));
...
$('#save_new_delivery_addr').on('click',function(){
     ...
     else if($('#select_myaddr').val()!='' && $('#select_myaddr').val()!=null)
...

     {
...

            {
                ...
                $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Enter The Delivery Address Name"});

            }

            else if($('#del_country').val()=='' || $('#del_country').val()==null)
...

            {
                ...
                $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Enter The Address1"});
...

            {
                ...
                $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Enter The Address2"});
...

            {
                ...
                $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Enter The PIN Code"});
...

            {
                ...
                +($('#del_address2').val()!='' ? $('#del_address2').val() :'')+'</br>'

                +($('#del_address3').val()!='' ? $('#del_address3').val() :'')+'</br>'

                +($('#del_city option:selected').text()+' - '+($('#del_pincode').val()))+'</br>'

                +$('#del_state option:selected').text()+'</br>'

                +$('#del_country option:selected').text()+'</br>'
...

{
    ...
    $.ajax({
        ...
        $("#state").select2("val",(cus_id_state!='' ? cus_id_state:(id_state!=null && id_state!=''? id_state:'')));
...
if($('#bill_cus_id').val()!='' && typeof $('#bill_cus_id').val()!='undefined')

{
 ...
 $('#city option').remove();
	...
	url: base_url+'index.php/admin_ret_billing/getCustomersindRecords/?nocache=' + my_Date.getUTCSeconds(),             
...

{
  ...
  $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>"+"Kindly Select Any One Customer Records.."});
...
$('#billed_search').on('click',function(){
	...
	else if($('#paymentmode_ed').val() == '')

	{

		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br> Enter Bill Number.."});

		return false;

	}

    else if($('#paymentmode_ed').val() != '' && $('#branch_select').val() != '')
...

        				{
...

        				   {
        					   ...
        					   var bank_list='';
...

                            	{
                            		...
                            		bank_list += "<option value='"+item.id_bank+"' "+(data[i].id_branch==[i] ? selected:'')+">"+item.acc_number+"</option>";
                            	...
                            	row += '<tr>'
                            				...
                            				+'<td><select name="card_details[card_name][]" class="form-control card_name"><option value="1" '+(data[i].card_type==1 ? selected:'')+'>RuPay</option><option value="2" '+(data[i].card_type==2 ? selected:'')+'>VISA</option><option value="3" '+(data[i].card_type==3 ? selected:'')+'>Mastro</option><option value="4" '+(data[i].card_type==4 ? selected:'')+'>Master</option></select></td>'

                            				+'<td><select name="card_details[card_type][]" class="form-control card_type"><option value="1" '+(data[i].payment_mode == "CC" ? selected:'')+'>CC</option><option value="2" '+(data[i].payment_mode == "DC" ? selected:'')+'>DC</option></select></td>'

                            				+'<td><select class="form-control id_bank" name="card_details[id_bank][]" >'+bank_list+'</select></td> '

                            				+'<td><input type="number" step="any" class="form-control card_no" name="card_details[card_no][]" value="'+data[i].card_no+'"/></td>'

                            				+'<td><input type="number" step="any" class="form-control card_amt" name="card_details[card_amt][]" value="'+data[i].payment_amount+'"/></td>' 
...

        				   {
                            	...
                            	row += '<tr>'
                            				...
                            				+'<td><input class="form-control  datemask date cheque_date" data-date-format="dd-mm-yyyy" name="cheque_details[cheque_date][]" type="text" placeholder="Cheque Date" value="'+data[i].cheque_date+'"/></td>'

                            				+'<td><select class="form-control id_bank" name="cheque_details[id_bank][]">'+bank_list+'</select></td> '

                            				+'<td><input type="number" step="any" class="cheque_no" name="cheque_details[cheque_no][]" value="'+data[i].cheque_no+'"/></td>' 

                            				+'<td><input type="number" step="any" class="payment_amount" name="cheque_details[payment_amount][]" value="'+data[i].payment_amount+'"/></td>' 
...

        				   {
        					   ...
        					   var bank_list='';
                                ...
                                +'<td><select name="nb_details[nb_type][]" class="form-control nb_type"><option value=1 '+(data[i].NB_type==1 ? selected:'')+'>RTGS</option><option value=2 '+(data[i].NB_type==2 ? selected:'')+'>IMPS</option><option value=3 '+(data[i].NB_type==3 ? selected:'')+'>UPI</option></select></td>'

                                +'<td class="upi_type" "'+ (data[i].NB_type==3 ? 'style="display: none "':'style="display: block "')+'"><select class="form-control " name="nb_details[id_bank][]"  >'+bank_list+'</select></td> '

                                +'<td><input class="form-control  datemask date nb_date" data-date-format="yyyy-mm-dd" name="nb_details[nb_date][]" type="text" placeholder="NB Date"  value="'+data[0].payment_date+'" /></td>'

			                    +'<td class="device"  ><select class="form-control id_device" name="card_details[id_device][]" style="width: 100px !important;">'+device_list+'</select></td> '

                                +'<td><input type="number" step="any" class="form-control ref_no" name="nb_details[ref_no][]" placeholder="Enter Ref No" value="'+data[i].payment_ref_number+'"/></td>'

                                +'<td><input type="number" step="any" class="form-control amount" name="nb_details[amount][]" placeholder="Enter The Amount" value="'+data[i].payment_amount+'"/></td>'
        				 ...
        				 $.toaster({ priority : 'warning', title : 'Warning!', message : ''+"</br> Invalid Bill No..."});	
...
$('#save_bill_edit').on('click',function(){
...

	{
			...
			$('#card-detail-modal .modal-body #card_details > tbody  > tr').each(function(index, tr) {
                        ...
                        card_payment_edit.push({'card_name':$(this).find('.card_name').val(),'card_type':$(this).find('.card_type').val(),'id_bank':$(this).find('.id_bank').val(),'card_no':$(this).find('.card_no').val(),'card_amt':$(this).find('.card_amt').val(),'id_device':$(this).find('.id_device').val()});
			...
			if($(this).find('.payment_amount').val() != ""){
				...
				chq_payment_edit.push({'cheque_date':$(this).find('.cheque_date').val(),'cheque_no':$(this).find('.cheque_no').val(),'id_bank':$(this).find('.id_bank').val(),'bank_branch':$(this).find('.bank_branch').val(),'payment_amount':$(this).find('.payment_amount').val()});
			 ...
			 $('#net_banking_modal .modal-body #net_bank_details > tbody  > tr').each(function(index, tr) {
                ...
                nb_payment_edit.push({'nb_type':$(this).find('.nb_type').val(),'id_device':$(this).find('.id_device').val(),'id_bank':$(this).find('.id_bank').val(),'amount':$(this).find('.amount').val(),'ref_no':$(this).find('.ref_no').val()});
				...
				data:{'bill_id':bill_id,'payment_date':payment_date,'card_payment':card_payment_edit,'chq_payment':chq_payment_edit,'nb_payment':nb_payment_edit,'created_by':created_by,'cash_pay':$('#make_pay_cash').val(),'branch':$('#branch_select').val(),'bill_no':$('#paymentmode_ed').val()},		
				...
				success:function(data){
...

    					{
							...
							$.toaster({ priority : 'success', title : 'Success!', message : ''+"</br> Bill Edited SuccessFully..."});
...

    					{
							...
							$.toaster({ priority : 'warning', title : 'Success!', message : ''+"</br> Unable to Bill Edit"});
...
$(document).on('keyup', '.amount, .payment_amount, .card_amt, #make_pay_cash', function(e){
		...
		var total_payment_amount_edit = (parseFloat(cash_final) + parseFloat(card_final) + parseFloat(chq_final) + parseFloat(nb_final));

        

		if(parseFloat(tot_bill_amount_edit).toFixed(2) == parseFloat(total_payment_amount_edit).toFixed(2))
...
$('#card-detail-modal,#cheque-detail-modal,#net_banking_modal').on('hidden.bs.modal', function () {
...
function validate_max_cash() {
	...
	let max_cash_amt = isNaN($("#max_cash_amt").val()) || $.trim($("#max_cash_amt").val()) == "" ? 0 : $("#max_cash_amt").val();

	let cash_pay = isNaN($("#make_pay_cash").val()) || $.trim($("#make_pay_cash").val()) == "" ? 0 : $("#make_pay_cash").val();

	let chit_cash_paid = isNaN($("#chit_total_cash_amt").val()) || $.trim($("#chit_total_cash_amt").val()) == "" ? 0 : $("#chit_total_cash_amt").val();

	let adv_total_cash_amt = isNaN($("#adv_total_cash_amt").val()) || $.trim($("#adv_total_cash_amt").val()) == "" ? 0 : $("#adv_total_cash_amt").val();

	max_cash_amt = parseFloat(max_cash_amt) - parseFloat(chit_cash_paid) - parseFloat(adv_total_cash_amt);

	if(parseFloat($('#validate_max_cash').val()) == 1 ? ((parseFloat(cash_pay)) >= parseFloat(max_cash_amt)) : false) {

		$("#make_pay_cash").val(0);

		max_cash_amt = parseFloat($("#max_cash_amt").val()) - 1;

		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Maximum cash amount for this bill is "+max_cash_amt});
...
function validate_max_cash_for_receipt() {
	...
	if(parseFloat($('#validate_max_cash').val()) == 1 ? ((parseFloat(cash_pay)) >= parseFloat(max_cash_amt)) : false) {
		...
		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Maximum cash amount for this receipt is "+(parseFloat(max_cash_amt)-1)});
...

{
	...
	$.ajax({
		...
		url: base_url+'index.php/admin_ret_billing/send_credit_bill_otp/?nocache='+my_Date.getUTCSeconds(),             
...
$('#verify_credit_otp').on('click',function(){
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/verify_credit_otp/?nocache='+my_Date.getUTCSeconds(), 
...

{
	...
	let total_payment_amount = $.isNumeric($(".sale_amt_with_tax").html()) ? parseFloat($(".sale_amt_with_tax").html()) : 0;

	let summary_discount_amt = $.isNumeric($("#summary_discount_amt").val()) ? parseFloat($("#summary_discount_amt").val()) : 0;

	//let handling_charges = $.isNumeric($("#handling_charges").val()) ? parseFloat($("#handling_charges").val()) : 0;

	total_payment_amount = parseFloat(total_payment_amount) + parseFloat(old_discount);
	...
	if(total_payment_amount > 0 && summary_discount_amt > 0) {
		...
		$('#billing_sale_details > tbody tr').each(function(idx, row) {
			...
			var purchase_cost  = (isNaN(curRow.find('.purchase_cost').val()) || curRow.find('.purchase_cost').val() == '')  ? 0 : curRow.find('.purchase_cost').val(); 
			...
			if(calculation_type == 0 || calculation_type == 1 || calculation_type == 2) {
				...
				rate_with_mc  = parseFloat((isNaN(curRow.find('.purchase_cost').val()) || curRow.find('.purchase_cost').val() == '')  ? 0 : curRow.find('.purchase_cost').val()); 

				rate_with_mc = parseFloat(rate_with_mc) + (parseFloat(rate_with_mc) * parseFloat(mrpmargin_perc) / 100);
			...
			total_amount_with_mc_va_limit =  (parseFloat(total_amount_with_mc_va_limit) + parseFloat(inclusive_tax_rate)).toFixed(2);
		...
		console.log("total_amount_with_mc_va_limit ", total_amount_with_mc_va_limit);

		let _discount_limit = parseFloat(total_payment_amount) - parseFloat(total_amount_with_mc_va_limit);
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/getBranchDayClosingData/?nocache='+my_Date.getUTCSeconds(), 
    ...
    $.each(other_metal_details,function(key,items){
        ...
        $.each(purity_rate,function(key,item){
...

             {
                ...
                rate_per_gram = (isNaN($('#'+rate_field).val()) || $('#'+rate_field).val() == '')  ? 0 : $('#'+rate_field).val();
...

{
    ...
    row= '<tr>'
	    ...
	    +'<td><span>'+repair_percentage+'</span><input type="hidden" class="repair_percent" name="order[repair_percent][]" value="'+repair_percentage+'" /><input type="hidden" class="repair_tot_tax" name="order[repair_tot_tax][]" value="" /><input type="hidden" class="sale_tax_group" name="order[taxgroup][]" value="" /><input type="hidden" class="sale_cgst" name="order[cgst][]" value="" /><input type="hidden" class="tax_type" name="order[tax_type][]" value="" /><input type="hidden" class="sale_sgst" name="order[sgst][]" value="" /><input type="hidden" class="sale_igst" name="order[igst][]" value="" /><input type="hidden" class="sale_cgst_per" name="order[cgst_per][]" value="'+repair_percentage/2+'" /><input type="hidden" class="sale_sgst_per" name="order[sgst_per][]" value="'+repair_percentage/2+'" /></td>'
...

{
...

    {
        ...
        $('#billing_repair_order_other_details > tbody  > tr').each(function(index, tr) {
    		...
    		if($(this).find('.sale_product_id').val() == "" || $(this).find('.rapair_amount').val() == "" || $(this).find('.completed_weight').val() == "" ){
	...
	var tot_amount=0;
...

    	{
    		...
    		if($(this).find('.rapair_amount').val() != ""){
    			...
    			tot_amount+=parseFloat(parseFloat(amount)+parseFloat(total_other_item_amount));
...

{
	...
	total_bill_amount		= parseFloat((isNaN($('#total_cost').val()) || $('#total_cost').val() == '')  ? 0 : $('#total_cost').val()).toFixed(2);
...
$('.service_bill_receive_amount,#service_bill_cash').on('keyup',function(){
...

    {
        ...
        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Enter The Valid Amount.."});
...

{
	...
	var receive_amount       = ($('.service_bill_receive_amount').val()!='' ? $('.service_bill_receive_amount').val():0);

	var make_pay_cash        = ($('#service_bill_cash').val()!='' ? $('#service_bill_cash').val():0);
...

	{
		...
		total_payment_amount=parseFloat(parseFloat(make_pay_cash)+parseFloat(cc)+parseFloat(dc)+parseFloat(chq)+parseFloat(NB)).toFixed(2);

		bal_amount=parseFloat(parseFloat(receive_amount)-parseFloat(total_payment_amount)).toFixed(2);
...
$('#service_bill_pay_submit').on('click',function(){
...

		{
...

		    {
		        ...
		        $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Check The Payment Amount Details.."});
...

		{
			...
			var url=base_url+ "index.php/admin_ret_billing/service_bill/save?nocache=" + my_Date.getUTCSeconds();
		    ...
		    $.ajax({ 
		        ...
		        success:function(data){
...

					{
						...
						window.open( base_url+'index.php/admin_ret_billing/service_bill_invoice/'+data['id'],'_blank');

						window.location.href= base_url+'index.php/admin_ret_billing/service_bill/add';
...

{
	...
	$.ajax({
		 ...
		 url:base_url+"index.php/admin_ret_billing/service_bill/ajax?nocache=" + my_Date.getUTCSeconds(),
...

{
...

    {
        ...
        "aoColumns": [{ "mDataProp": "id_service_bill" },
        ...
        print_url=base_url+'index.php/admin_ret_billing/service_bill_invoice/'+id;

        delete_url=(access.delete=='1' ? base_url+'index.php/admin_ret_billing/billing/delete/'+id : '#' );

        billcancel_url=(access.edit=='1' ? base_url+'index.php/admin_ret_billing/billing/cancell/'+id+'/'+row.bill_no : '#' );

        action_content='<a href="'+print_url+'" target="_blank" class="btn btn-info btn-print" data-toggle="tooltip" title="Billing Receipt"><i class="fa fa-print" ></i></a>'+(row.allow_cancel==1 && profile.allow_bill_cancel ==1 && row.bill_status==1 && access.edit=='1'?'<button class="btn btn-warning" onclick="confirm_delete('+id+','+profile.bill_cancel_otp+')"><i class="fa fa-close" ></i></button>' :'');
...
$('#cancell_service_bill').on('click',function(){
	...
	$.ajax({
		...
		url:base_url+ "index.php/admin_ret_billing/cancel_service_bill?nocache=" + my_Date.getUTCSeconds(),

		dataType:'json',

		data:{'remarks':$('#service_bill_remark').val(),'bill_id':$('#bill_id').val()},
...

{
        ...
        url:base_url+ "index.php/admin_ret_billing/order_place?nocache=" + my_Date.getUTCSeconds()+''+my_Date.getUTCMinutes()+''+my_Date.getUTCHours(),
...

{
        ...
        url:base_url+ "index.php/admin_ret_billing/update_branch?nocache=" + my_Date.getUTCSeconds()+''+my_Date.getUTCMinutes()+''+my_Date.getUTCHours(),
    ...
    var cmp_state           = $('#cmp_state').val();
     ...
     $('#return_item_detail > tbody tr').each(function(idx, row){
        ...
        retItemRowCost = parseFloat(retTotal) + parseFloat(stoneAmount) + parseFloat(otherMetalAmount);
         ...
         var tax_details     = get_tax_details(parseFloat(retItemRowCost), parseInt(curRow.find('.return_item_tax_id').val()));
...
$(document).on('change', '.purreturnrate, .purreturnpcs, .purreturnweight,.purreturncaltype', function(event) {
...

	{
...

        {
...

            {
                ...
                nontagreturnitemlist[index].piece = parseFloat(parseFloat(nontagreturnitemlist[index].piece)+parseFloat($('#issue_pcs').val())).toFixed(3);

                nontagreturnitemlist[index].gross_wt =parseFloat(parseFloat(nontagreturnitemlist[index].gross_wt)+parseFloat($('#issue_weight').val())).toFixed(3);
...

        {
...

            {
                ...
                insertedcatdetails[index].piece = parseFloat(parseFloat(insertedcatdetails[index].piece)+parseFloat($('#issue_pcs').val())).toFixed(3);

                insertedcatdetails[index].gross_wt =parseFloat(parseFloat(insertedcatdetails[index].gross_wt)+parseFloat($('#issue_weight').val())).toFixed(3);
...

            {
                ...
                insertedcatdetails.push({'cat_id':cat_id,'piece':piece,'catname':categoryname,'gross_wt':issue_weight,'tgrp_id':tgrp_id,'less_wt':0,'other_metal_weight':0,'stone_details':'','other_metal_details':''});
...

{
	...
	$.ajax({
		...
		url: base_url+'index.php/admin_ret_billing/getCustomerDet/?nocache='+ my_Date.getUTCSeconds(),

		dataType:"json",

		method: "POST",

		data:{'id_customer':id_customer,'id_branch':($('#branch_filter').val()!='' && $('#branch_filter').val()!=undefined ? $('#branch_filter').val(): $("#branch_select").val() || $('#id_branch').val())},
	...
	"<table id='bill_list' class='table table-bordered table-striped text-center'>"+
	   ...
	   "<tfoot >"+
		 ...
		 "<th style='text-align: left'>Total Outstanding Amount : "+money_format_india(total_pending_amt)+"</th>"+
...

{
	...
	$.ajax({
        ...
        url: base_url+'index.php/admin_ret_billing/getCreditPending/?nocache=' + my_Date.getUTCSeconds(),             

        dataType: "json", 

        method: "POST", 

        data: {'id_customer': id_customer,'id_branch' : $("#id_branch").val(),'col_type':col_type},
...

			{
...

				{
					...
					var row = '<tr>'
					           ...
					           +'<td><input type="checkbox" class="select_credit_bill" value="1" /><input type="hidden" class="ref_no" value='+val.s_ref_no+'></td>'

							   +'<td class="bill_no"><a href='+url+' target="_blank">'+val.bill_no+'</td>'

							   +'<td class="bill_date">'+val.bill_date+'</td>'

							   +'<td class="tot_bill_amount">'+val.tot_bill_amount+'</td>'

							   +'<td class="due_amount">'+val.due_amount+'</td>'

							   +'<td class="paid_amount">'+val.paid_amount+'</td>'

							   +'<td class="bal_amt">'+val.bal_amt+'</td>'
...

{
	...
	$.ajax({
		...
		url: base_url+'index.php/admin_ret_billing/getCustomerSalesDetails/?nocache=' + my_Date.getUTCSeconds(),             
...
$('#bill_split_search').on('click',function(){
...

	{
		...
		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Enter the Est no.."});
...

{
	...
	url:base_url+ "index.php/admin_ret_billing/bill_split/esti_details?nocache=" + my_Date.getUTCSeconds()+''+my_Date.getUTCMinutes()+''+my_Date.getUTCHours(),

	data: {'id_branch':($('#branch_filter').val()!='' && $('#branch_filter').val()!=undefined ? $('#branch_filter').val(): $("#branch_select").val()),'est_no':$('#est_split_no').val()},
...
$('#ratio_apply').on('click',function(){
...

	{
			...
			$('#billing_sale_details > tbody tr').each(function(idx, row){
...

				{
					...
					let applied_weight = parseFloat((ratio_per / 100) * availableBeginning).toFixed(3);
...

					{
...

							{
								...
								$.each(stone_details,function(k,val){
...

									{
											...
											if(parseFloat(splitted_stn_weight)<parseFloat(stone_weight))

											{
...

												{
													...
													row_stn_details.push({
														...
														'stone_price'       :(val.stone_cal_type==1 ? parseFloat(parseFloat(splitted_stn_weight)*parseFloat(val.rate_per_gram)).toFixed(2) :parseFloat(parseFloat(val.stone_pcs)*parseFloat(val.rate_per_gram)).toFixed(2)),
						...
						bill_split_details.push({
							...
							'sale_pcs'                      :(index==0 ? curRow.find('.sale_pcs').val():0),
							...
							'charges_details'               :(index==0 ?curRow.find('.charges_details').val() :[]),
				...
				balance_weight = parseFloat(parseFloat(gross_wt) - parseFloat(splitted_weight)).toFixed(3);
...

				{
...

					{
						...
						$.each(tag_stone_details,function(k,stval){
							...
							var balance_stone_weight = parseFloat(parseFloat(tag_less_wt)-parseFloat(splitted_stone_weight)).toFixed(3);
...

							{
								...
								balacne_stone_details.push({
									...
									'stone_price'       :(stval.stone_cal_type==1 ? parseFloat(parseFloat(balance_stone_weight)*parseFloat(stval.rate_per_gram)).toFixed(2) :parseFloat(parseFloat(stval.stone_pcs)*parseFloat(stval.rate_per_gram)).toFixed(2)),
					...
					bill_split_details.push({
						...
						'stone_details'                 :(balacne_stone_details.length > 0 ? balacne_stone_details:[]),
...

			{
				...
				$.each(bill_split_details,function(key,estval){
					...
					row+= '<tr id="'+key+'" class="'+key+'" >'
									...
									+'<td><span>'+estval.sale_pro_hsn+'</span><input type="hidden" name="split_sale[id_collecion_maping_det][]" class="id_collecion_maping_det" value="'+estval.id_collecion_maping_det+'"><input type="hidden" name="split_sale[order_no][]" class="order_no" value="'+estval.order_no+'"><input type="hidden" name="split_sale[id_orderdetails][]" class="id_orderdetails" value="'+estval.id_orderdetails+'"><input type="hidden" name="split_sale[id_customerorder][]" class="id_customerorder" value="'+estval.id_customerorder+'"><input type="hidden" class="sale_pro_hsn" name="split_sale[hsn]" value="'+estval.sale_pro_hsn+'" /><input type="hidden" class="sale_type" name="split_sale[sourcetype][]" value="1" /><input type="hidden" class="sale_item_type" name="split_sale[itemtype][]" value="'+estval.sale_item_type+'" /><input type="hidden" class="is_est_details" value="1" name="split_sale[is_est_details][]" /><input type="hidden" class="est_itm_id" name="split_sale[est_itm_id][]" value="'+estval.est_itm_id+'" /><input type="hidden" class="sale_cal_type" name="split_sale[calltype][]" value="'+estval.sale_cal_type+'" /><input type="hidden" class="sale_metal_type" value="'+estval.sale_metal_type+'" /><input type="hidden" class="sale_purity" value="'+estval.sale_purity+'"  name="split_sale[purity][]" /><input type="hidden" class="sale_size" value="'+estval.sale_size+'"  name="split_sale[size][]" /><input type="hidden" class="sale_uom" value="'+estval.sale_uom+'"  name="split_sale[uom][]" /><input type="hidden" class="total_tax" name="split_sale[item_total_tax][]"><input type="hidden" class="is_partial" value="'+estval.is_partial+'"  name="split_sale[is_partial][]" /><input type="hidden" class="min_wastage" value="'+estval.min_wastage+'"  name="split_sale[min_wastage][]" /><input type="hidden" class="max_wastage" value="'+estval.max_wastage+'"  name="split_sale[max_wastage][]" /><input type="hidden" class="stock_type" name="split_sale[stock_type][]" value="'+estval.stock_type+'" /><input type="hidden" class="is_non_tag" name="split_sale[is_non_tag][]" value="'+estval.is_non_tag+'" /><input type="hidden" class="rate_field"  value="'+estval.rate_field+'" /><input type="hidden" class="mc_limit"  value="'+estval.mc_limit+'" /><input type="hidden" class="va_limit"  value="'+estval.va_limit+'" /><input type="hidden" class="margin_mrp"  value="'+estval.margin_mrp+'" /><input type="hidden" class="purchase_cost" value="'+estval.purchase_cost+'" /></td>'

									+'<td><span class="product_name">'+estval.product_name+'</span><input class="sale_product_id" type="hidden" name="split_sale[product][]" value="'+estval.sale_product_id+'" /><input class="scheme_closure_benefit" type="hidden" name="split_sale[scheme_closure_benefit][]" value="'+estval.scheme_closure_benefit+'" /><input type="hidden" class="esti_no"  name="split_sale[esti_no][]"  value="'+estval.esti_no+'" /></td>'

									+'<td><span class="design_name">'+estval.design_name+'</span><input type="hidden" class="sale_design_id" name="split_sale[design][]" value="'+estval.sale_design_id+'" /><input type="hidden" class="id_sub_design" name="split_sale[id_sub_design][]" value="'+estval.id_sub_design+'" /></td>'

									+'<td><span>'+estval.sale_pcs+'</span><input type="hidden" class="sale_pcs" name="split_sale[pcs][]" value="'+estval.sale_pcs+'"  /></td>'

									+'<td><span>'+estval.bill_gross_val+'</span><input type="hidden" class="bill_gross_val" name="split_sale[gross][]" value="'+estval.bill_gross_val+'" /></td>'

									+'<td><span class="bill_split_lesswt">'+estval.bill_less_val+'</span><input type="hidden" class="bill_less_val" name="split_sale[less][]" value="'+estval.bill_less_val+'" /></td>'

									+'<td><span class="bill_sale_net_wt"></span><input type="hidden" class="bill_net_val" name="split_sale[net][]" value="'+estval.bill_net_val+'" /><input type="hidden" class="bill_tag_net_wt_val"  value="'+estval.bill_net_val+'" /></td>'

									+'<td><span class="est_wastage">'+estval.bill_wastage_per+'</span><input type="hidden" class="bill_wastage" value="'+estval.bill_wastage_per+'" /><input type="hidden" class="bill_wastage_per" name="split_sale[wastage][]" value="'+estval.bill_wastage_per+'" /></td>'
									...
									+'<td><span class="making_charge">'+estval.bill_mc+'</span><input type="hidden" class="mc_dis" value="0"><input type="hidden" class="bill_mctype" name="split_sale[bill_mctype][]" value="'+estval.bill_mctype+'" /><input type="hidden" class="bill_mc" name="split_sale[mc][]" value="'+estval.bill_mc+'" /><input type="hidden" class="discount"  name="split_sale[adjusted_dis][]" value="" /><input type="hidden" class="bill_mc_value" value="'+estval.bill_mc+'" /></td>'

									+'<td><input type="number" class="bill_discount" name="split_sale[discount][]"  step="any" readonly/></td>'

									+'<td><span class="bill_taxable_amt"></span></td>'

									+'<td><span class="tgrp_name">'+estval.tgrp_name+'</span><input type="hidden" class="sale_tax_group" name="split_sale[taxgroup][]" value="'+estval.sale_tax_group+'" /></td>'

									+'<td class="tax_amt"></td>'

									+'<td class="total_charges">'+estval.charge_value+(estval.charges_details.length>0 ? ' <a href="#" onClick="show_chargesdetails(event, $(this).closest(\'tr\'));" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>' :'-')+'<input type="hidden" value='+estval.charges_details+' class="charges_details" /><input type="hidden" class="charge_value" value="'+estval.charge_value+'" /></td>'

									//+'<td>'+(stone_details.length>0 ? '<a href="#" onClick="create_new_empty_bill_sales_stone_item($(this).closest(\'tr\'));" class="btn btn-success"><i class="fa fa-plus"></i></a>' :'-')+'<input type="hidden" class="certification_cost" value="'+estval.certification_cost+'" /><input type="hidden" class="est_old_stone_val" value="" /><input type="hidden" class="est_old_dust_val" value="" /><input type="hidden" class="bill_material_price" value="'+estval.othermat_amount+'"/><input type="hidden" name="sale[stone_details][]" value=\''+(JSON.stringify(stone_details))+'\' class="stone_details" /><input type="hidden" class="tag_other_itm_amount" value="'+tag_other_itm_amount+'"/><input type="hidden" class="other_metal_details" name="sale[other_metal_details][]" value='+(JSON.stringify(other_metal_details))+' class="other_metal_details" /></td>'

									+'<td>'+(estval.stone_details.length>0 ? '<a href="#" onClick="create_new_empty_bill_sales_stone_item($(this).closest(\'tr\'));" class="btn btn-success"><i class="fa fa-plus"></i></a>' :'-')+'<input type="hidden" class="certification_cost" value="'+estval.certification_cost+'" /><input type="hidden" class="est_old_stone_val" value="" /><input type="hidden" class="est_old_dust_val" value="" /><input type="hidden" class="bill_material_price" value="'+estval.othermat_amount+'"/><input type="hidden" name="split_sale[stone_details][]" value=\''+(JSON.stringify(estval.stone_details))+'\' class="stone_details" /><input type="hidden" class="tag_other_itm_amount" value=""/><input type="hidden" class="other_metal_details" name="split_sale[other_metal_details][]" value="" class="other_metal_details" /></td>'

									+'<td><input type="hidden" class="bill_stone_price" value="'+estval.bill_stone_price+'" /><input type="number" class="bill_amount" name="split_sale[billamount][]" value="" step="any" readonly /><input type="hidden" class="per_grm_amount" name="split_sale[per_grm][]" value="'+estval.per_grm_amount+'" step="any" /><input type="hidden" class="round_of_amt" name="split_sale[round_of_amt][]" value="" /></td>'
									...
									+'<td><span class="tag_code">'+estval.tag_code+'</span><input type="hidden" class="sale_tag_id" name="split_sale[tag][]" value="'+estval.sale_tag_id+'" /><input type="hidden" name="split_sale[total_cgst][]" class="sale_cgst"/><input type="hidden" class="sale_sgst" name="split_sale[total_sgst][]"/><input type="hidden" class="sale_igst" name="split_sale[total_igst][]"/></td>'

									+'<td>'+estval.order_no+'</td>'

									+'<td><span class="esti_number">'+estval.esti_number+'</span><input type="hidden" class="sale_est_itm_id" name="split_sale[estid][]" value="'+estval.est_item_id+'" /><input type="hidden" class="tag_sales_value"  value="'+estval.item_cost+'" /><input type="hidden" class="gift_applicable"  value="'+estval.gift_applicable+'" /></td>'
...

	{
		...
		$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Please Enter The Valid Ratio in Percentage"});
	...
	$('#billing_split_sale_details > tbody tr').each(function(idx, row){
		...
		if(calculation_type == 0){ 
			...
			if(curRow.find('.bill_mctype').val() != 3){
    			...
    			var mc_type       =  parseFloat(curRow.find('.bill_mctype').val() == 2 ? parseFloat(retail_max_mc * gross_wt ) : parseFloat(retail_max_mc * pcs));
			...
			if(curRow.find('.bill_mctype').val() != 3){
    			...
    			var mc_type       =  parseFloat(curRow.find('.bill_mctype').val() == 2 ? parseFloat(retail_max_mc * net_wt ) : parseFloat(retail_max_mc * pcs));
		...
		var split_item_cost = parseFloat(Math.round(inclusive_tax_rate)).toFixed(2);
...

{
	...
	$('#billing_split_sale_details > tbody tr').each(function(idx, row){
...

		{
			...
			$.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Received Amount Is Empty.."});
...

{
...

	{
		...
		var url=base_url+ "index.php/admin_ret_billing/billing/split_save?nocache=" + my_Date.getUTCSeconds();
...

{
	...
	$.each(payment_mode_details,function(k,pm){
		...
		paymode+= "<option value='" + pm.short_code + "' >" + pm.short_code + "</option>";
	...
	$.each(bank_details,function(k,bnk){
		...
		bank+= "<option value='" + bnk.id_bank + "' >" + bnk.acc_number + "</option>";
	...
	$.each(payment_device_details,function(k,val){
		...
		device+= "<option value='" + val.id_device + "' >" + val.device_name + "</option>";

	});



	var net_bank_type = '<option value="">Select Net Bank Type</option>'
	...
	var card_name = '<option value="">Select Card</option>'
	...
	trHtml = '<tr>'
			  ...
			  +'<td><select class="paymode form-control"  value="" style="width: 100px;">'+paymode+'</select></td>'

			  +'<td><input type="number" class="recd_amt form-control" style="width: 100px;"></td>'

			  +'<td><select class="card_name form-control"  value="" style="width: 100px;" disabled>'+card_name+'</select></td>'

			  +'<td><select class="device_type form-control"  value="" style="width: 100px;" disabled>'+device+'</select></td>'

			  +'<td><select class="net_bank_type form-control"  value="" style="width: 100px;" disabled>'+net_bank_type+'</select></td>'

			  +'<td><input type="number" class="ref_no form-control" style="width: 100px;" disabled></td>'

			  +'<td><input type="number" class="approval_no form-control" style="width: 100px;" disabled></td>'

			  +'<td><select class="bankname form-control"  value="" style="width: 100px;" disabled>'+bank+'</select></td>'
...

{
	...
	$('#pay_details_table > tbody tr').each(function(idx, row){
		...
		if(curRow.find('.recd_amt').val()=='' || curRow.find('.recd_amt').val()==0)
		...
		else if((curRow.find('.paymode').val()=='CC' || curRow.find('.paymode').val()=='DC') && ((curRow.find('.card_name').val()=='') || (curRow.find('.ref_no').val()=='')))
		...
		else if((curRow.find('.paymode').val()=='NB') && (curRow.find('.net_bank_type').val()==''))
		...
		else if((curRow.find('.net_bank_type').val()==3) && (curRow.find('.device_type').val()==''))
...

{
	...
	$('#pay_details_table > tbody tr').each(function(idx, row){
		...
		recd_amount+= parseFloat((isNaN(curRow.find('.recd_amt').val()) || curRow.find('.recd_amt').val() == '')  ? 0 : curRow.find('.recd_amt').val());
	...
	$('#bill_split_adv_adj > tbody tr').each(function(idx, row){
		...
		adj_amount+= parseFloat((isNaN(curRow.find('.adj_amount').val()) || curRow.find('.adj_amount').val() == '')  ? 0 : curRow.find('.adj_amount').val());
	...
	balance_amount = parseFloat(parseFloat(tot_bill_split_amount)-parseFloat(recd_amount)-parseFloat(adj_amount)).toFixed(2);
...

{
	...
	if(curRow.find('.id_customer').val()=='' || curRow.find('.id_customer').val()==null)
...

{
	...
	let id_issue_receipt = (items.id_issue_receipt!=undefined && items.id_issue_receipt!='' ? items.id_issue_receipt :'');

	let bill_no = (items.bill_no!=undefined && items.bill_no!='' ? items.bill_no :'');

	let total_amount = (items.total_amount!=undefined && items.total_amount!='' ? items.total_amount :'');

	let adj_amount = (items.adj_amount!=undefined && items.adj_amount!='' ? items.adj_amount :'');

	let cash_pay = (items.cash_pay!=undefined && items.cash_pay!='' ? items.cash_pay :'');

	let total_cash_pay = (items.total_cash_pay!=undefined && items.total_cash_pay!='' ? items.total_cash_pay :'');

	trHtml='<tr>'

	+'<td><input type="checkbox" class="bill_id_issue_receipt"  name="adv_adj[id_issue_receipt]" value="'+id_issue_receipt+'" '+(adj_amount!='' ? 'checked' :'')+'><input type="hidden" class="cash_pay"  value="'+cash_pay+'"><input type="hidden" class="total_cash_pay"  value="'+total_cash_pay+'"></td>'

	+'<td><div class="adv_bill_no">'+bill_no+'</div></td>'

	+'<td><div class="advance_amount" >'+total_amount+'</div></td>'

	+'<td><input type="number" class="form-control adj_amount" name="adv_adj[adj_amount]" value="'+adj_amount+'" '+(adj_amount!='' ? '' :'readonly')+'></td>'
...
function create_new_empty_payment_row(pay_data=[]) {
	...
	$.each(payment_mode_details,function(k,pm){
		...
		paymode+= "<option value='" + pm.short_code + "' "+selected+" >" + pm.short_code + "</option>";
	...
	$.each(bank_details,function(k,bnk){
		...
		bank+= "<option value='" + bnk.id_bank + "' "+selected+" >" + bnk.acc_number + "</option>";
	...
	$.each(payment_device_details,function(k,val){
		...
		device+= "<option value='" + val.id_device + "' "+selected+" >" + val.device_name + "</option>";
	...
	var net_bank_type = '<option value="">Select Net Bank Type</option>'
						...
						+'<option value="1" '+(pay_data.net_bank_type==1 ? 'selected' :'')+' >RTGS</option>'

						+'<option value="2" '+(pay_data.net_bank_type==2 ? 'selected' :'')+'>IMPS</option>'

						+'<option value="3" '+(pay_data.net_bank_type==3 ? 'selected' :'')+' >UPI</option>';

	

	var card_name = '<option value="">Select Card</option>'

	+'<option value="1"  '+(pay_data.card_name==1 ? 'selected' :'')+' >RuPay</option>'

	+'<option value="2" '+(pay_data.card_name==2 ? 'selected' :'')+' >VISA</option>'

	+'<option value="3" '+(pay_data.card_name==3 ? 'selected' :'')+' >Mastro</option>';

	+'<option value="4" '+(pay_data.card_name==4 ? 'selected' :'')+' >Master</option>';
	...
	trHtml = '<tr>'
			  ...
			  +'<td><input type="number" class="recd_amt form-control" value="'+pay_data.recd_amt+'" style="width: 100px;"></td>'

			  +'<td><select class="card_name form-control"  value="" style="width: 100px;" '+(pay_data.payment_mode=='CC' || pay_data.payment_mode=='DC' ? '' :'disabled')+'>'+card_name+'</select></td>'

			  +'<td><select class="device_type form-control"  value="" style="width: 100px;" '+(pay_data.payment_mode=='CC' || pay_data.payment_mode=='DC' ? '' :'disabled')+'>'+device+'</select></td>'

			  +'<td><select class="net_bank_type form-control"  value="" style="width: 100px;" '+(pay_data.net_bank_type!='' ? '' :'disabled')+'>'+net_bank_type+'</select></td>'

			  +'<td><input type="number" class="ref_no form-control" value="'+pay_data.ref_no+'" style="width: 100px;" '+(pay_data.ref_no!='' ? '' :'disabled')+'></td>'

			  +'<td><input type="number" class="approval_no form-control" value="'+pay_data.approval_no+'" style="width: 100px;" '+(pay_data.payment_mode=='CC' || pay_data.payment_mode=='DC' ? '' :'disabled')+' ></td>'
...
$('#payment_detail_modal  #close_payment_details').on('click', function(){
	...
	$('#payment_detail_modal .modal-body').find('#bill_split_adv_adj tbody').empty();
...
$('#payment_detail_modal  #save_payment_details').on('click', function(){
...

	{
...

		{
			...
			$('#payment_detail_modal .modal-body #pay_details_table > tbody  > tr').each(function(index, tr) {
			...
			$('.'+catRow).find('.split_recd_amt').val(parseFloat(total_recd_amt).toFixed(2));

			$('.'+catRow).find('.split_payment_details').val(payment_details.length>0 ? JSON.stringify(payment_details):'');

			

			calculateSaleBillSplitRowTotal();

			$('#payment_detail_modal .modal-body').find('#pay_details_table tbody').empty();
...

	{
		...
		$('#payment_detail_modal .modal-body #bill_split_adv_adj > tbody  > tr').each(function(index, tr) {
			...
			advance_adj.push({
				...
				'is_receipt_select'		:($(this).find('.bill_id_issue_receipt').is(':checked') ? 1:0)

			});

		});

		$('.'+catRow).find('.cus_advance_details').val(advance_adj.length>0 ? JSON.stringify(advance_adj):'');
...

{
	...
	total_bill_split_recd_amt = parseFloat(total_bill_split_net_banking_amt)+parseFloat(total_bill_chq_amt)+parseFloat(total_bill_split_card_amt)+parseFloat(total_bill_split_cash_amt)+parseFloat(total_bill_split_adv_adj_amt);

	total_bill_split_blc_amt = parseFloat(receive_amount)-parseFloat(total_bill_split_recd_amt);

	$('#total_bill_split_net_banking_amt').val(parseFloat(total_bill_split_net_banking_amt).toFixed(2));

	$('#total_bill_chq_amt').val(parseFloat(total_bill_chq_amt).toFixed(2));

	$('#total_bill_split_card_amt').val(parseFloat(total_bill_split_card_amt).toFixed(2));

	$('#total_bill_split_cash_amt').val(parseFloat(total_bill_split_cash_amt).toFixed(2));

	$('.total_bill_split_recd_amt').html(parseFloat(total_bill_split_recd_amt).toFixed(2));

	$('.total_bill_split_blc_amt').html(parseFloat(total_bill_split_blc_amt).toFixed(2));

	$('#total_bill_split_adv_adj_amt').val(parseFloat(total_bill_split_adv_adj_amt).toFixed(2));
...
$(document).on("keyup",".bill_wastage", function() {
	...
	let bill_wastage = (curRow.find(".bill_wastage").val()!='' ? curRow.find(".bill_wastage").val():0);
			...
			$("#adv_trns_to_cust").autocomplete(
				{
					...
					response: function (e, i) {
						...
						if (adv_trns_to_cust != "") {
							if (i.content.length === 0) {
								$(".tocustomerAlert").html('<p style="color:red">Enter a valid customer name / mobile</p>');
							} else {
...
function get_advance_detail(id_fromcustomer) {
	...
	$.ajax({
		...
		dataType: "json",
		method: "POST",
		data: { 'bill_cus_id': id_fromcustomer },
		success: function (data) {
			if(data.length > 0){
				...
				$.each(data, function (key, items) {
						html += '<tr>'
						+ '<td><input type="checkbox" class="adv_trans_id_issue_receipt"  name="adv_trans[id_issue_receipt]" value="' + items.id_issue_receipt + '"><input type="hidden" class="id_ret_wallet" value="' + items.id_ret_wallet + '"><input type="hidden" class="adv_cash_paid" value="' + items.cash_pay + '"></td>'
						+ '<td><div class="adv_bill_no" value="' + items.bill_no + '">' + items.bill_no + '</div></td>'
						+ '<td><div class="at_rcpt_bal_amount" >' + items.amount + '</div></td>'
...
$('#submit_advance_transfer,.submit_advance_transfer').on('click', function () {
	...
	if ($('#id_branch').val() == 0 || $('#id_branch').val() == undefined || $('#id_branch').val() == null) {
		...
		$.toaster({ priority: 'danger', title: 'Warning!', message: '' + "</br>Please Enter Transfer From Customer.." });
		return false;
	}
	else if ($('#to_cus_id').val() == '') {

		$.toaster({ priority: 'danger', title: 'Warning!', message: '' + "</br>Please Enter Transfer To Customer.." });
		return false;
	}
	...
	$('#advance_trns_list > tbody  > tr').each(function (index, tr) {
		if ($(this).find('.adv_trans_id_issue_receipt').is(':checked')) {
			transfered_amount_Details.push({
				'adv_trans_id_issue_receipt': $(this).find('.adv_trans_id_issue_receipt').val(),
				'transfer_amount': $(this).find('.transfer_amount').val(),
				'tot_blc_amount': $(this).find('.tot_blc_amount').val(),
				'adv_trans_otp': $('#adv_trans_otp').val(),

			});
			console.log("transfered_amount_Details : " + JSON.stringify(transfered_amount_Details));
			$('#advance_transfer').val(transfered_amount_Details.length > 0 ? JSON.stringify(transfered_amount_Details) : '');
		}
	});
	$.ajax({
		type: "POST",
		...
		dataType: 'JSON',
		data: { 'send_resend': $('#send_resend').val(),'transfer_amount': $('#advance_transfer').val(), 'tot_transfer_amount': $('#total_transfered_amt').val(), 'id_branch': $('#branch_select').val() != '' || $('#branch_select').val() != undefined || $('#branch_select').val() != null ? $('#branch_select').val() : $('#branch_filter').val(), 'is_eda': $('#is_eda').val(), 'to_cus_id': $('#to_cus_id').val() },
		success: function (data) {
			if (data.status == true) {
				window.open(base_url + 'index.php/admin_ret_billing/receipt/receipt_print/' + data['id'], '_blank');
				$.toaster({ priority: 'success', title: 'Success!', message: '' + "</br> Transfer Amount  SuccessFully..." });
				window.location.reload();
				$("div.overlay").css("display", "none");
			}
			else {
				$.toaster({ priority: 'warning', title: 'Success!', message: '' + "</br> Unable to Transfer Amount" });
				window.location.reload();
				$("div.overlay").css("display", "none");
			}
		}
...
$(document).on('keyup', ".transfer_amount", function (e) {
	...
	$(' #advance_trns_list > tbody  > tr').each(function (index, tr) {
		var row = $(this).closest('tr');
		if (row.find('.adv_trans_id_issue_receipt').is(':checked')) {
			total_transfered_amt += (isNaN(row.find('.transfer_amount').val()) || (row.find('.transfer_amount').val() == '') ? 0 : parseFloat(row.find('.transfer_amount').val()));
		}
	});

	if (row.find('.transfer_amount').val() != '' && row.find('.transfer_amount').val() > 0) {
		row.find('.adv_trans_id_issue_receipt').prop('checked', true);
		if (parseFloat(at_rcpt_bal_amount) < parseFloat(row.find('.transfer_amount').val())) {
			row.find('.transfer_amount').val(0);
			row.find('.tot_blc_amount').val(0);
			row.find('.adv_trans_id_issue_receipt').prop('checked', false);
			row.find('.transfer_amount').attr('readonly', true);
			$.toaster({ priority: 'danger', title: 'Warning!', message: '' + "</br>" + "The Entered Transfer Amount Is more than the Receipt Balance value" });
		}
		else {
			tot_balance_amount = parseFloat(at_rcpt_bal_amount) - parseFloat(row.find('.transfer_amount').val());
			row.find('.adv_trans_id_issue_receipt').prop('checked', true);
			row.find('.tot_blc_amount').val(parseFloat(tot_balance_amount).toFixed(2));
		}
	} else {
		row.find('.adv_trans_id_issue_receipt').prop('checked', false);
		row.find('.transfer_amount').val(0);
...
function calculate_advance_transfer_amount() {
	...
	$('#advance_trns_list > tbody  > tr').each(function (index, tr) {
		var row = $(this).closest('tr');
		if (row.find('.adv_trans_id_issue_receipt').is(':checked')) {
			transfer_amt += (isNaN(row.find('.transfer_amount').val()) || (row.find('.transfer_amount').val() == '') ? 0 : parseFloat(row.find('.transfer_amount').val()));
			total_balance_amt += (isNaN(row.find('.tot_blc_amount').val()) || (row.find('.tot_blc_amount').val() == '') ? 0 : parseFloat(row.find('.tot_blc_amount').val()));
		}
	});
...
function at_send_otp() {
	...
	var send_resend = $('#send_resend').val();
	$.ajax({
		url: base_url + "index.php/admin_ret_billing/adtrnssendotp?nocache=" + my_Date.getUTCSeconds() + '' + my_Date.getUTCMinutes() + '' + my_Date.getUTCHours(),
		type: "POST",
		data: { 'mobile': mobile, 'send_resend': send_resend },
		dataType: "json",
		async: false,
...
function verify_advance_transfer_otp() {
	...
	my_Date = new Date();
	$.ajax({
		url: base_url + "index.php/admin_ret_billing/verify_advance_transfer_otp?nocache=" + my_Date.getUTCSeconds() + '' + my_Date.getUTCMinutes() + '' + my_Date.getUTCHours(),
