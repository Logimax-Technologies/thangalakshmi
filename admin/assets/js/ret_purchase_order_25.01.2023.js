...
function set_po_order_item_preview()
{
    ...
    var trHtml='<tr>'
                +'<td><input type="hidden" class="po_item_id" name="order_item[po_item_id][]" value='+po_item_id+'><input type="hidden" class="id_category" name="order_item[id_category][]" value="'+$('#select_category').val()+'"><input type="hidden" class="id_order_no" name="order_item[po_order_no][]" value="'+$('#select_po_no').val()+'">'+$("#select_category option:selected").text()+'</td>'
                ...
                +'<td><input type="hidden" class="total_diawt" value="'+parseFloat(dia_weight).toFixed(3)+'">'+parseFloat(dia_weight).toFixed(3)+'</td>'
                ...
                +'<td>'+$('#other_metal_amount').val()+'<input type="hidden" class="other_metal_amt" value='+$('#other_metal_amount').val()+'><input type="hidden" class="other_metal_details" name="order_item[other_metal_details][]" value='+$('#other_metal_details').val()+'></td>'
                +'<td><input type="hidden" class="total_payable_amt" name="order_item[total_payable_amt]" value="'+$("#item_cost").val()+'"><input type="hidden" class="cgst_cost" name="order_item[cgst_cost]" value="'+cgst_cost+'"><input type="hidden" class="sgst_cost" name="order_item[sgst_cost]" value="'+sgst_cost+'"><input type="hidden" class="igst_cost" name="order_item[igst_cost]" value="'+igst_cost+'"><input type="hidden" class="total_tax_rate" name="order_item[total_tax_rate]" value="'+total_tax_rate+'"><input type="hidden" class="ratecaltype" name="order_item[ratecaltype][]" value="'+purmode+'"><input type="hidden" class="rate_per_gram" name="order_item[rate_per_gram][]" value="'+$('#rate_per_gram').val()+'"><input type="hidden" name="order_item[item_cost][]" value="'+$('#item_cost').val()+'"><input type="hidden" name="order_item[is_halmerked][]" value="'+is_halmerked+'"><input type="hidden" name="order_item[is_rate_fixed][]" value="'+is_rate_fixed+'">'+$("#item_cost").val()+'</td>'
...
function calculate_purchase_order_item_total()
{
    ...
    var tot_stone_price=0;
     $('#item_detail > tbody tr').each(function(idx, idx){
        ...
        total_diawt+=parseFloat(row.find('.total_diawt').val());
    });
...
function calculate_purchase_item_cost()
{
    ...
    var ratecaltype = ($('#purchase_mode').val()!='' ? $('#purchase_mode').val():$('#select_product option:selected').attr('data-purmode')); //2-Flexible,2-Fixed
   ...
   /* var mctax                   = purchase_type == 2 ? 3 : $('#is_cus_repair_order').val() == 1 ? 18 : (is_metal_issue==1 ? 5 : 3);
    ...
    {
        var purewt = format(parseFloat((parseFloat(net_wt) * (parseFloat(purity) + parseFloat(wastage))) / 100)).replace(/,/g, '');
        var total_mc_value  = (mc_type==1 ? parseFloat(parseFloat(mc_value)*parseFloat(tot_gwt)).toFixed(2) :parseFloat(mc_value).toFixed(2));
        if(karigar_type == 1)
        {
        ...
        if(selData!='' && selData!='[]')
        {
            ...
            $(charges).each(function(key,val)
            {
                ...
                $(charges_list).each(function(idx, charges){
                    ...
                    options += "<option value='"+charges.id_charge+"' "+(charges ? (charges.id_charge == val.charge_id ? 'selected' : '') : '')+">"+val.name_charge+"</option>";
                ...
                let new_row = "<tr class='ch_"+row_cls+"'><td class='sno'>"+sno+"</td><td><select class='form-control chargesType'><option value=''>--Select--</option>"+options+"</select></td><td><input type='text' value='"+(charges ? (val.charge_value == undefined ? 0 : val.charge_value) : '')+"' class='form-control chargesValue' /></td><td class='chargeModal_buttons'></td></tr>";
...
$('#cus_stoneModal  #update_return_stn_details').on('click', function(){
	if(validateStoneCusItemDetailRow())
	{
    	...
    	$('#cus_stoneModal .modal-body #estimation_stone_cus_item_details > tbody  > tr').each(function(index, tr) {
    		...
    		{
    		    stone_wt+=parseFloat($(this).find('.stone_wt').val()/5);
    		}
    		else
    		{
...
function validateGrnItemDetailRow()
{
	...
	$('#grn_item_details > tbody  > tr').each(function(index, tr) {
		if($(this).find('.category_select').val() == "" || $(this).find('.pcs').val() == ""  ||  $(this).find('.rate_per_gram').val() == "" || $(this).find('.rate_per_gram').val() == 0 || $(this).find('.item_cost').val() == "" || $(this).find('.item_cost').val() == 0 ){
			validate = false;
		}
