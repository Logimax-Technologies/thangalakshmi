...
// 	var id_branch=$('#branch_select').find(":selected").val();
 ...
 function get_payment(data){
 	...
 	var filename="<b><span style='font-size:15pt;'> " + cmp_name + " | Admin</span></b></br>"+"<span style=font-size:13pt;>&nbsp;Selected Date&nbsp;:&nbsp;"+from+"</span><span style=font-size:13pt;>&nbsp;-&nbsp;"+to+"</span>";
...
function customer_wise_payment(id_branch="",id_scheme="")
{
	my_Date = new Date();
	 $("div.overlay").css("display", "block"); 
...
if(data.accounts.summary!=null){
	...
	trHTML += '<table  style="margin-left:15px;"><thead><tr><th>Scheme</th><th style="text-align:center;padding:6px" width="25%">Customer Count</th><th style="text-align:right;">Amount</th></tr></thead><tbody><tr>';
   var  Tcount=0
   var  Tamount=0
   var pending_due_summary=[]
    $.each(data.accounts.summary, function (key, customer) {
...
+ '<td style="text-align: center;">'+customer.length+'</td>'
...
trHTML+='</tbody><tfoot style="border-top: 15px solid #ecf0f5;"><tr></tr><tr><th style="font-weight: bold;"> Total </th><td  style="text-align: center;font-weight: bold;">'+Tcount+'</td><td  style="text-align: right;font-weight: bold;">'+indianCurrency.format(Tamount)+'</td></tr></tfoot></table>';
...
+"<b><span style='font-size:15pt;'>Pending Dues report</span></b></br>"
...
select_date+='</br><div><table class="table table-bordered table-striped text-center" style="border: 1px solid black;border-collapse: collapse; width:40%;margin-left:300px;">'+
					...
					if(pending_due_summary.length ==0 ){
						...
						select_date+= '<tr><th>Total</th><th>'+pending_due_summary[last].Tcount+'</th><th>'+indianCurrency.format(pending_due_summary[last].TAmount)+'</th></tr>';
...
'<td style="text-align:left;">'+items.id_scheme_account+'</td>'+

'<td style="text-align:right;">'+items.id_customer+'</td>'+

'<td style="text-align:left;">'+items.customer_name+'</td>'+

'<td style="text-align:right;">'+items.scheme_name+'</td>'+


'<td style="text-align:left;">'+items.account_name+'</td>'+
...
'<td style="text-align:left;">'+items.start_date+'</td>'+
'<td style="text-align:left;">'+items.last_paid_date+'</td>'+
'<td style="text-align:right;">'+indianCurrency.format(items.paid_amount)+'</td>'+
'<td style="text-align:right;">'+items.total_installments+'</td>'+
'<td style="text-align:right;">'+items.paid_ins+'</td>'+
'<td style="text-align:left;">'+items.unpaid_month+'</td>'+
...
$("#total_account").text(count);
	...
	$('#customer_pay_details > tbody').html('');
}
...
oTable = $('#customer_pay_details').dataTable({
                   ...
                   "buttons": [
                       ...
                       {  
                           extend: 'print',
                           footer: true,
                           title:'',messageTop:select_date,orientation: 'landscape',
                                customize: function ( win ) {
...

                            {
                                ...
                                title: 'Pending Dues Report '+$('#rpt_payments1').html()+' - '+$('#rpt_payments2').html(),
			...
			if($("input[name='id_transaction[]']:checked").val())
			{
				...
				$("#intertable_translist tbody tr").each(function(index, value){
					if($(value).find("input[name='id_transaction[]']:checked").is(":checked")){ 
						data = { 'id_transaction'   : $(value).find(".id_transaction").val(), 
...
function get_closed_acc_list()
{
	...
	postData = {'from_date':$('#rpt_payments1').text(),'to_date':$('#rpt_payments2').text(),'id_scheme':$('#scheme_select').val(),'id_employee':$('#emp_select').val(),'close_id_branch':($('#branch_filter').val()!='' && $('#branch_filter').val()!=undefined ? $('#branch_filter').val(): $("#close_branch_select").val())},
	 $("div.overlay").css("display", "block"); 
	$.ajax({
...
//Customer Account Details
	...
	var from_date=$('#rpt_payments1').text();
	var to_date=$('#rpt_payments2').text();
	var title_details=get_title(from_date,to_date,'Closed Account List');
	var count=0;
	...
	$("div.overlay").css("display", "none");  
					...
					title_summ+='<table class="table table-bordered text-center" style="width:800px;margin:0 auto;">'+
					...
					{
						srHTML+='<tr>'+
						'<td style="text-align:left;">'+sum_data.scheme_name+'</td>'+

						'<td style="text-align:right;">'+sum_data.acc_count+'</td>'+

						'<td style="text-align:right;">'+indianCurrency.format(sum_data.closing_amount)+'</td>'+

						'<td style="text-align:right;">'+parseFloat(sum_data.closing_weight).toFixed(3)+'</td>'+
						'</tr>';
					...
					srHTML+='<tr style="background: #d2d6de;">'+
						'<td style="text-align:left;font-weight:bold">Total : </td>'+

						'<td style="text-align:right;font-weight:bold">'+sum_tot_acc+'</td>'+

						'<td style="text-align:right;font-weight:bold">'+indianCurrency.format(sum_tot_amt)+'</td>'+

						'<td style="text-align:right;font-weight:bold">'+parseFloat(sum_tot_wgt).toFixed(3)+' g</td>'+
						'</tr>';
					...
					$.each(account,function(key,ac_data)
					{
						trHTML+='<tr>'+
										...
										$.each(ac_data,function(index,items)
										{
											...
											var url = base_url+'/index.php/reports/payment/account/'+items.id_scheme_account;
											//for account number
										if(items.scheme_wise_acc_no==3)
											{
												var acc_num='<a href='+url+' target="_blank">'+items.branch_code+items.code+'-'+items.scheme_acc_number+'</a>'
											}
											else
											{
												if(items.has_lucky_draw==1)

												{

													
													var acc_num='<a href='+url+' target="_blank">'+items.scheme_group_code+' '+items.scheme_acc_number+'</a>'
...

												{
													...
													var acc_num='<a href='+url+' target="_blank">'+items.code+'  '+items.scheme_acc_number+'</a>'
											...
											if(items.bill_id!='' && items.bill_id!=null)
												{
													var bill_num='<a href='+bill_url+' target="_blank">'+items.bill_no+'</a>';
												...
												{
													var closing_bal=indianCurrency.format(items.closing_balance);
													sub_close_amt+=parseFloat(items.closing_balance);
												}
												else
												{
													var closing_bal= indianCurrency.format(items.closing_amount);
												...
												//amountpaid 
												sub_cus_paid+= parseFloat(parseFloat(items.closing_paid_amt)+parseFloat(items.balance_amount)-parseFloat(items.closing_benefits));						    
												//pre close charge
												sub_pre_close+=parseFloat(items.closing_add_chgs);
												//Bonus
												sub_bonus+=parseFloat(items.closing_benefits);
											trHTML+='<tr>'+
											'<td>'+items.id_scheme_account+'</td>'+
											...
											'<td>'+indianCurrency.format(items.closing_amount)+'</td>'+
											'<td>'+parseFloat(items.closing_weight).toFixed(3)+'</td>'+
											'<td>'+items.employee_closed+'</td>'+
											'<td>'+closing_branch+'</td>'+
											'<td>'+items.closing_date+'</td>'+
											'<td>'+items.paid_installments+'/'+items.total_installments+'</td>'+
											
											'<td>'+indianCurrency.format(parseFloat(parseFloat(items.closing_paid_amt)+parseFloat(items.balance_amount)-parseFloat(items.closing_benefits)).toFixed(2))+'</td>'+
											'<td>'+indianCurrency.format(items.closing_add_chgs)+'</td>'+
											'<td>'+indianCurrency.format(items.closing_benefits)+'</td>'+
											// '<td style="text-align:right;">'+indianCurrency.format(items.pay_amount)+'</td>'+
											'</tr>';
										...
										trHTML+='<tr>'+
												...
												'<td></td>'+  
												'<td></td>'+ 
												'<td style="font-weight:bold;color:#605ca8;";>SUB TOTAL : </td>'+  
												'<td></td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(sub_close_amt)+'</td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+parseFloat(sub_close_wgt).toFixed(3)+' g</td>'+  
												...
												'<td></td>'+  
												'<td></td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(sub_cus_paid)+'</td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(sub_pre_close)+'</td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(sub_bonus)+'</td>'+  
					...
					trHTML+='<tr>'+
												...
												'<td></td>'+  
												'<td></td>'+  
												'<td style="font-weight:bold;color:#605ca8; ";>GRAND TOTAL : </td>'+  
												'<td></td>'+ 
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(grand_close_amt)+'</td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+parseFloat(grand_close_wgt).toFixed(3)+' g</td>'+  
												...
												'<td></td>'+  
												'<td></td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(grand_cus_paid)+'</td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(grand_pre_close)+'</td>'+  
												'<td style="font-weight:bold;color:#605ca8;">'+indianCurrency.format(grand_bonus)+'</td>'+  
					...
					if ( ! $.fn.DataTable.isDataTable( '#closed_list' ) )
					{
...

								{
											...
											title: 'Closed Account List '+from_date+' - '+to_date,
											...
											"columnDefs": 
												[
													...
													{
														targets: [0,1,2,3,4,5,6,7,10,11,12,13], 
														className: 'dt-left'
													},
													{
														targets: [8,9,14,15,16], 
														className: 'dt-right'
													},
...
indianCurrency.format(data.online_total)
   ...
   // title+='<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
	...
	/* scheme wise report summary print
		Created by Kanaga Durga 10/01/2023 starts here */

		
						/*		console.log(data.scheme_data);
								var sc_data=data.scheme_data;
									if(sc_data==null){
										// offHTML+='<div style="text-align: center;"><strong><span >No data available</span></strong></div>';
					
										// $('#report_scheme_summary_title').html(offHTML);
									}
									else
									{
										var total_amount=0;
										...
										//	alert(count);
										title+='</br><div"><table class="table table-bordered text-center" style="border: 1px solid black;">'+
										'<tr style="border: 1px solid black;">'+
										'<th style="border: 1px solid black;">Sno</th>'+
										'<th style="border: 1px solid black;">Scheme Name</th>'+
										'<th style="border: 1px solid black;" colspan="'+count+'">Mode Wise Details</th>'+
										'<th style="border: 1px solid black;">Total Weight</th>'+
										'<th style="border: 1px solid black;">Total Amount</th>'+
										'</tr>';
										// '<tr><th>Scheme Name </th><th>Payment Details</th></tr>';
										$.each(sc_data,function(key,val)
										{ 
										...
										var sc_nm=key;
										var mode_data=val;
										title+='<tr style="border: 1px solid black;">'+
										'<td style="border: 1px solid black;">'+sno+'</td>'+
										'<td style=" text-align: center;border: 1px solid black;"><span ><strong>'+sc_nm+'</strong></span></td>';
										
											
										$.each(mode_data,function(key,val)
										{
											//alert(mode_data.length);
											var mode_data_count=mode_data.length;
											if(mode_data_count==count)
											{
											var mode=val.pay_mode!='-'?val.pay_mode:'-';
											var amount=val.pay_amt!='-'?indianCurrency.format(val.pay_amt):'-';

											total_amount=total_amount+parseInt(val.pay_amt);
											title+='<td style="border: 1px solid black;">'+mode+
											'</br>'+amount+'</td>';
											//onHTML+='<table><tr>ss</br>ss</tr></table>';
											
											
											total_weight=val.weight!=''?weight=val.weight:'-';
											}
											else
											{
												var diff=count-mode_data_count;
												//alert(diff);
												var mode=val.pay_mode!='-'?val.pay_mode:'-';
												var amount=val.pay_amt!='-'?indianCurrency.format(val.pay_amt):'-';
												...
												{
													title+="<td style='border: 1px solid black;'>-</td>";
												}
										...
										title+='<td style="border: 1px solid black;" >'+total_weight+'</td>'+
										'<td style="border: 1px solid black;">'+indianCurrency.format(total_amount)+'</td>';
										sno++;
		...
		/* scheme wise report summary print
		Created by Kanaga Durga 10/01/2023 End here */
	 		...
	 		$('#report_payment_daterange').dataTable().fnDestroy();  
                ...
                $.each(data.schemes,function(key,payment){
                    ...
                    $.each(payment,function(key,items){
                        ...
                        var receipt_no = '';
                        if(items.is_lucky_draw == 1){
	                	    scheme_acc_number = items.scheme_group_code+' '+items.scheme_acc_number;
	                	}else{ 

    	                	if(items.schemeaccNo_displayFrmt == 0){   //only acc num
	                        
	                            scheme_acc_number = items.scheme_acc_number;
	                        
    	                    }else if(items.schemeaccNo_displayFrmt == 1){ //based on acc number generation setting
    	                        
    	                        if(items.scheme_wise_acc_no==0){
        							scheme_acc_number = items.scheme_acc_number;
        						}else if(items.scheme_wise_acc_no==1){
        							scheme_acc_number = items.acc_branch+'-'+items.scheme_acc_number;
        						}else if(items.scheme_wise_acc_no==2){
        							scheme_acc_number = items.code+'-'+items.scheme_acc_number;
        						}else if(items.scheme_wise_acc_no==3){
        							scheme_acc_number = items.code+''+items.acc_branch+'-'+items.scheme_acc_number;
        						}else if(items.scheme_wise_acc_no==4){
        							scheme_acc_number = items.start_year+'-'+items.scheme_acc_number;
        						}else if(items.scheme_wise_acc_no==5){
        							scheme_acc_number = items.start_year+''+items.code+'-'+items.scheme_acc_number;
        						}else if(items.scheme_wise_acc_no==6){
        							scheme_acc_number = items.start_year+''+items.code+''+items.acc_branch+'-'+items.scheme_acc_number;
        						}
    	                    }else if(items.schemeaccNo_displayFrmt == 2){  //customised
    	                        scheme_acc_number = items.scheme_acc_number;
    	                    }
	                	}
	                	...
	                	if(items.receiptNo_displayFrmt == 0){   //only acc num
                        
                            receipt_no = items.receipt_no;
                        
	                    }else if(items.receiptNo_displayFrmt == 1){ //based on acc number generation setting
	                        
	                        if(items.scheme_wise_receipt==1){
    							receipt_no = items.receipt_no;
    						}else if(items.scheme_wise_receipt==2){
    							receipt_no = items.acc_branch+'-'+items.receipt_no;
    						}else if(items.scheme_wise_receipt==3){
    							receipt_no = items.code+'-'+items.receipt_no;
    						}else if(items.scheme_wise_receipt==4){
    							receipt_no = items.code+''+items.acc_branch+'-'+items.receipt_no;
    						}else if(items.scheme_wise_receipt==5){
    							receipt_no = items.start_year+'-'+items.receipt_no;
    						}else if(items.scheme_wise_receipt==6){
    							receipt_no =  items.start_year+''+items.code+''+items.acc_branch+'-'+items.receipt_no;
    						}
	                    }else if(items.receiptNo_displayFrmt == 2){  //customised
	                        receipt_no = items.receipt_no;
	                    }
                        ...
                        trHTML+='<tr>'+
                                ...
                                //  '<td ><a href='+acc_ledger+' target="_blank">'+(items.has_lucky_draw==1 ? items.scheme_group_code+' '+items.scheme_acc_number : items.code+' '+items.scheme_acc_number)+'</td>'+
                                    '<td style="text-align:right;"><a href='+receipt_url+' target="_blank">'+receipt_no+'</td>'+
                                    '<td style="text-align:left;">'+items.name+'</td>'+								
                                    '<td style="text-align:left;">'+scheme_acc_number+'</td>'+
                                    '<td style="text-align:right;">'+items.paid_installments+'</td>'+
                                    '<td style="text-align:left;">'+items.date_payment+'</td>'+
                                //  '<td>'+items.approval_date+'</td>'+
                                    '<td style="text-align:left;">'+items.payment_mode+'</td>'+
                                    '<td style="text-align:right;">'+items.metal_rate+'</td>'+
                                  '<td style="text-align:right;">'+parseFloat(items.metal_weight).toFixed(3)+'</td>'+
                                    '<td style="text-align:right;">'+parseFloat(parseFloat(items.payment_amount)-parseFloat(items.discountAmt)).toFixed(2)+'</td>'+
                                //  '<td>'+items.discountAmt+'</td>'+
                  ...
                  /* trHTML+='<tr>'+
                                ...
                                '<td><strong>'+parseFloat(metal_weight).toFixed(2)+'</strong></td>'+
                                '<td><strong>'+parseFloat(paid_amount).toFixed(2)+'</strong></td>'+
                                '<td><strong>'+parseFloat(bonus_amount).toFixed(2)+'</strong></td>'+
                                '<td><strong>'+parseFloat(parseFloat(paid_amount)+parseFloat(bonus_amount)).toFixed(2)+'</strong></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
                            ...
                            trHTML+='<tr>'+
                                ...
                                '<td></td>'+  '<td></td>'+
                                '<td><strong>SUB TOTAL</strong></td>'+
                                '<td style="text-align:right;">'+parseFloat(metal_weight).toFixed(3)+'</td>'+ 
                                '<td style="text-align:right;"><strong>'+indianCurrency.format(parseFloat(paid_amount).toFixed(2))+'</strong></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
                ...
                trHTML+='<tr>'+
                                ...
                                '<td style="text-align:right;">'+parseFloat(total_metal_weight).toFixed(3)+'</td>'+ 
               ...
               /* trHTML+='<tr>'+
                                ...
                                '<td></td>'+
                                '<td><strong>GRAND TOTAL</strong></td>'+  
                                '<td><strong>'+parseFloat(total_metal_weight).toFixed(3)+'</strong></td>'+
                                '<td><strong>'+parseFloat(total_pay_amount).toFixed(2)+'</strong></td>'+
                                '<td><strong>'+parseFloat(total_bonus_amount).toFixed(2)+'</strong></td>'+
                                '<td><strong>'+parseFloat(parseFloat(total_pay_amount)+parseFloat(total_bonus_amount)).toFixed(2)+'</strong></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
...
function set_schemewise_table(data_array)
{
	...
	({
        ...
        "aoColumns": [
			...
			{ "mDataProp": function ( row, type, val, meta )
			{
				return(indianCurrency.format(row.total_amount));
			}
			},
			{
				"mDataProp": null,
		...
		"footerCallback": function( row, data, start, end, display )
			{
				if(data.length>0)
				{
					...
					for( var i=0; i<=data.length-1;i++)
					{
						...
						$( api.column(4).footer() ).html(indianCurrency.format(total_amount));
                ...
                if(data.offline_total == null){
                    ...
                    $('#offline_modewise').html(offHTML);
        			     ...
        			     $.each(data.mode_wise.offline,function(key,val){
            			     ...
            			     if(mode != null && val.offline_amt != null){    
            			      offHTML+=' <div class="row">'+
                    			 '<div class="col-md-6" style="text-align: left;">'+mode+'</div>'+
                    			 '<div class="col-md-2"><lable> : </lable></div>'+
                    			 '<div class="col-md-4" style="text-align: right;"><strong><span>'+indianCurrency.format(val.offline_amt)+'</span></strong></div>'+
                			   '</div> ' ;
            			     }
        			     ...
        			     offHTML+='<div class="row"></div><hr>'+
        							...
        							'<div class="col-md-2"><lable> : </lable></div>'+
        							'<div class="col-md-4" style="text-align: right;"><strong><span>'+indianCurrency.format(data.offline_total)+'</span></strong></div>'+
        							'</div>';
            			   $('#offline_modewise').html(offHTML);
                }		   
          ...
          // online  	
                ...
                if(data.online_total == null){
            	    ...
            	    $.each(data.mode_wise.online,function(key,val){
        			     ...
        			     if(mode != null && val.online_amt != null){  
            			    ...
            			    onHTML+=' <div class="row">'+
                    			 '<div class="col-md-6" style="text-align: left;">'+mode+'</div>'+
                    			 '<div class="col-md-2"><lable> : </lable></div>'+
                    			 '<div class="col-md-4" style="text-align: right;"><strong><span>'+indianCurrency.format(val.online_amt)+'</span></strong></div>'+
                			   '</div> ' ;
        			     }
                    ...
                    onHTML+='<div class="row"></div><hr>'+
            							...
            							'<div class="col-md-6" style="text-align: left;"> <strong>Total Payment</strong></div>'+
            							'<div class="col-md-2"><lable> : </lable></div>'+
            							'<div class="col-md-4" style="text-align: right;"><strong><span>'+indianCurrency.format(data.online_total)+'</span></strong></div>'+
            							'</div>';
                	$('#online_modewise').html(onHTML);
                }
    ...
    function getGiftIssuedList()
    ...
    
    		{
    			...
    			$.ajax({
    				...
    				data: ({'from_date':$('#rpt_payments1').html(),'to_date' :$('#rpt_payments2').html(),'id_scheme':$('#scheme_select').val(),'id_metal':$('#metal_select').val(),'id_gift':$('#gift_list').val(),'id_employee':$('#employee_select').val(),'id_branch':($('#branch_filter').val()!='' && $('#branch_filter').val()!=undefined ? $('#branch_filter').val(): $("#id_branch").val())}),
...

{
    ...
    var gift = data.gift;
					...
					$('#gift_report_list').dataTable().fnDestroy(); 
									...
									$("#from_date_selected").text("Fm:"+date1);

									$("#to_date_selected").text("To:" +date2);
					...
					sum_weight=0;
...

							{
								...
								'<td style="text-align:left;">'+items.account_no+'</td>'+
								...
								'<td style="text-align:left;">'+items.gift_id_emp_name+'</td>'+
								...
								'<td style="text-align:right;">'+items.quantity+'</td>'+
...
/*Out Standing Report function starts*/
 ...
 function getSchemeDateRangeList()
{
	...
	$.ajax({
			 ...
			 success:function(data){
                ...
                title+='</br><div"><table class="table table-bordered table-striped text-center" style="border: 1px solid black;border-collapse: collapse; width:65%;margin-left:150px;">'+
		'<thead style="font-size:11pt;">'+
		'<tr><th style="text-align: center;">Scheme-wise Accounts<span></span></th>'+
		'<th><span style="text-align: center;"><span >Group-wise Accounts</span></th></tr>'+
		'</thead>'+
		'<tbody style="font-size:11pt;"></br>';         
    ...
    if(data.scheme_summary == null){
         ...
         title+='<tr><td>';
		var scheme_count=0;
        $.each(data.scheme_summary,function(key,val){
            
            if(val.scheme_name != null && val.scheme_count != null){
               title+= '<span class="pull-left">'+val.scheme_name+'</span><span></span><span class="pull-right" ><strong>'+val.scheme_count+'</strong></span><br>';
	    ...
	    if(val.is_lucky_draw==1){
			...
			title+='<span class="pull-left"><b>'+val.scheme_name+'</b></span><br>';
			$.each(val.group_scheme,function(keys,vals){
			title+= '<span class="pull-left">'+vals.group_code+'</span><span></span><span class="pull-right" ><b>'+vals.count+'</b></span><br>';   
			scheme_grp_count+=parseInt(vals.count);
		});		
	}		 
			}
		});
			...
			title+='</br></tr><tr><td> <strong>Total Account  &nbsp;&nbsp;: &nbsp;&nbsp;'+scheme_count==0? " ":scheme_count+' </strong></td><td> <strong>Total Account  &nbsp;&nbsp;: &nbsp;&nbsp;'+scheme_grp_count+' </strong></td></tr>';
		 ...
		 '</table></div></br>';
		    ...
		    $("#scheme_wise_detail_report > tbody > tr").remove();
	 		...
	 		$('#scheme_wise_detail_report').dataTable().fnDestroy();  
                ...
                trHTML+='<tr>'+
                                ...
                                '<td></td>'+
                                '<td><strong>GRAND TOTAL</strong></td>'+
                                '<td><strong>'+grand_ins+'</strong></td>'+
                                '<td><strong>'+indianCurrency.format(total_pay_amount)+'</strong></td>'+
                                '<td><strong>'+grand_weight+'</strong></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
                                '<td></td>'+
			...
			//Scheme Summary				
			   ...
			   if(data.scheme_summary == null){
					offHTML+='<strong><span>No data available</span></strong>';
				   $('#offline_modewise').html(offHTML);
				   $('#online_modewise').html('');
			   }else{
				   offHTML+=' <div class="row">'+
								'<div class="col-md-5" style="text-align: left;"><strong><span>Scheme</strong></span> </div>'+
								'<div class="col-md-1"><lable> | </lable></div>'+
								'<div class="col-md-2" style="text-align: right;"><strong><span>No.Acc</span></strong></div>'+
								'<div class="col-md-2" style="text-align: right;"><strong><span>Ins</span></strong></div>'+
						...
						$.each(data.scheme_summary,function(key,val){
							...
							if(val.scheme_name != "" && val.scheme_count != ""){    
							 offHTML+=' <div class="row">'+
								'<div class="col-md-5" style="text-align: left;">'+val.scheme_name+'</div>'+
								'<div class="col-md-1"><lable> : </lable></div>'+
								'<div class="col-md-2" style="text-align: right;"><strong><span>'+val.scheme_count+'</span></strong></div>'+
								'<div class="col-md-2" style="text-align: right;"><strong><span>'+val.paid_installments+'</span></strong></div>'+
								'<div class="col-md-2" style="text-align: right;"><strong><span>'+indianCurrency.format(parseFloat(val.paid_amount).toFixed(2))+'</span></strong></div>'+
							  '</div>';
							  scheme_count+=parseInt(val.scheme_count);
							  scheme_ins+=parseInt(val.paid_installments);
							  scheme_amt+=parseInt(val.paid_amount);
							}
							if(val.is_lucky_draw==1){
								if(val.scheme_name != "" && val.scheme_count != ""){ 
								 grp = 0;
							   ...
							   onHTML+='<strong><span>'+val.scheme_name+'</span></strong><br>';
							   $.each(val.group_scheme,function(keys,vals){
							   onHTML+=' <div class="row">'+
												 '<div class="col-md-5" style="text-align: left;">'+vals.group_code+'</div>'+
												 '<div class="col-md-1"><lable> : </lable></div>'+
												 '<div class="col-md-2" style="text-align: right;"><strong><span>'+vals.count+'</span></strong></div>'+
												 '<div class="col-md-2" style="text-align: right;"><strong><span>'+vals.paid_installments+'</span></strong></div>'+
												 '<div class="col-md-2" style="text-align: right;"><strong><span>'+indianCurrency.format(parseFloat(vals.paid_amount).toFixed(2))+'</span></strong></div>'+
										  '</div> ' ;
										  grp+=parseInt(vals.count);
										  grp_ins+=parseInt(vals.paid_installments);
										  grp_paid+=parseInt(vals.paid_amount);
									   });
									  
									   onHTML+='<hr><div class="row">'+
									   '<div class="col-md-5" style="text-align: left;"> <strong>Total</strong></div>'+
									   '<div class="col-md-1"><lable> : </lable></div>'+
									   '<div class="col-md-2" style="text-align: right;"><strong><span>'+grp+'</span></strong></div>'+
									   '<div class="col-md-2" style="text-align: right;"><strong><span>'+ grp_ins+'</span></strong></div>'+
									   '<div class="col-md-2" style="text-align: right;"><strong><span>'+indianCurrency.format(parseFloat(grp_paid).toFixed(2))+'</span></strong></div>'+
									   '</div><hr>';
						...
						offHTML+='<div class="row"></div><hr>'+
								   ...
								   '<div class="col-md-5" style="text-align: left;"> <strong>Total </strong></div>'+
								   '<div class="col-md-1"><lable> : </lable></div>'+
								   '<div class="col-md-2" style="text-align: right;"><strong><span> '+scheme_count+' </span></strong></div>'+
								   '<div class="col-md-2" style="text-align: right;"><strong><span> '+scheme_ins+' </span></strong></div>'+
								   '<div class="col-md-2" style="text-align: right;"><strong><span> '+indianCurrency.format(parseFloat(scheme_amt).toFixed(2))+' </span></strong></div>'+  					 
								   '</div>';
						  $('#offline_modewise').html(offHTML);
						  $('#online_modewise').html(onHTML);
			   }		   
	                     ...
	                     if(data.schemes.length !== 0)
	                     {
	                         oTable = $('#scheme_wise_detail_report').dataTable({ 
										...
										"columnDefs": [{
                                                ...
                                                {
                                                    targets: [0,1,2,3,4,8,9,10,11,12,13],
                                                    className: 'dt-left'
                                                }
                                        ],
