    ...
    if($('#select_karigar').val()!='' && $('#select_karigar').val()!=null && $('#select_product').val()!='' && $('#select_product').val()!=null && $('#select_design').val()!='' && $('#select_design').val()!=null && $('#select_sub_design').val()!='' && $('#select_sub_design').val()!=null)
    {
        ...
        if(!check_yet_toapprove_status)
        {
            ...
            $.each(karigar_wastage_details.approved_details,function(key,items){
                if((items.id_karikar==$('#select_karigar').val()) && (($('#select_product').val()==items.id_product) || (items.id_product==0)) && (($('#select_sub_design').val()==items.id_sub_design) || (items.id_sub_design==0)) && (($('#select_design').val()==items.id_design) || (items.id_design==0)) )
                {
                    item_charge_details = [];
                ...
                $.each(data,function(key,items){
                    ...
                    if(!rowExists)
                    {
                        ...
                        $.each(items.stone_details,function(k,stnval){
                            ...
                            $.each(uom_details,function(key,item){
                                     if(item.uom_id==stnval.po_stone_uom)
                                     {
                                         if(((item.uom_short_code).toLowerCase() =='ct') && (item.divided_by_value!=null && item.divided_by_value!='')) //For Carat Need to convert into gram
                                         {
                        ...
                        +'<td><div class="form-group"><div class="input-group "><input class="form-control custom-inp add_less_wt" value="'+ parseFloat(stone_wt).toFixed(3) +'" onClick="create_new_empty_qc_issue_stone_item($(this).closest(\'tr\'));"  type="number" name="item[less_wt][]" step="any" readonly /><span class="input-group-addon input-sm add_tag_lwt" onClick="create_new_empty_est_cus_stone_item($(this).closest(\'tr\'));">+</span></div><input type="hidden" class="stone_details" value='+JSON.stringify(stone_details)+'><input type="hidden" class="qc_issue_less_wt" value="'+items.less_wt+'"></div></td>'
			...
			if (tagged_details != '') {
				...
				$.each(tagged_details, function (key, tagged_details) {
					...
					$.each(tagged_details, function (key, val) {
					    ...
					    tot_blc_gwt += parseFloat(parseFloat(val.inw_gwt)-parseFloat(val.pur_ret_gwt)-parseFloat(val.lot_gwt)-parseFloat(val.tag_gwt));
						tot_blc_nwt += parseFloat(parseFloat(val.inw_nwt)-parseFloat(val.pur_ret_nwt)-parseFloat(val.lot_nwt)-parseFloat(val.tag_nwt));
						tot_blc_pcs += parseFloat(parseFloat(val.inw_pcs)-parseFloat(val.pur_ret_pcs)-parseFloat(val.lot_pcs)-parseFloat(val.tag_pcs)); 
					    tot_blc_diawt = parseFloat(parseFloat(val.inw_stn_wt)-parseFloat(val.ret_stone_wt)-parseFloat(val.lot_stn_wt)-parseFloat(val.tag_stn_wt));
                        
                        blc_pcs+= parseFloat(parseFloat(val.inw_pcs)-parseFloat(val.pur_ret_pcs)-parseFloat(val.lot_pcs)-parseFloat(val.tag_pcs));
                        blc_gwt+= parseFloat(parseFloat(val.inw_gwt)-parseFloat(val.pur_ret_gwt)-parseFloat(val.lot_gwt)-parseFloat(val.tag_gwt));
                        blc_nwt+= parseFloat(parseFloat(val.inw_nwt)-parseFloat(val.pur_ret_nwt)-parseFloat(val.lot_nwt)-parseFloat(val.tag_nwt));
                        blc_diawt+= parseFloat(parseFloat(val.inw_stn_wt)-parseFloat(val.ret_stone_wt)-parseFloat(val.lot_stn_wt)-parseFloat(val.tag_stn_wt));
						...
						trHtml += '<tr>'
							...
							+ '<td>' + parseFloat(parseFloat(val.inw_pcs)-parseFloat(val.pur_ret_pcs)-parseFloat(val.lot_pcs)-parseFloat(val.tag_pcs)).toFixed(0)+ '</td>'
							+ '<td>' + parseFloat(parseFloat(val.inw_gwt)-parseFloat(val.pur_ret_gwt)-parseFloat(val.lot_gwt)-parseFloat(val.tag_gwt)).toFixed(3)+ '</td>'
							+ '<td>' + parseFloat(parseFloat(val.inw_nwt)-parseFloat(val.pur_ret_nwt)-parseFloat(val.lot_nwt)-parseFloat(val.tag_nwt)).toFixed(3) + '</td>'
							+ '<td>' + parseFloat(parseFloat(val.inw_stn_wt)-parseFloat(val.ret_stone_wt)-parseFloat(val.lot_stn_wt)-parseFloat(val.tag_stn_wt)).toFixed(3) + '</td>'
							+ '</tr>';
					});
