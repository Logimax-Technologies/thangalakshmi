...

		              {
					   ...
					   $(document).on("click","input[name='mc_va_attr_update']",function() {
						...
						$(document).on('click', '.delete_design_attribute', function(e) {
							...
							let del_status = delete_design_attributes(delete_href);
							...
							if(del_status) {
								...
								if(des_attr_rows == 1) {
									...
									$("#edit_mas_design_list tbody tr").find(`[data-id='${parent_id}']`).remove();
...
function delete_design_attributes(href)
{
	let del_design_attr = {}

	let result = _ajaxCallPost(href, del_design_attr)
			.then((data) => {
				if(data.status) {
			...
			.catch((error) => {
				console.log("Error On delete_design_attributes ",error);
