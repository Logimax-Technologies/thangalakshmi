...
function get_pocket_details()
{
    ...
    $.ajax({
        ...
        $("#select_pocket,#select_polish_pocket").select2(
        {
            placeholder:"Select Pocket",
            allowClear: true		    
        });
        $.each(data,function (key, item) {
            $('#select_pocket,#select_polish_pocket').append(
            $("<option></option>")
        ...
        $.each(pocketDetails,function(key,items){
            if(items.id_metal_pocket==id_metal_pocket)
            {
                ...
                trHtml+='<tr>'
                            ...
                            +'<td><input type="hidden" name="pocket[avg_purity][]" class="avg_purity" value="'+items.avg_purity+'">'+items.avg_purity+'</td>'
...
function calculate_polishing_pocketing_details()
{
    ...
    if( $('#polish_pocket_details > tbody tr').length>0)
    {
        ...
        avg_purity=parseFloat(tot_purity/$('#polish_pocket_details > tbody tr').length);
    }
