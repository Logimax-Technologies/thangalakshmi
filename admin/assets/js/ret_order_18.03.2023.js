...
function filterByCatId( prop, value){
    ...
    $(document).on('keyup change',".order_from,.category,.weight,.mc,.stn_amt,.mc_type", function(){ 
    	calculate_orderSale_value();
    });
...
// add new metal information 
	...
	function validateOrderDetailRow()
    {
    	...
    	$('#item_detail > tbody  > tr').each(function(index, tr) {
    		if($(this).find('.id_product').val() == "" || $(this).find('.category').val() == ""  || $(this).find('.purity').val() == ""  || $(this).find('.id_design').val() == "" || $(this).find('.sub_design').val() == "" || $(this).find('.weight').val() == "" || $(this).find('.qty').val() == "" ){
    			validate = false;
    		}
    	});
    	return validate;
    ...
    function calculate_orderSale_value()
    {
         $('#item_detail > tbody tr').each(function(idx, row){
            ...
            rate_per_grm = metal_rates.goldrate_22ct;
            }else{
            rate_per_grm = metal_rates.silverrate_1gm;
            }
            ...
            var value_charge = (isNaN(row.find('.value_charge').val()) || row.find('.value_charge').val() == '')? 0 : row.find('.value_charge').val();
            ...
            taxable = parseFloat(parseFloat(parseFloat(rate_per_grm) * (parseFloat(wast_wgt) + parseFloat(weight))) +parseFloat(mc)+parseFloat(stn_amt)+parseFloat(value_charge)).toFixed(2);  
    ...
    function create_new_empty_cus_order_row()
    {
			...
			var subproduct_required=$('#subproduct_required').val(); 
                ...
                "<td><input type='text' class='form-control weight' name='o_item["+i+"][weight]' placeholder='Enter The Weight'  required id='weight_"+i+"' autocomplete='off' style='width: 70px;' /></td>"+
                "<td><select class=size  class='form-control ' placeholder='Size' name='o_item["+i+"][size]' required='true' style='width: 70px;'/></td>"+
                "<td><input type='checkbox' class='is_set_items' name='o_item["+i+"][is_set_items]' value='0'/></td>"+
                "<td><input type='number' class='form-control qty' placeholder='Pcs' name='o_item["+i+"][totalitems]' required='true' style='width: 70px;'/></td>"+
                '<td><input type="number" class="form-control cus_due_day" style="width: 70px"; name="o_item['+i+'][cus_due_day]" ></td>'+
                "<td><input type='number' class='form-control wast_percent' placeholder='Wast %' name='o_item["+i+"][wast_percent]' required='true' style='width: 100px;'/></td>"+
                "<td><input type='number' class='form-control wast_wgt' placeholder='Wast Weight' name='o_item["+i+"][wast_wgt]' required='true' style='width: 70px;' readonly/></td>"+
                ...
                "<td><input type='number' step='any' class='form-control mc' placeholder='MC Amount' name='o_item["+i+"][mc]' style='width: 70px;'/></td>"+
                "<td><a href='#' onClick='create_new_empty_est_cus_charges_item($(this).closest(\"tr\"));' class='btn btn-success'><i class='fa fa-plus'></i></a><input type='hidden' class='charges_details' name='o_item["+i+"][charges_details]'></td>"+
                "<td><input type='number' step='any' class='form-control value_charge' placeholder='Charge Amount' name='o_item["+i+"][value_charge]' step='any' style='width: 100px;'readonly /></td>"+
                "<td><a href='#' onClick='create_new_empty_est_cus_stone_item($(this).closest(\"tr\"));' class='btn btn-success'><i class='fa fa-plus'></i></a><input type='hidden' class='stone_details' name='o_item["+i+"][stone_details]'></td>"+
                "<td><input type='number' step='any' class='form-control stn_amt' placeholder='Stone Amount' name='o_item["+i+"][stn_amt]' step='any' style='width: 100px;'readonly/></td>"+                "<td><input type='text' class='form-control order_amt' placeholder='Amount' name='o_item["+i+"][rate]' required='true' readonly='true' style='width: 70px;'/></td>"+
                '<td><a href="#" onClick="update_image_upload($(this).closest(\'tr\'));" class="btn btn-default btn-sm"><i class="fa fa-plus"></i></a><input type="hidden" class="order_img" name="o_item['+i+'][order_img]""></td>'+
                '<td><a href="#" onClick="update_order_description($(this).closest(\'tr\'));" class="btn btn-default btn-sm"><i class="fa fa-plus"></i></a><input type="hidden" class="order_des" name="o_item['+i+'][description]"></td>'+
                "</td>"+
                '<td><a href="#" onClick="$(this).closest(\'tr\').remove();" class="btn btn-danger btn-del"><i class="fa fa-trash"></i></a></td>'
