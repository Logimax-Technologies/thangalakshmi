...
function add_to_trans(trans_data){
	...
	if(trans_type == 1){ 
		...
		postData = {'trans_data': trans_data,'transfer_from' : from_brn,'transfer_to' : to_brn,'item_tag_type' :trans_type, 'grs_wt' : grs_wt, 'net_wt':net_wt, 'pieces' : pieces, 'isOtherIssue' : isOtherIssue,'is_eda':1,'form_secret' :form_secret}; 
	}
	else if(trans_type == 2){
		// Non Tagged
		postData = {'trans_data': trans_data,'transfer_from' : from_brn,'transfer_to' : to_brn,'item_tag_type' :trans_type, 'isOtherIssue' : isOtherIssue,'is_eda':1,'form_secret' :form_secret};
	}
	else if(trans_type == 3)	// Old Metal
	{
		grs_wt = $(".old_prev_grs_wt").val();
		net_wt = $(".old_prev_net_wt").val();
		postData = {'trans_data': trans_data,'transfer_from' : from_brn,'transfer_to' : 1,'item_tag_type' :trans_type,'grs_wt' : grs_wt, 'net_wt':net_wt,'is_eda':is_eda,'form_secret' :form_secret};
	}
	else if(trans_type == 4) // Packaging Items
	{
	    ...
	    postData = {'trans_data': trans_data,'transfer_from' : from_brn,'transfer_to' : to_brn,'item_tag_type' :trans_type,'pieces':total_pcs,'is_eda':1,'form_secret' :form_secret};
	}
	else if(trans_type == 5){
		// Repair Order
		grs_wt = $(".total_weight").html();
		pieces = $(".total_items").html();
		postData = {'trans_data': trans_data,'transfer_from' : from_brn,'transfer_to' : to_brn,'item_tag_type' :trans_type, 'grs_wt' : grs_wt, 'pieces' : pieces, 'isOtherIssue' : isOtherIssue,'is_eda':1,'form_secret' :form_secret};
	}
	$.ajax({		
	 	type: 'POST',		
