...
$(document).ready(function() {
				 	...
				 	case 'add':
						...
						{				 	   
							$(document).on('keypress',function(e){
								...
								{
									if($('#ret_sales_type').val()==1) //IF ret_sales_type is 1 Normal Sale,2-No2 Sale
									{
										if($('#allow_other_bill_types').val()==1)
										{
											$('#ret_sales_type').val(2);
											$('#bill_cus_name').css("background-color","red");
										}
									}else{
										$('#ret_sales_type').val(1);
	...
	$('#pay_submit').on('click',function(){
		...
		else if(($('#billing_sale_details >tbody>tr').length>0) && bill_type!=11)
		{
    	    ...
    	    if(($('.sales_sgst').html()==0 && $('.sales_cgst').html()==0) && ($('.sales_igst').html()==0 && $('#ret_sales_type').val()==1))
		    {
                   ...
                   total_est_discount = data.responsedata.item_details[0].est_discount;
					...
					var tag_other_itm_amount=0;
						...
						if(estval.tag_id=='')
						{
							...
							$.each(estval.stone_details,function(key,item){
								...
								stone_details.push({'stone_id' : item.stone_id,'stone_pcs' :item.pieces,'stone_wt':item.wt,'stone_price':item.amount,'certification_cost':item.certification_cost,'stone_name':item.stone_name,'is_apply_in_lwt':item.is_apply_in_lwt,'stone_cal_type':item.stone_cal_type,'rate_per_gram':item.rate_per_gram});
							});
					...
					if(estval.is_non_tag==1)
					    ...
					    if(!rowExist)
					    {
                            ...
                            +'<td><span>'+estval.tag_code+'</span><input type="hidden" class="sale_tag_id" name="sale[tag][]" value="'+estval.tag_id+'" /><input type="hidden" name="sale[total_cgst][]" class="sale_cgst"/><input type="hidden" class="sale_sgst" name="sale[total_sgst][]"/><input type="hidden" class="sale_igst" name="sale[total_igst][]"/></td>'
...
$('#disc_apply').on('click',function(){
	...
	if(disc_limit_type==2) //2-Percentage,1-Amount
	 ...
	 if($('#summary_discount_amt').val()>parseFloat(emp_disc_limit))
     {
         ...
         {
             $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br> Maximum Discount Limit is Rs."+emp_disc_limit});	
...
function calculateSaleBillRowTotal()
{
	...
	$('#billing_sale_details > tbody tr').each(function(idx, row){
		...
		var ret_sales_type = $('#ret_sales_type').val();
                    ...
                    if(parseFloat(mc_type) < parseFloat(discount))
        	        {
        	            ...
        	            if(blc_discount > 0)
        	            {
        	                if(wast_wgt_amt > 0)
        	                {
        	                    ...
        	                    {
        	                        wastage_discount = parseFloat(parseFloat(wast_wgt_amt) - parseFloat(blc_discount)).toFixed(2);
        	                    }
		...
		{*/
		    if(cus_del_country==cmp_country && ret_sales_type==1)
		    {
	   ...
	   curRow.find('.mc_discount').val(parseFloat(mc_discount).toFixed(2));
		...
		curRow.find('.est_wastage').html(parseFloat(parseFloat(parseFloat(parseFloat(wast_wgt)-parseFloat(discount_weight))*100)/net_wt).toFixed(3));
...
function add_cutomer(){ //replace the function
    var billing_for = $("input[name='billing[billing_for]']:checked").val();
...
function update_cutomer(){ ////replace the function
    var billing_for = $("input[name='billing[billing_for]']:checked").val();
	var form_data = new FormData();
	form_data.append('cusName',$('#cus_first_name').val());
	form_data.append('cusMobile',$('#cus_mobile').val());
...
function calculatePaymentCost()
{
	...
	if(bill_type!=10)
	{
    	...
    	else if(pay_to_cus>0)
    	{
    		...
    		if(make_pay_cash<10000 && allowsubmit)
    		{
    ...
    if(((parseFloat(sales_amount))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#ret_sales_type')==1))
	{
	...
	if(((parseFloat(purchase_amt))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 ($('#ret_sales_type')==1))
	{
        if($('#pan_no').val()=='' || $('#pan_no').val()==0)
        {
            $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Enter The PAN No"});
	...
	if(((parseFloat(sale_return))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#ret_sales_type')==1))
	{
        if($('#pan_no').val()=='' || $('#pan_no').val()==0)
        {
            $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Enter The PAN No"});
	...
	if(((parseFloat(order_advance_recd_amt))>=parseFloat($('#min_pan_amt').val())) && $('#is_pan_required').val()==1 && ($('#ret_sales_type')==1))
	{
        if($('#pan_no').val()=='' || $('#pan_no').val()==0)
        {
            $.toaster({ priority : 'danger', title : 'Warning!', message : ''+"</br>Enter The PAN No"});
...
function calculate_est_chit_closing_balance()
{
    ...
    $.each(chit_details,function(k,val){
        ...
        /*if(is_wast_and_mc_benefit_apply==1 && scheme_type==0)
		{
		    ...
		    saving_weight = parseFloat(chit_amount/goldrate_22ct).toFixed(3);
		    var closing_weight = (parseFloat(saving_weight).toFixed(3));
		}
		else
		{
		    saving_weight = val.closing_weight;
		    ...
		    {
		        /*var bill_type = $("input[name='billing[bill_type]']:checked").val();
		        if(val.scheme_type != 0 && bill_type!=10){
                    chit_amount = parseFloat(val.closing_balance * parseFloat($('#goldrate_22ct').val())).toFixed(2);
                }else
                {
                    chit_amount = val.closing_amount;
                }*/
...
function getBillDetails(billNo, billType){
	...
	$.ajax({
        ...
        dataType: "json", 
        method: "POST", 
        data: {'billNo' : billNo, 'billType' : billType, 'id_branch' : $("#id_branch").val(),'fin_year':$('#bill_fin_year_select').val(),'metal_type':metal_type},
        success: function (data) { 
			if(data.success == true){
				$('#bill-detail-popup').modal('toggle');
				// BILL SOLD ITEMS
...
function create_new_empty_bill_sales_stone_item(curRow,id)
{
	...
	if(stone_details.length>0)
	{
		...
		console.log(pitem);
		row+='<tr>'
		    +'<td>'+(pitem['is_apply_in_lwt']==1 ? 'Yes' :'No')+'</td>'
		    +'<td>'+(pitem['stone_name'])+'</td>'
		    +'<td>'+(pitem['stone_pcs'])+'</td>'
		    +'<td>'+(pitem['stone_wt'])+'</td>'
		    +'<td>'+(pitem['stone_cal_type']==1 ? 'By Wt' :'By Pcs')+'</td>'
		    +'<td>'+(pitem['rate_per_gram'])+'</td>'
		    +'<td>'+(pitem['stone_price'])+'</td>'
		   +'</tr>';
...
$(document).on('change',".balance_amount,.discount_amt",function(){
    var row = $(this).closest('tr'); 
    var issue_amt       = parseFloat(row.find('.issue_amt').val()).toFixed(2);
    var payable         = parseFloat(row.find('.balance_amount').val()!='' ? row.find('.balance_amount').val():0).toFixed(2);
    ...
    var balance_amount        = (parseFloat(issue_amt)-parseFloat(receipt_blc_amt));
    console.log('receipt_blc_amt:'+receipt_blc_amt);
    console.log('payable:'+payable);
    if(parseFloat(receipt_blc_amt) < parseFloat(parseFloat(payable)+parseFloat(discount_amt)))
    {
        $.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br> Please Enter The Valid Amount"});
        row.find('.balance_amount').val(0);
        row.find('.discount_amt').val(0);
    }
    else if(parseFloat(receipt_blc_amt) < parseFloat(discount_amt))
    {
        $.toaster({ priority : 'danger', title : 'Success!', message : ''+"</br> Please Enter The Valid Amount"});
        row.find('.balance_amount').val(0);
        row.find('.discount_amt').val(0);
    ...
    /*else
    {
        balance_amount   = parseFloat(receipt_blc_amt)-parseFloat(discount_amt);
        row.find('.balance_amount').val(parseFloat(balance_amount).toFixed(2));
    }*/
   ...
   /* else
    {
        discount_amount  = (row.find('.discount_amt').val()!='' ? row.find('.discount_amt').val() :0);
        balance_amount   = parseFloat(row.find('.payable').val())-parseFloat(row.find('.paid_amt').val())-parseFloat(discount_amount);
        row.find('.balance_amount').val(parseFloat(balance_amount).toFixed(2));
    }*/
});
...
function discount_validation_for_salebill()
{
	...
	let total_payment_amount = parseFloat($(".sale_amt_with_tax").html());
	let summary_discount_amt = parseFloat($("#summary_discount_amt").val());
	...
	if(total_payment_amount > 0 && summary_discount_amt > 0) {
		...
		//calculate_sales_details();  //Calculate Sales Details
		//calculate_purchase_details();	//Calculate Purchase Details
		let amt_without_disc = parseFloat(total_payment_amount) - parseFloat(summary_discount_amt);
		console.log("amt_without_disc ", amt_without_disc);
		let _discount_limit = parseFloat(total_payment_amount) - parseFloat(total_amount_with_mc_va_limit);
		if(parseFloat(amt_without_disc) >= parseFloat(total_amount_with_mc_va_limit)) {
