...
$('#get_tag_details').on('click',function(){
	...
	$.ajax({
        ...
        data: {'id_branch':id_branch,'tag_id':tag_id,'id_mc_type':id_mc_type,'lot_product':lot_product,'from_weight':from_weight,'to_weight':to_weight,'mc_value':mc_value,'making_per':making_per,'id_design':id_design,'id_sub_design':id_sub_design}, 
...

{
	...
	var row_cls = $('#estimation_stone_cus_item_details tbody tr').length;
        ...
        row = '<tr id="'+$('#estimation_stone_cus_item_details tbody tr').length+'" class="st_'+$('#estimation_stone_cus_item_details tbody tr').length+'">'
            ...
            //+'<td><input class="show_in_lwt" type="checkbox"name="est_stones_item[show_in_lwt][]" value="1" '+show_in_lwt+'></td>'

            +'<td><select class="show_in_lwt form-control" name="est_stones_item[show_in_lwt][]"><option value="">-Select-</option><option value=1 '+(show_in_lwt==1 ? 'selected' :'')+'>Yes</option><option value=0 '+(show_in_lwt==0 ? 'selected' :'')+'>No</option></select></td>'
			...
			+'<td><div class="input-group"><input class="stone_wt form-control" type="number" name="est_stones_item[stone_wt][]" value="'+stone_wt+'" style="width:100%;"/><span class="input-group-btn" style="width: 100px;"><select class="stone_uom_id form-control" name="est_stones_item[uom_id][]">'+uom_list+'</select></span></div></td>'
...

{
	...
	success:function(data){
...

		{
		    ...
		    $("#tag_size").select2("val",(id!='' && id>0?id:''));
...

{
		...
		data:{'lot_id':$("#tag_edit_lot").val(),'id_branch':$("#branch_select").val(),'id_product':$('#prod_select').val(),'tag_code':$('#tag_code').val(),'est_no':$('#est_no').val()},
    			 ...
    			 $.each(data,function(key,row){
...

    			        {
    			              ...
    			              trHtml+='<tr>'
    			                ...
    			                +'<td><input type="checkbox" class="tag_id" name="tag_id[]" value="'+row.tag_id+'"/><input type="hidden" class="design_id" name="design_id[]" value="'+row.design_id+'"/><input type="hidden" class="id_sub_design" name="id_sub_design[]" value="'+row.id_sub_design+'"/><input type="hidden" class="size" name="size[]" value="'+row.size+'"/>'+row.tag_code+'</td>'
...
function get_tag_edit_lots(){
...

    {
        ...
        data:  {'id_design':$('#des_select').val(),'id_size':$('#select_size').val(),'id_sub_design':$('#sub_des_select').val(),'req_data':data},
...
$("#reset_tag_form").on('click',function(e){
	...
	$("#update_attribute_detail tbody").empty();
...
$(document).on('input keyup', '#tag_pcs,#tag_gwt,#tag_lwt,#tag_wast_perc,#tag_mc_value,#tag_id_mc_type,#tag_calculation_based_on,#tag_sell_rate', function(e){
	...
	if(this.id == 'tag_pcs') {
		...
		var tag_act_blc_pcs = $("#tag_act_blc_pcs").val() == '' ? 0 : parseFloat($("#tag_act_blc_pcs").val());
		...
		if(tag_blc_pcs > 0){
			...
			if(tag_input_pcs > tag_blc_pcs){
				...
				$("#tag_blc_pcs_disp").html("Blc : "+parseFloat(blc_pc));
		...
		var tag_act_gross_blc = $("#tag_act_gross_blc").val() == '' ? 0 : parseFloat($("#tag_act_gross_blc").val());

		if(tag_blc_wgt >0){

			if(tag_input_wgt > tag_blc_wgt){

				$("#tag_gwt").val(parseFloat(tag_act_gross_blc).toFixed(3));
				...
				var blc_wt = parseFloat(tag_blc_wgt) - parseFloat(tag_input_wgt);

				$("#tag_blc_gross_disp").html("Blc : "+ parseFloat(blc_wt).toFixed(3));
...

{
	...
	let attributesRow = $("#update_attribute_detail tbody tr");

	let attr_preview = [];

	$.each(attributesRow, function (attrkey, attritem) {

		let attr_id 	= $(attritem).find(".tag_upd_attr_name").val();

		let attr_val_id = $(attritem).find(".tag_upd_attr_value").val();
		...
		attr_preview.push(attrs);

	});

	let attrs_json = JSON.stringify(attr_preview);

	var attrsPostRow = "<input type='hidden' class='tag_attributes' name='lt_item[attrs][]' value='"+attrs_json+"' />";
...
function edit_tag(curRow){
	...
	//load_attributes_on_edit(item.find('.tag_attributes').val());
...
$('#cus_chargeModal .modal-body .create_charge_item_details').on('click', function(){
...

{
	...
	if(charges_validated) {
	    ...
	    var row_cls = $('#table_charges tbody tr').length;

		let new_row = "<tr class='ch_"+row_cls+"'><td><select class='chargesType'><option value=''>--Select--</option>"+options+"</select></td><td><input type='text' value='"+(selData ? (selData.charge_value == undefined ? 0 : selData.charge_value) : '')+"' class='chargesValue' /></td><td><button type='button' class='btn btn-success btn-xs create_charge_item_details'><i class='fa fa-plus'></i></button>&nbsp;<button type='button' class='btn btn-danger btn-xs' onclick='remove_charge(this)'><i class='fa fa-trash'></i></button></td></tr>";
...

{
    ...
    var row_validated = true;

    $('#table_charges > tbody  > tr').each(function(index, tr) {

        console.log(parseFloat($(this).find('td:first .chargesType').val()) +'--'+ parseFloat($(this).find('td:eq(1) .chargesValue').val()) );

		if(parseFloat($(this).find('td:first .chargesType').val()) > 0 || parseFloat($(this).find('td:eq(1) .chargesValue').val()) > 0){
...

    {
    	...
    	$('#cus_chargeModal .modal-body #table_charges> tbody  > tr').each(function(index, tr) {
    		...
    		charge_details.push({
    		            ...
    		            'charge_name'       : $(this).find('.chargesType :selected').text(),
...

{
	...
	data :{'design_no':$('#des_select').val(),'id_product':$('#tag_lt_prod').val()},
...

{
    ...
    var weight_per=$('#weight_per').val();
...

        {
            ...
            $('#tag_blc_pcs').val(parseFloat(lot_bal_pcs-total_used_pcs));

            $('#tag_blc_gross').val(parseFloat((((lot_bal_wt*weight_per)/100)+lot_bal_wt)-total_used_wt));

        }else{

            $('#tag_blc_pcs').val(parseFloat(lot_bal_pcs-total_used_pcs));

            $('#tag_blc_gross').val(parseFloat(lot_bal_wt-total_used_wt));
        ...
        $('#tag_blc_gross_disp').html(parseFloat(lot_bal_wt-total_used_wt).toFixed(3));

        $("#tag_blc_pcs_disp").html("Blc : "+parseFloat(lot_bal_pcs-total_used_pcs-1));
...
/*function load_attributes_on_edit(attr_data)
{
...
function add_tag_attribute(selected_attr_id = 0, selected_attr_val_id = 0)
{
	if(validate_tagAttr_row()) {
		...
		if(check_has_attributes()) {
			...
			let attr_lastObj 	= $('#update_attribute_detail .attr_row:last-child').find('.tag_upd_attr_name');
			let attrVal_lastObj = $('#update_attribute_detail .attr_row:last-child').find('.tag_upd_attr_value');
			...
			$.toaster({ priority : 'warning', title : 'Warning!', message : ''+"</br>"+'No Attributes Available. All Are Filled.'});
...
{
	let all_selected_attribute = getall_selected_attribute();
...
function getall_selected_attribute()
{
	let attr_selected_ids = [];

	let tableRows = $("#update_attribute_detail tbody tr");
...
function validate_tagAttr_row()
{
	...
	let tag_attr_rows = $("#update_attribute_detail tbody tr");
...
function add_attribute_buttons() {
	...
	let attr_rows = $("#update_attribute_detail tbody");
...
function display_attribute_details()
{
	...
	$.each(tag_attr_rows, function (attrkey, attritem) {
		...
		let attr_name 	= $(attritem).find('.tag_upd_attr_name option:selected').text();
		let attr_value 	= $(attritem).find('.tag_upd_attr_value option:selected').text();
