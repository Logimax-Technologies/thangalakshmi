    ...
    //console.log(ctrl_page[1]);
     ...
     switch(ctrl_page[1]){ // Retail masters
		...
		case 'karigar':
						...
						$(document).on('click','.add_stone,.add_karigar_stones',function()
						{
							add_karigar_stones();
						});
						$(document).on('click', '.remove_karigar_stones', function() {
							remove_karigar_stones($(this));
						});
						$(document).on('change', '.stone_type', function() {
							if($(this).val() > 0) {
								getActiveStonesForKarigar($(this));
							} else {
...
{ 
  ...
  $.ajax({
	...
	$('#karigar_calc_type').on('change',function(){
	    var karigar_calc_type = this.value;
	    $('#product_item_list > tbody  > tr').each(function(index, tr) {
	        $(this).find('.wast_percent').prop("disabled",false);
	        $(this).find('.mc').prop("disabled",false);
	        $(this).find('.mc_type').prop("disabled",false);
	        if(karigar_calc_type==2)
	        {
	            $(this).find('.wast_percent').prop("disabled",true);
	            $(this).find('.mc').prop("disabled",true);
	            $(this).find('.mc_type').prop("disabled",true);
	        }
    	});
	});
		...
		success:function(data)
		{
			...
			if (list!= null && list.length > 0)
			{
				...
				$.each(list,function(key,val)
                {
					...
					'<td class="wast_per">'+val.wastage_per+'</td>'+
					'<td class="mc">'+val.type+'<input type="hidden" class="mc_type" value="'+val.mc_type+'"/></td>'+
					'<td class="mc_value">'+val.mc_value+'</td>'+
					'<td class="pur_touch">'+val.pur_touch+'</td>'+
...
$('#status_submit').on('click',function()
{
	...
	if(approval_type==0)
	{
		$('#karigar_wastage_list tbody tr').each(function (index,value)
		{
			...
			{
				ApprovedList.push({"id_karikar_wast":row.find('.kar_wast_id').val(),"id_karigar":row.find('.id_karigar').val(),"pro_id":row.find('.pro_id').val(),"des_id":row.find('.des_id').val(),"sub_des_id":row.find('.sub_des_id').val()});
			}
		});
	}
	else if(approval_type==1)
...
function validateOrderDetailRow()
{
	...
	$('#product_item_list > tbody  > tr').each(function(index, tr) {
		...
		else if(karigar_calc_type==2 && $(this).find('.pur_touch').val()=='')
		{
		    	validate = false;
		}
		else if(karigar_calc_type!=2 && ($(this).find('.wast_percent').val()=='' || $(this).find('.mc').val()==''))
		{
		    	validate = false;
		}
	});
...
function create_new_empty_karigar_product_detail()
{
		...
		$("#i_increment").val(i); 
            ...
            "<td><input type='number' class='form-control wast_percent' placeholder='Wast %' name='o_item["+i+"][wast_percent]' required='true'/></td>"+
            ...
            "<td><input type='number' step='any' class='form-control mc' placeholder='MC Amount' name='o_item["+i+"][mc]'/></td>"+
            "<td><input type='number' step='any' class='form-control pur_touch' placeholder='Touch' name='o_item["+i+"][pur_touch]'/></td>"+
	...
	$.ajax({
        ...
        success: function (data) {  
	        ...
	        $.each(data.karigar_wastages, function (key, item) {
			    ...
			    "<td><input class='form-control wast_percent' placeholder='wastages ' name='o_item["+i+"][wastage]' required='true' value='"+item.wastage_per+"' id='wastage"+i+"' style='width:70px' "+(karigar_calc_type==2 ? 'disabled' :'')+" disabled /></td>"+
				"<td><select class='mc_type' class='form-control' name='o_item["+i+"][id_mc_type]' style='width: 100px;' disabled "+(karigar_calc_type==2 ? 'disabled' :'')+"  ><option value='1' "+(item.mc_type==1 ? 'selected' :'')+">Gram</option><option value='2' "+(item.mc_type==2 ? 'selected' :'')+">Piece</option></select></td>"+
				"<td><input class='mc form-control' placeholder='mc ' name='o_item["+i+"][mc]' required='true' value='"+item.mc_value+"' disabled "+(karigar_calc_type==2 ? 'disabled' :'')+"  id='mc"+i+"' style='width:70px'/></td>"+
                "<td><input type='number' step='any' class='form-control pur_touch' placeholder='Touch' name='o_item["+i+"][pur_touch]' disabled value='"+item.pur_touch+"' /></td>"+
                "<td><a href='#' onClick='update_image_upload($(this).closest(\"tr\"));' class='btn btn-default btn-sm'><i class='fa fa-plus'></i></a><input type='hidden' class='order_img' name='o_item['+i+'][order_img]'></td>"+
