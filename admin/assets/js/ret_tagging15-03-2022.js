...
function calculateTagFormSaleValue(row)
{
	...
	*/
	...
	if(current_po_details.length > 0){ // Purchase cost calculation 
        ...
        var mctax               = 3;
    	purchase_cost = parseFloat(((parseFloat(pure_wt))*parseFloat(po_rate_per_gram))+parseFloat(pur_mc_type == 1 ? ((pur_mc_val * gross_wt) * ((100 + mctax) / 100)) : ((pur_mc_val * tot_pcs) * ((100 + mctax) / 100)))+parseFloat(stone_po_price * (103/100))).toFixed(2);
...
function calculate_other_metal_amount() {
    ...
    $('#other_metal_table > tbody  > tr').each(function(index, tr) {
        ...
        var mc_value        = (mc_type==1 ? parseFloat(net_wt*making_charge) : (mc_type==2 ? parseFloat(making_charge*piece) :0));
		...
		success:function(data){
			 ...
			 {  	
				oTable = $('#old_metal_sale_list').dataTable({
						...
						"aoColumns": [
						{ "mDataProp": function ( row, type, val, meta ){ 
		                	chekbox='<input type="checkbox" class="old_metal_sale_id" name="old_metal_sale_id[]" value="'+row.old_metal_sale_id+'"/><input type="hidden" class="purity" value="'+row.purity+'"><input type="hidden" class="net_wt" value="'+row.net_wt+'"><input type="hidden" class="gross_wt" value="'+row.net_wt+'">' 
		...
		success:function(data){
			 ...
			 {  	
				oTable = $('#retagging_list').dataTable({
						...
						"aoColumns": [
						{ "mDataProp": function ( row, type, val, meta ){ 
		                	chekbox='<input type="checkbox" class="tag_id" name="tag_id[]" value="'+row.tag_id+'"/><input type="hidden" class="product_short_code" value="'+row.product_short_code+'">' 
		                	return chekbox+" "+row.tag_id;
		                }},	
						{ "mDataProp": "tag_code" },
						{ "mDataProp": "bill_date" },
...
function create_retag(data="")
{
    ...
    data:  {'req_data':data,'id_branch':$("#id_branch").val(),'tag_process':$('#tag_process').val(),'report_type':$('#report_type').val(),'id_product':$('#prod_select').val(),'id_design':$('#des_select').val(),'id_sub_design':$('#sub_des_select').val()},
    type:"POST",
    async:false,
    dataType: "json", 
    success:function(data)
    {
        if(data.status)
        {
                ...
                {
                    	window.open(base_url+'index.php/admin_ret_tagging/tagging/generate_barcode?tag='+data.tag_ids,'_blank');
                }
