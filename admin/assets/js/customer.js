	   ...
	   $('#cus_create').submit(function(e)  {
	    ...
	    { 
			...
			var to_date=(date.getFullYear()+"-"+(date.getMonth() + 1)+"-"+date.getDate());
					...
					$('#customer_list1').text(moment().startOf('month').format('YYYY-MM-DD'));

					$('#customer_list2').text(moment().endOf('month').format('YYYY-MM-DD'));	
...
$("#cus_pincode").on('keyup',function(event) 
...
function validate_pincode()
...

 {
...

	  {
		...
		if(!regexp.test($("#cus_pincode").val()))
...

		{
			 ...
			 $('#cus_pincode').attr('placeholder', 'Not a valid Pincode.');
...
$('#firstname,#lastname,#email,#address1,#address2,#address3,#cus_pincode').on('keyup',function(){
...

{
...

    {
     ...
     if($('#cus_pincode').val()!='' && $('#cus_pincode').val()!=null)
...
$('#update_profile').on('click',function(){
    ...
    else if($('#cus_pincode').val()!='' && $('#cus_pincode').val().length < 6 )
...
$("#aadharid,#mobile").bind("cut copy paste",function(e) 
