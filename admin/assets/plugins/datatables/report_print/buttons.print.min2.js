(function(d) {
    ...
    i.ext.buttons.print = {
        ...
        action: function(a, b, h, e) {
			...
			// before code			e.footer && c.footer && (a += "<tfoot>" + k(c.footer, "th") + "</tfoot>");
			//e.footer && (a += "<tfoot>" + k(c.footer, "th") + "</tfoot>");
			...
			var MyRows = $('table#schdatewise_report').find('tfoot').find('tr');
			for (var i = 0; i < MyRows.length; i++) {	
				var paydate = $(MyRows[i]).find('td:eq(0)').text();
				var group = $(MyRows[i]).find('td:eq(1)').text();
				var noofrep = $(MyRows[i]).find('td:eq(2)').text();
				var payamt = $(MyRows[i]).find('td:eq(3)').text();
				var sgst = $(MyRows[i]).find('td:eq(4)').text();
				var cgst = $(MyRows[i]).find('td:eq(5)').text();
				var tgst = $(MyRows[i]).find('td:eq(6)').text();
				var tot = $(MyRows[i]).find('td:eq(7)').text();
			var MyIndex=[paydate,group,noofrep,payamt,sgst,cgst,tgst,tot];
		     MyIndexValue.push(MyIndex);
	       }
		}else{
			...
			for (var i = 0; i < MyRows.length; i++) {	
				...
				var noofrep = $(MyRows[i]).find('td:eq(2)').text();
				var payamt = $(MyRows[i]).find('td:eq(3)').text();
				var tot = $(MyRows[i]).find('td:eq(4)').text();
			var MyIndex=[paydate,group,noofrep,payamt,tot];
		     MyIndexValue.push(MyIndex);
	       }
	...
	for (var a = a + "<tfoot>", l = 0, i = MyIndexValue.length; l <
                i; l++)
				a += k(MyIndexValue[l], "td");
            a += "</tfoot>";
		 	// change code new report
