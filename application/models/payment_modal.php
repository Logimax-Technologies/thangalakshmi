...
,cs.wallet_points
  ...
  as paid_installments,
	...
	{
	  $query_customer = $this->db->query("SELECT cus.nominee_name,cus.nominee_mobile, cus.nominee_relationship,reference_no,cus.id_employee,cus.last_sync_time,
		cus.nominee_address1,cus.nominee_address2,cus.id_customer,cus.mobile,cus.id_branch,cs.branchWiseLogin,cs.is_branchwise_cus_reg,
        ...
        */
        ...
        if($luckyDraw['is_lucky_draw'] == 1 && $luckyDraw['max_members'] > 0 ){
            ...
            if($data['scheme_wise_acc_no'] == 1 && $branch > 0){ // 1 - Common with branch wise,
                 ...
                 ".($grpCode!=null && $grpCode !=''? "AND sa.group_code='".$grpCode."'" :'AND sa.added_by != 6')."
            ...
            else if($data['scheme_wise_acc_no'] == 5) // financial year with scheme wise
            {
                ...
                $sql = "SELECT max(sa.scheme_acc_number) as lastSchAcc_no FROM scheme_account sa 
                left join customer c on c.id_customer= sa.id_customer
                ".($grpCode!=null && $grpCode !=''? "WHERE sa.group_code='".$grpCode."'" :'WHERE sa.added_by != 6')."
        ...
        */
        ...
        if($data['branch_settings']==1){ // Branch Enabled
            if($data['scheme_wise_receipt'] == 2 && $branch >0){  // 2 - branch wise Receipt number
                $sql = "Select max(p.receipt_no) as receipt_no
    				From payment p
    				Where p.added_by != 6 and p.payment_status=1   and p.id_branch=".$branch." group by id_branch ";
            }
            else if($data['scheme_wise_receipt'] == 3){ // 3 - Scheme-wise Receipt number
                $sql = "Select max(p.receipt_no) as receipt_no
    				From payment p
        				left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
    				Where p.added_by != 6 and p.payment_status=1 and sa.id_scheme=".$id_scheme." group by id_scheme";
            }
            else if($data['scheme_wise_receipt'] == 4){ // 4 - Scheme-wise with branch wise Receipt number
                $sql = "Select max(p.receipt_no) as receipt_no
    				From payment p
        				left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
    				Where p.added_by != 6 and p.payment_status=1 and sa.id_scheme=".$id_scheme." AND p.id_branch=".$branch."  group by id_scheme,p.id_branch ";
            }
                ...
                $financial_year = $res->row()->fin_date;
                 $sql = "Select p.receipt_no
    				...
    				left join scheme_account sa on sa.id_scheme_account=p.id_scheme_account
        			left join customer c on c.id_customer= sa.id_customer
    				Where p.added_by != 6 and  date(date_payment) BETWEEN '".$financial_year."' AND DATE(CURDATE()) and p.payment_status=1 order by receipt_no desc limit 0,1 ";
            }
            ...
            else  // If other cases fails,generate 1 - common Receipt number
            {
                $sql = "Select receipt_no
    				From payment p
    				Where p.added_by != 6 and  p.payment_status=1 order by receipt_no desc limit 0,1 "; 
            }
        } else{
