 ...
 /*-- / Coded by ARVK --*/	
	...
	function get_products($id_product)
   {
	...
	function get_SMS_OTPdata($service_id,$customer_data){
        $sms_msg = "";
        $sql = $this->db->query("Select c.comp_name_in_sms as comp_name_sms,c.id_company,c.company_name as cmp_name,c.short_code,c.pincode,c.mobile,c.phone as cmp_ph,c.email,c.website,c.address1,c.address2,c.id_country,c.id_state,c.id_city,ct.name as city,s.name as state,cy.name as country,cs.currency_symbol,cs.currency_name
				from company c
        ...
        if($sql->num_rows() == 1){ 
            ...
            $customer_data['comp_name_sms'] = $cust_data['comp_name_sms'];
        }
