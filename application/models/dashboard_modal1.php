...
class Dashboard_modal extends CI_Model {
...

	{
		...
		$query = $this->db->query("SELECT cs.has_lucky_draw,id_payment,DATE_FORMAT(date_payment,'%d-%m-%Y') AS date_payment,sch.scheme_name AS scheme_name,metal_weight,payment_amount,ref_no AS client_id,sch.code,if(cs.has_lucky_draw=1,concat(ifnull(schAcc.group_code,''),' ',ifnull(schAcc.scheme_acc_number,'Not allocated')),concat(ifnull(sch.code,''),' ',ifnull(schAcc.scheme_acc_number,'Not allocated')))as scheme_acc_number,psm.payment_status AS payment_status,if(payment_mode='CC','Credit Card',if(payment_mode='NB','Net Banking',if(payment_mode='CD','Cheque or DD',if(payment_mode='CO','Cash Pick Up',if(payment_mode='OP','Other',if(payment_mode='DC','Debit Card',pm.mode_name)))))) as payment_mode
