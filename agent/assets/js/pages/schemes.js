...
//existing reg sub//
	...
	//check valid customer/emp referal code 
	...
	//check valid agent referal code 		
			if($("#agent_code").val() != "" && $("#agent_code").val() != null)
			{
				$('.overlay').css('display','block');
				$.ajax({
					type: 'POST',
					data:{'agent_code':$("#agent_code").val()},
					url:  baseURL+'index.php/chitscheme/check_agent_refcode',
					dataType: 'json',
					success: function(data) {
						if(data.status==1){
							isValidAgentRefCode = true;	
							if(isValidRefCode && isValidAgentRefCode && isValidPan && isValidAcName && validAmt){
								$("#schemeForm").submit(); 
							}
						}
						else
						{     
							alert(data.msg);
							$('#agent_code').val('');
							$('#agent_code').attr("placeholder",data.msg);	
							isValidAgentRefCode = false;	
						}	
					},error:function(error)  
					{
