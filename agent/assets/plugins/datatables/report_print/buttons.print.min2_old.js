    ...
    i.ext.buttons.print = {
        ...
        action: function(a, b, h, e) {
			...
			for (var i = 0; i < MyRows.length; i++) {	
				var group = $(MyRows[i]).find('td:eq(0)').text();
				var noofreceipt = $(MyRows[i]).find('td:eq(1)').text();
				var gst = $(MyRows[i]).find('td:eq(2)').text();
				var amount = $(MyRows[i]).find('td:eq(3)').text();
			var MyIndex=[group,noofreceipt,gst,amount];
